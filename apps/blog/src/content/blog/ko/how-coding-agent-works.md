...
> 내가 만든 AI 에이전트 라이브러리로 Claude Code 같은 CLI를 만들 수 있을까?

10개월 전, `Robota SDK`를 만들어 npm에 등록했다. 멀티 AI 프로바이더, 도구, 플러그인, Team 에이전트까지 갖춘 AI 에이전트 라이브러리였다.

최근 Claude Code를 쓰면서 `AGENTS.md`를 알게 됐고, 내 SDK로 코딩 에이전트 CLI를 만들어 보고 싶었다.
...
Claude Code에서 `AGENTS.md`가 있으면 에이전트가 규칙을 따르면서 작업한다. 확인해 보니 마크다운 형식으로 프로젝트 규칙을 정리한 지시문이었다.
...
| `.agents/rules/`        | 프로젝트 규칙 — `AGENTS.md`에서 참조           |
...
- 시스템 프롬프트(`AGENTS.md` 등)는 압축하지 않고 보존
- 도구 출력이 너무 크면 잘라내기
...
여기까지 파악하고 나니 내 SDK로 만들 수 있겠다는 확신이 들었다. `AGENTS.md`와 실행 루프만 조립했더니 정말 Claude Code처럼 동작했다. 핵심은 기본 도구 6개(`Bash, Read, Write, Edit, Glob, Grep`)였다. 그리고 이 구조는 Anthropic API에 종속하지 않는다 — OpenAI API를 써도 동일하게 돌아갈 구조다.
...
Claude Code를 쓰던 사람이 무리 없이 사용하려면 익숙한 기능을 다 갖춰야 한다. 하나하나 구현하는 건 지루했지만 빠지면 안 되는 기능이었다.
...
- 문제 — 터미널 raw mode에서 한국어 IME가 정상 작동하지 않았다. 글자 탈락, 마지막 글자 누락, 스페이스 입력 시 한글 누락.
...
agent-core에 `IHistoryEntry` 타입을 추가하고 리팩토링해서 모든 정보를 하나의 타임라인에 담았다:

- **append-only, read-only** 규칙 — 수정/삭제 없음. 데이터가 없으면 fallback이 아니라 버그
...
서브에이전트도 구현했다. Claude Code의 스킬 파일 중 프론트매터에 `context: fork`가 있으면 메인 대화와 분리한 별도 세션에서 실행한다. 아직 완벽하지 않다.
