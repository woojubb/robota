...
- `/monitor` -- CLI second-screen browser monitor. Connects to a running CLI session via WebSocket
  ...
  `SessionMonitor` from `@robota-sdk/agent-ui-web/client` (the shared GUI core package).
...
| `SessionMonitor` (`@robota-sdk/agent-ui-web/client`)    | MonitorClient   | `src/app/monitor/MonitorClient.tsx` |
