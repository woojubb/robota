...
Set `ROBOTA_GUI_SIDECAR_CMD` to override the sidecar command **binary** (default `robota`); the `--serve` flag is
always appended, so the shell always launches the headless runtime host.
