...
`dag-runtime-server` is a ComfyUI-compatible Prompt API execution server. It receives workflow prompts in ComfyUI's native JSON format, translates them into DAG definitions, executes them through runtime and worker services, and exposes results via ComfyUI-identical HTTP endpoints and WebSocket events. The server owns no domain types; it is the runtime composition root that wires ports from `dag-core`, controller utilities from `dag-api`, and concrete runtime/worker packages.
...
| Execution composition (`createDagExecutionComposition`)                                                           | `dag-runtime-server`                          |
| DAG run orchestration services                                                                                    | `dag-runtime`                                 |
| Worker loop and DLQ execution services                                                                            | `dag-worker`                                  |
...
Service Layer
  ...
  ├── composition/create-dag-execution-composition.ts ← runtime + worker wiring
...
5. A DAG run is created and started through the `IDagExecutionComposition` port from `dag-api`, implemented by the local composition factory.
...
| `createDagExecutionComposition` | runtime-server composition factory | `src/composition/create-dag-execution-composition.ts` |
...
| `createDagExecutionComposition` | Local composition factory | dag-runtime-server |
...
### Cross-Package API Consumption (from dag-api)

| API surface (from dag-api) | Used By                         | Purpose                     |
| -------------------------- | ------------------------------- | --------------------------- |
...
| `@robota-sdk/dag-api`                  | `PromptApiController`, `RunProgressEventBus`           |
| `@robota-sdk/dag-runtime`              | Run orchestration, query, and cancellation services    |
| `@robota-sdk/dag-worker`               | Worker loop and dead letter queue reinjection services |
