...
export function toAssetReference(
  ...
  return {
    ...
    runtimeAssetId: metadata.runtimeAssetId,
  };
}

export function getAssetContentUri(
...
export function parseTaskRunPayloadSnapshot(
  snapshot: string | undefined,
): TPortPayload | undefined {
...
export async function validateAssetReferences(
  ...
  for (const node of definition.nodes) {
    ...
    if (
      ...
      assetValue === null ||
      Array.isArray(assetValue) ||
      !('referenceType' in assetValue)
    ) {
      errors.push({
    ...
    const hasAssetId =
      'assetId' in assetValue &&
      typeof assetValue.assetId === 'string' &&
      (assetValue.assetId as string).trim().length > 0;
    const hasUri =
      'uri' in assetValue &&
      typeof assetValue.uri === 'string' &&
      (assetValue.uri as string).trim().length > 0;
