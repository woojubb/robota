...
export function registerDefinitionRoutes(
  ...
  router.post('/v1/dag/definitions', async (req: Request, res: Response) => {
    ...
    const assetErrors = await validateAssetReferences(
      req.body.definition as IDagDefinition,
      assetStore,
    );
    if (assetErrors.length > 0) {
      res
        .status(HTTP_BAD_REQUEST)
        .json({ ok: false, status: HTTP_BAD_REQUEST, errors: assetErrors });
      return;
    }
  ...
  router.put(
    ...
    async (req: Request<{ dagId: string }>, res: Response) => {
      if (!req.body || typeof req.body.definition !== 'object' || req.body.definition === null) {
        res.status(HTTP_BAD_REQUEST).json({
          ok: false,
          status: HTTP_BAD_REQUEST,
  ...
  router.get(
    ...
    async (req: Request<{ dagId: string }>, res: Response) => {
      const parsedVersion = parseOptionalPositiveIntegerQuery(
        req.query.version as string | undefined,
      );
      if (!parsedVersion.ok) {
        res
          .status(HTTP_BAD_REQUEST)
          .json({ ok: false, status: HTTP_BAD_REQUEST, errors: [parsedVersion.error] });
        return;
      }
  ...
  router.get('/v1/dag/nodes', async (req: Request, res: Response) => {
    const listed = await designController.listNodeCatalog({
      correlationId: resolveCorrelationId(req, 'dag-node-catalog-list'),
    });
    res.status(listed.status).json(listed);
  });
}
