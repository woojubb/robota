...
class StubPromptApiClient implements IPromptApiClientPort {
  ...
  async getQueue(): Promise<
    TResult<{ queue_running: unknown[]; queue_pending: unknown[] }, IDagError>
  > {
  ...
  async getSystemStats(): Promise<
    TResult<{ system: Record<string, unknown>; devices: unknown[] }, IDagError>
  > {
...
let promptClient: StubPromptApiClient;

function createTestApp(): express.Express {
  const app = express();
  app.use(express.json());

  promptClient = new StubPromptApiClient();
  runService = new OrchestratorRunService(promptClient);
  const assetStore = new StubAssetStore();

  const router = express.Router();
  registerRunRoutes(router, runService, assetStore, 'http://127.0.0.1:8188');
  app.use(router);
...
describe('dag-orchestrator-server endpoint contract tests', () => {
  ...
  describe('POST /v1/dag/runs', () => {
    ...
    it('accepts partialRun and passes partial execution targets to runtime prompt', async () => {
          ...
          edges: [
            {
              from: 'source',
              to: 'output',
              bindings: [{ outputKey: 'output', inputKey: 'input' }],
            },
          ],
        },
        partialRun: { startNodeId: 'source' },
      });
      const { preparationId } = (createResult.body as { data: { preparationId: string } }).data;

      await post(`/v1/dag/runs/${preparationId}/start`);

      expect(promptClient.submittedPrompts[0].partial_execution_targets).toEqual([
        'source',
        'output',
      ]);
    });
  ...
  describe('POST /v1/dag/runs/:id/start', () => {
    it('returns 202 with { ok: true, data: { dagRunId, preparationId } }', async () => {
      ...
      const envelope = body as {
        ok: boolean;
        status: number;
        data: { dagRunId: string; preparationId: string };
      };
      expect(envelope.ok).toBe(true);
