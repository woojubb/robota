...
if [[ "$ANY_BASE_DECLARATION" == "true" ]]; then
  ...
  if [[ "$CUR_BRANCH" == "$TRUSTED_INTEGRATION_BRANCH" && "$TRUSTED_LOCAL_SHA" != "$(hook_git_in "$PROJECT_DIR" rev-parse HEAD)" ]]; then
    ...
    SYNC_EXPECTED_TREE_OUTPUT=$(hook_git_in "$PROJECT_DIR" merge-tree --write-tree "$SYNC_PARENT1" "$SYNC_PARENT2" 2>/dev/null) || SYNC_MERGE_STATUS=$?
    ...
    if [[ "$SYNC_RANGE_COUNT" != "1" \
      ...
      || ! hook_git_in "$PROJECT_DIR" merge-base --is-ancestor "$SYNC_PARENT2" "$SYNC_DEVELOP_SHA" 2>/dev/null; then
      echo "[pre-push-check] Blocked: invalid integration-base sync; require one HEAD merge with trusted remote base first and an origin/develop ancestor second." >&2
      exit 2
    fi
    if [[ "$SYNC_MERGE_STATUS" == "0" && "$SYNC_HEAD_TREE" != "$SYNC_EXPECTED_TREE" ]]; then
      echo "[pre-push-check] Blocked: clean integration-base sync has changes beyond the automatic merge tree." >&2
      exit 2
    fi
    if [[ "$SYNC_MERGE_STATUS" == "1" ]] && ! hook_git_in "$PROJECT_DIR" diff --check "$SYNC_PARENT1" HEAD >/dev/null 2>&1; then
      echo "[pre-push-check] Blocked: conflicted integration-base sync retains conflict markers or whitespace errors." >&2
      exit 2
    fi
    if [[ "$SYNC_MERGE_STATUS" != "0" && "$SYNC_MERGE_STATUS" != "1" ]]; then
      echo "[pre-push-check] Blocked: integration-base merge could not be computed." >&2
      exit 2
    fi
