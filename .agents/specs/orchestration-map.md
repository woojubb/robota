...
expectation/observation schema plus `scan-recommendation-endorsement.mjs`: topic and staged modes prove the
review checkpoint preceded implementation, while persisted mode survives squash by validating the current
decision projection. New-surface placement remains an additive GATE-APPROVAL judgement rather than a
replacement for this universal floor.
