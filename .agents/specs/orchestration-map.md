...
| **Post-merge cycle** (HARNESS-049)              | `post-merge-cycle` (shared; called by `pr-finding-resolution-loop` and `worktree-parallel-orchestration`)      | (deletion + base-reset run in the skill)                      | `merge-verifier` → MERGE VERIFIED                                                                                 | auto → bounded (2 base re-cuts); halt on a FAIL landing verdict or an exhausted bound                        | branch-guard hook (remote delete requires a merged PR); husky pre-commit (lessons-churn block)                     |
