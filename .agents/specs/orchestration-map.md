...
| **Backlog execution** (HARNESS-049, RULE-2326)  | `multi-backlog-initiative` → `backlog-execution-orchestrator` → `user-execution-scenario`                      | `user-execution-scenario-author` (SCENARIO DRAFTED)           | `proposal-reviewer` → REVIEW VERDICT; `finding-depth-triager` → DEPTH; `backlog-gate-guard` → GATE VERDICT                                                                                         | auto → escape: no-progress, plus per-phase caps the skills own (recommendation revision, scenario redesign, defect round); halt-for-user at every cap | recommendation-endorsement (staged/topic/persisted), loop-run-records, user-execution-plan-order, done-evidence, backlog-placement            |
| **Delegated mechanical refactor** (HARNESS-049) | `delegated-refactor-green-gate`                                                                                | `mechanical-refactor-worker`                                  | `pr-review-reviewer` → ACTIONABLE FINDINGS                                                                                                                                                         | auto → escape: no-progress, plus per-step caps the skill owns (re-specification, re-verify, review); halt at every cap                                | scan-review-findings (signal + retained-snapshot repair delta) (§)                                                                            |
...
The `proposal-reviewer` recommendation dispatch is mechanically floored by the canonical loop-run
expectation/observation schema and `scan-recommendation-endorsement.mjs`. Topic and staged modes prove
the planning-only checkpoint precedes implementation; persisted mode validates the current stable
subject-plus-projection key. New-surface placement remains an additive GATE-APPROVAL judgement.

§ `scan-review-findings` reads the reviewer, both local-review orchestrators, the execution-cadence
rule, and the merge verifier. It protects the `ACTIONABLE FINDINGS` signal, the PR merge/verifier
contracts, and same-reviewer repair-delta continuation in both loops. Whether a repair materially
widens scope remains reviewer judgement. The gate at step 3 of `delegated-refactor-green-gate` is not
