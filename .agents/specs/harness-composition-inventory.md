...
| New-Surface Architecture Placement               | invariant | —     | stays; validation points at `architecture-auditor`                   | ⚖     | Items (1)(2) are criteria an auditor applies; (3)(4) are invariants about recording/ordering.    |
...
| Validated-recommendation checks (`spec-workflow.md`)   | **Yes — `proposal-reviewer` + `architecture-auditor`**              | **Reuse.** Reachability and capability-preservation are design-quality judgements the auditor makes; the adversarial pass is the reviewer's charter.                                                                         |
| New-surface placement validation (`spec-workflow.md`)  | **Yes — `architecture-auditor` / `proposal-reviewer`**              | **Reuse.** The rule already names both agents; phase 2 only removes the restated criteria.                                                                                                                                   |
...
| SW-12 | Contract-boundary / wide-blast-radius designs must verify reachability, capability preservation, adversarial pass                     | stays (mandate); criteria → `proposal-reviewer` / `architecture-auditor` |
| SW-13 | Record the verification in Architecture Review before GATE-APPROVAL; otherwise a process violation                                    | stays                                                                    |
| SW-14 | A new surface must mirror an analogous existing layer or justify differing, and state its product family                              | stays (mandate); criteria → `architecture-auditor`                       |
| SW-15 | A new surface reuses shared core/contract layers, never a skin on a sibling product                                                   | stays (mandate); criteria → `architecture-auditor`                       |
