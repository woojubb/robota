...
| Agent wake / schedule host-context bridges | `packages/agent-framework/src/command-api/host-context.ts` (`spawnScheduledWake`, `spawnMonitorWake`) | FLOW-001~006 bridges that `/schedule` and `/monitor` use to register cron/one-shot and process-monitor agent wakes (see [../background-task-layer.md](../background-task-layer.md) § Wake & Scheduling). |
...
| Auth contracts                             | `packages/auth/docs/SPEC.md` — **planned (package not yet created; see ADR-002)**                     | Auth verifier ports and scope policy.                                                                                                                                                                    |
| Credits contracts                          | `packages/credits/docs/SPEC.md` — **planned (package not yet created; see ADR-002)**                  | Credit account, reservation, and settlement contracts.                                                                                                                                                   |
