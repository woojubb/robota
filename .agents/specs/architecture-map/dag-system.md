...
DAG workflow engine: node-graph definition, in-process runtime execution, run lifecycle/persistence, and the CLI / scheduler surfaces (absorbed via WORKFLOW-001, decoupled from the external workflow runtime).
...
| Surfaces       | `dag-cli`, `dag-scheduler`                                         | Product shells: `robota-dag` CLI; scheduled-run triggering. Assemble framework + selected nodes.                                                                                                                                                                                                                                                                                                                                                                                                 |
| Nodes          | `dag-nodes/*` (`@robota-sdk/dag-node-*`)                                             | Node-family packages: llm-text providers, image edit, http, file r/w, router, instant-node, utility-text. Depend on `dag-core`/`dag-node`; several also consume the `agent-*` subsystem one-way — `agent-core`/`agent-provider-*` (LLM-text, image, video, instant-node families), `agent-tools` (tool node), `agent-framework`/`agent-interface-transport` (skill node). This DAG→agent edge is one-directional (no `agent-*` package depends back on a DAG package). Consumed by `dag-framework`/`dag-cli`. |
