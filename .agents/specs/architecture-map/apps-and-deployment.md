...
| `apps/agent-app`      | Electron desktop app (GUI-005)                          | Packaged desktop app (not a hosted service)      | Desktop GUI shell: spawns `robota --serve` → `startRuntimeHost` and drives the session over a loopback WS sidecar; renders the shared GUI core `@robota-sdk/agent-transport-gui`. Deps: `agent-transport-gui` + `agent-transport-ws`; NOT `agent-framework`/`agent-core`. |
...
| `apps/agent-app`         | Electron desktop shell | The desktop GUI: spawns `robota --serve` → `startRuntimeHost` and renders the shared GUI core over a loopback WS sidecar (GUI-005). Depends on `agent-transport-gui` + `agent-transport-ws`; not on `agent-framework`.       |
