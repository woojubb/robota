...
| `apps/agent-app`      | Electron desktop app (GUI-005)                          | Packaged desktop app (not a hosted service)      | Desktop GUI shell: spawns `robota --serve` → `startRuntimeHost` and drives the session over a loopback WS sidecar; renders the shared GUI core `@robota-sdk/agent-ui-web`. Deps: `agent-ui-web` + `agent-transport-ws`; NOT `agent-framework`/`agent-core`. |
...
`apps/agent-web` vs `apps/agent-web-monitor` vs `apps/agent-app` disambiguation:

| Item                     | Kind                   | Role                                                                                                                                                                                                                         |
| ------------------------ | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/agent-web`         | Next.js host app       | Full Playground web application; consumes `agent-playground`; its `/monitor` route mounts `SessionMonitor` from the shared GUI core `@robota-sdk/agent-ui-web`.                                                              |
...
| `apps/agent-app`         | Electron desktop shell | The desktop GUI: spawns `robota --serve` → `startRuntimeHost` and renders the shared GUI core over a loopback WS sidecar (GUI-005). Depends on `agent-ui-web` + `agent-transport-ws`; not on `agent-framework`.              |

The former `packages/agent-web-ui` library was retired in GUI-006: its `SessionMonitor` moved to the shared GUI
core (`@robota-sdk/agent-ui-web`), its browser WebRTC peer to `@robota-sdk/agent-transport-webrtc-web`,
and its CLI-served SPA to `apps/agent-web-monitor`. Do not import `apps/agent-web-monitor` from anywhere; consume the
shared GUI surface from `@robota-sdk/agent-ui-web`.
