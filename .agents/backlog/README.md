...
### Dependency Security Remediation (2026-06-12)
...
L0~L3 제품 검증 캠페인(npm 설치본 beta.73, 실 provider 포함)에서 발견된 결함. 상세 리포트:
`.design/validation/agent-cli-product-verification-2026-06.md`.
...
| [CLI-065](completed/CLI-065-init-yes-ignores-prompts.md)         | `init --yes`가 overwrite/마이그레이션 프롬프트에서 무시됨 — 비대화식 사용 불가 | high     |
...
| [HARNESS-006](completed/HARNESS-006-parsed-args-consumption-check.md)     | CLI 플래그 배선 검사 — 파싱만 되고 소비처 없는 플래그 검출         | high     |
...
| [HARNESS-007](completed/HARNESS-007-tool-description-schema-tests.md)  | 전 빌트인 도구 description-스키마 일치 테스트 (Grep 패턴 확대)   | medium   |
| [HARNESS-009](completed/HARNESS-009-permission-module-coverage.md)     | permission/보안 모듈 최소 커버리지 요구                          | medium   |
| [HARNESS-010](completed/HARNESS-010-event-contract-continuity.md)      | 이벤트 루프 연속성 — 기록되는 이벤트의 발행/표시 경로 보장       | medium   |
...
### Design-Quality Audit (2026-06-14)

설계 품질 심층 감사(정합성 감사와 별개로 "이 설계가 옳은가"를 코드 직접 판단)에서 도출된 17건을
테마별로 묶은 항목. 0 P0 / 8 P1 / 7 P2 / 2 NIT. 보고서:
`.design/architecture-audit/2026-06-14/design-quality-audit.md`.

| ID                                                                          | 제목                                                                               | 우선순위 |
| --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | -------- |
| [DQ-AUDIT-001](completed/DQ-AUDIT-001-rule-violation-cleanup.md) ✅         | 규칙 위반 정리 — core 벤더 디폴트 리터럴(DQ-02) + deprecated 죽은 메서드(DQ-03)    | high     |
| [DQ-AUDIT-002](completed/DQ-AUDIT-002-ssot-consolidation.md) ✅             | SSOT 통합 — 가격(DQ-04)/토큰추정(DQ-09)/컨텍스트 타입(DQ-10)/re-export(DQ-12)      | high     |
| [DQ-AUDIT-003](completed/DQ-AUDIT-003-interface-package-purity.md) ✅       | 인터페이스 패키지 순수성 — tui 타입가드(DQ-01) + interface-transport 의존(DQ-11)   | medium   |
| [DQ-AUDIT-004](completed/DQ-AUDIT-004-responsibility-relocation.md) ✅      | 책임 재배치 — cli 세션분석(DQ-05/06, OBS-001 재검토) + transport LLM 네이밍(DQ-07) | medium   |
| [DQ-AUDIT-005](completed/DQ-AUDIT-005-transport-package-cohesion.md) ✅     | agent-transport 부엌-싱크 배럴/분할(DQ-08)                                         | medium   |
| [DQ-AUDIT-006](completed/DQ-AUDIT-006-error-observability-hygiene.md) ✅    | 에러/관측 위생 — 타입 throw(DQ-13)/훅 실패 관측(DQ-14)/스텁 메트릭(DQ-15)          | medium   |
| [DQ-AUDIT-007](completed/DQ-AUDIT-007-minor-fallback-and-api-surface.md) ✅ | 사소 — OpenAI 무음 기본모델(DQ-16) + IAIProvider 이중 API(DQ-17)                   | low      |

### Session Lessons → Harness/Skill Integration (2026-06-15)

beta.76 릴리즈 세션에서 **실제로 발생한** 마찰·실수를 스킬 신설/업데이트 + 기계적 하네스 검사로
녹여 재발을 방지하는 항목. 각 항목은 한 건의 신규/업데이트 스킬 작업에 대응. 전부 구현 완료 (2026-06-15).

| ID                                                                       | 제목                                                                        | 우선순위 |
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------- | -------- |
| [LESSON-001](completed/LESSON-001-live-seam-cold-state-testing.md) ✅    | 라이브 상태변경 seam의 cold-state 테스트 — mock-the-buggy-collaborator 방지 | high     |
| [LESSON-002](completed/LESSON-002-design-quality-audit-skill.md) ✅      | 설계 품질 심층 감사 스킬 신설 — conformance 감사와 분리                     | high     |
| [LESSON-003](completed/LESSON-003-protected-branch-commit-guard.md) ✅   | branch-guard를 commit 단계까지 확장 — 보호 브랜치 직접 커밋 차단            | high     |
...
유지하면서 범용 에이전트의 발견성을 높이고, 이식 가능한 하네스 패턴을 문서화한다.

| ID                                                    | 제목                                                          | 우선순위 |
| ----------------------------------------------------- | ------------------------------------------------------------- | -------- |
| [HAUDIT-001](HAUDIT-001-agents-arch-tree.md)          | AGENTS.md 최상위 구조 트리 인라인 — ctx-agents-arch hollow    | medium   |
| [HAUDIT-002](HAUDIT-002-agents-pitfalls-pointer.md)   | AGENTS.md Common Pitfalls 진입점 — ctx-agents-pitfalls hollow | medium   |
| [HAUDIT-003](HAUDIT-003-claude-md-delegation-note.md) | CLAUDE.md 위임 의도 명시 — ctx-claude-exists 강화             | low      |
| [HAUDIT-004](HAUDIT-004-memory-discoverability.md)    | 영속 메모리/lessons 발견성 포인터 — auto-memory-system 강화   | low      |
| [HAUDIT-005](HAUDIT-005-portable-harness-patterns.md) | 이식 가능한 하네스 패턴 문서화 — 보편 강화                    | low      |
