...
- [ ] `ICommandSessionRuntime`에 `getFullHistory(): IHistoryEntry[]` 추가
- [ ] `/context list`가 system / conversation history / tool results / references 섹션을 모두 표시
- [ ] tool results 섹션에 toolName + 첫 번째 인수(파일경로 또는 명령어) 표시
- [ ] 각 섹션의 추정 토큰 합계가 표시됨
