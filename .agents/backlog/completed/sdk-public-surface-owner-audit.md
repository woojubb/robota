...
`packages/agent-framework/src/index.ts` intentionally exposes `InteractiveSession`, command contracts,
...
- `packages/agent-framework/src/types.ts`
...
1. Generate an export inventory for `packages/agent-framework/src/index.ts` and nested barrels.
2. Compare each export against `packages/agent-framework/docs/SPEC.md` and owner package specs.
...
- [x] `packages/agent-framework/docs/SPEC.md` describes the resulting public surface ownership.
...
- Documented the SDK public surface ownership classes in `packages/agent-framework/docs/PUBLIC-SURFACE.md`.
