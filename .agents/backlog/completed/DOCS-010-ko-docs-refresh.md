...
- [x] TC-01: ko/getting-started가 현재 영어 getting-started의 API/옵션/모델 id와 일치
- [x] TC-02: ko/getting-started에 비존재 패키지/심볼(`@robota-sdk/anthropic`, `createAgent`) 0건
- [x] TC-03: ko/README의 끊긴 내부 링크 0건(존재 섹션 또는 영어 대응으로 연결)
...
- [x] ko/getting-started 재작성 → ko/README 링크 정정 → grep/harness:scan

## Evidence Log

### 구현 완료 — 2026-06-16

- **ko/getting-started/README.md:** 영어 getting-started에 맞춰 전면 재작성 —
  `new Robota({ aiProviders, defaultModel: {...} })`, `AnthropicProvider`(`/anthropic`),
  `createZodFunctionTool`, `setModel`, `InteractiveSession`(cwd+complete 이벤트), 모델 id `claude-sonnet-4-6`.
  비존재 `@robota-sdk/anthropic`/`openai`·`createAgent`·`providers:` 제거.
- **ko/README.md:** 끊긴 `/ko/{guide,examples,packages}` → 영어 문서로 연결 + 번역 미비 안내.
- **TC-01~04:** ko phantom 식별자 grep 0건, 영어판과 API/옵션/모델 id 일치, `pnpm harness:scan` **26/26**.
