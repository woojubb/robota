...
- [x] TC-01: AGENTS.md "Project Structure"에 최상위 디렉터리 트리가 인라인됨
- [x] TC-02: 트리에 개별 패키지명/도메인 개념이 없음(domain-free 유지)
- [x] TC-03: `.agents/project-structure.md`로의 SSOT 포인터 유지
...
- [x] AGENTS.md 트리 추가 → harness:scan 검증

## Evidence Log

### 구현 완료 — 2026-06-15

- **TC-01/03:** AGENTS.md "Project Structure"에 최상위 디렉터리 트리(7개 generic dir) 인라인,
  `.agents/project-structure.md` SSOT 포인터 유지.
- **TC-02:** 트리는 `packages/ apps/ .agents/ scripts/ docs/ content/ examples/`만 — 개별
  패키지명·도메인 개념 0 (domain-free 유지).
- **TC-04:** `pnpm harness:scan` **26/26 passed**.

감사 효과: `ctx-agents-arch` hollow → pass.
