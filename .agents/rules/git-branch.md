...
- A multi-backlog integration base has the exact name `integration/<agreement-id>` (for example,
  ...
  foreign-merge exception and does not require a duplicate Task/spec pair. If `develop` moves, the only base sync is one clean merge
  at `HEAD` with parent 1 equal to the declared remote integration base and parent 2 equal to current
  `origin/develop`. The authoritative issue/request owns the bounded child set and final
