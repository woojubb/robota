...
- **A pure re-export barrel is not a monolith**, and is exempt — by name, with a reason, in
  `.agents/harness.config.json` → `fileSizeReexportBarrels`. It handles no concerns: it lists names.
  Since `sdk-public-surface` refuses `export *`, every public name costs one line wherever it is
  listed, so a sub-barrel moves lines instead of removing any, and judging the list as debt is what
  makes people regenerate the baseline instead. The exemption is EARNED, never merely declared: it
  applies only while every statement in the file is a re-export, and the moment one grows a function,
  a constant or an import the limit applies again and the entry is reported. Splitting a barrel is a
  change to the package's `exports` map and its consumers — a real decision, not a size remedy.
