...
- **Exercise new orchestration.** Run a new orchestration through a real terminal result when practical,
  and keep its result or error in the execution surface that produced it. Do not add a manually committed
  loop ledger or a proof-only commit. A focused test may reproduce routing semantics when a real external
  run is unavailable.
  Enforced by: the focused orchestration tests and the execution surface; there is no prospective
  loop-ledger proof scan.
