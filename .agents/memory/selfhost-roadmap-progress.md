...
roadmap (`.agents/tasks/SELFHOST-000..014`, derived from commercial-agent prior-art research) is the vehicle.
...
- serialize/deserialize persistence; adapter builds/accepts the index once, PR #1202), P3 (`updateRepoMapIndex`
  ...
  backend, may revise the port) DEFERRED** → `.agents/tasks/SELFHOST-003-P4-embedding-vector-backend.md`. Also filed:
...
durable entries dedup-skip the vector index (keyword-recallable; v2 `upsert-by-id`). **SELFHOST-008 NEUTRAL LIBRARY COMPLETE**; **P5 (concrete backend) DEFERRED to backlog** `.agents/tasks/SELFHOST-008-P5-concrete-semantic-backend.md` (surface-side; mirrors SELFHOST-003-P4). Also this session:
...
the port; run architecture-audit-fanout + architecture-conformance-auditor at mid-points (they caught the P1 defects early).
...
**SELFHOST-011 (evals-as-code, EPIC, `type: BEHAVIOR`) — P1 DONE** (PR #1232 merged+verified on develop `c1e856da3`). Neutral `agent-framework/src/evals/` SDK seam (mirror pure `src/self-hosting`/`src/goal` + the `agent-session-analytics` metrics-over-a-run sibling): `eval-types.ts` (`IMetric` = pure fn over the SSOT `IExecutionResult` — response+toolSummaries+usage+history, NOT the string; Alt-4 string-only REJECTED), `IEvalCase`/`IEvalDefinition` (cases×metrics×threshold), report types, `TEvalRunFn`; `runner.ts` `defineEval`+`runEval(def,runFn)` (injected runFn, boolean→1/0, numeric clamped to [0,1], overallScore=mean of case×metric, passed=overallScore>=threshold; IO-free); `session-run-fn.ts` `createSessionRunFn(runtime)` (default runFn = `createAgentRuntime().createSession()` capturing the terminal `complete`-event FULL IExecutionResult, fresh session per case, `await shutdown()` in finally; NOT `createQuery`). Barrel + `// ── Evals ──` root export + SPEC.md. Review (pr-review-reviewer #1232) found 2 SHOULD — **session leak** (no shutdown → fixed, finally) + **numeric false-pass** (unclamped >1 → clamp [0,1]) — both fixed `bf5692cb0`. **P2 DONE** (PR #1233 merged+verified develop `6366850c7`): `robota eval <definition>` CLI gate — `agent-cli/src/eval/eval-command.ts` `runEvalCommand(argv,cwd,deps?):Promise<number>` (dynamic-import consumer def → default runFn from CLI-resolved provider via createSessionRunFn → runEval → exit 0 pass/1 breach; injectable runFn/loadDefinition for tests); cli.ts pre-parse intercept (mirror `session analyze`) + fallthrough + help; `examples/capabilities/agent-eval/` (TC-04); TC-05 neutrality + mechanical floor filed **HARNESS-034**. TC-03 exit test (eval-command.test.ts, 7 cases). **AGENT-RUN VERIFIED** (owner directive): drove real `robota eval` vs live Anthropic — PASS def (model answered 4)→exit 0, FAIL def→exit 1, missing-path/malformed-threshold→1; evidence scenario. Review #1233: 1 SHOULD (TC-03 test misattribution → corrected) + 2 CONSIDER (runFn-in-try for ProviderConfigError; reject malformed --threshold) fixed `0c6605e4e`. **EPIC DONE** — GATE-VERIFY+GATE-COMPLETE PASS, spec in `spec-docs/done/`, P1+P2 tasks archived, all TC-01..06. **P3/P4 DEFERRED to backlog** `.agents/tasks/SELFHOST-011-P3-P4-evals-followups.md`.

**SELFHOST-012 (scheduled-task lifecycle, EPIC, `type: FLOW`) — P1 DONE** (PR #1235 merged+verified develop `181f0e89a`). Non-destructive pause/resume/edit over the EXISTING croner scheduler (NO new scheduler): `'paused'` added to `TBackgroundTaskStatus` (SSOT, non-terminal); state machine PAUSE (running/sleeping→paused) + RESUME (paused→sleeping) + paused→cancelled; scheduled-task-runner handle `pause()/resume()/editSchedule(patch)` wiring croner `.pause()/.resume()` (NEVER the irreversible `.stop()`) + a `state.paused` guard on all fire paths (closure + runOneFire + in-flight child close, closing the illegal paused→sleeping trap); edit rebuilds the Cron in place (same taskId); manager `pauseScheduledTask/resumeScheduledTask/editScheduledTask` + `markBackgroundTaskPaused/Resumed` (paused releases slot like sleeping; idempotent sleeping-event handler); `IAgentJobHostContext.listSchedules/pauseSchedule/resumeSchedule/editSchedule` wired in interactive-session-base. TC-01..04. Review #1235 **0 MUST/SHOULD**; applied 2 CONSIDERs (`d81439412`): clearer queued-schedule error + guard regression tests. agent-executor 87 tests, 57/57 scans. **P2 (`/schedule list|pause|resume|edit` surface mirror `/background` + AGENT-RUN verification) / P3 (paused survives restart via FLOW-003 re-arm predicates) PENDING.** SEQUENCING CONSTRAINT (review): **P3 must land before/with P2** — a restored paused task is else reconciled to `failed`, so exposing pause via P2 without P3 lets a restart silently kill a paused schedule.

**SELFHOST-012 P2+P3 DONE — EPIC COMPLETE** (PR #1237 merged+verified develop `4223a7d95`). **P2**: `/schedule list|pause|resume|edit` surface (first-token dispatch mirror `/background`; create begins with in/cron → no collision). **P3**: paused survives restart — shared predicate `isReArmableScheduledTask` (`interactive/schedule-rearm.ts`) widened to `sleeping||paused`; tracker re-arm-then-pauses a restored paused task (failure → system note). TC-05 (dispatch) + TC-06 (restart re-arm-as-paused). **AGENT-RUN VERIFIED** on the REAL runtime: every-second cron paused fired NOTHING across 2.5s, resumed fired, edited re-armed; scenario `.agents/evals/scenarios/selfhost-012-schedule-lifecycle-agent-run.md`. Review #1237 **0 MUST/SHOULD**; 4 CONSIDERs applied (`e574e7eba`). GATE-COMPLETE, spec `done/`, all TC-01..06. No new scheduler. agent-command 242 / agent-framework 1213 tests.

**SELFHOST-013 (multi-surface deployment, EPIC, INFRA) — DONE** (PR #1239 merged+verified develop `b56617537`). Packaging+docs+proof over the EXISTING transport DIP (no new transport/package/edge). Matrix `.agents/specs/deployment-matrix.md` + code-enumeration drift scan `scan-deployment-matrix.mjs` (transport names `{tui,ws,webrtc,http,mcp}` ↔ matrix; header-located column; TC-02) + reference-identity proof `one-session-many-transports.test.ts` (t1.attached===t2.attached===session; TC-01) + deploy guide `content/guide/deployment.md` + runnable example `examples/capabilities/multi-surface-deploy/`. AGENT-RUN VERIFIED (TC-04): ran example, one session over ws+http, exit 0. Review 1 SHOULD (local provider pkg) + fixes. GATE-COMPLETE, spec `done/`.
