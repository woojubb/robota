...
(`packages/agent-provider-anthropic/src/anthropic/provider.ts:126`,
`packages/agent-provider-anthropic/src/anthropic/provider.ts:198-199`), 하지만 동일한 경로에 `effort`를 넣는
코드는 존재하지 않는다.
...
1. `rg -n "effort" packages/agent-provider/src packages/agent-framework/src` — per-call 세션 옵션에서
   ...
   (`packages/agent-provider-openai/src/openai/types.ts:15`)와 deepseek의 정적 생성자 옵션
   `reasoningEffort`(`packages/agent-provider-openai-compatible/src/deepseek/provider.ts:242`)뿐이며, 둘 다 resolve된
...
- **`packages/agent-provider`** — provider 요청 빌더에서 effort를 해당 provider의 native 요청 파라미터로
  ...
  경로(`packages/agent-provider-anthropic/src/anthropic/provider.ts:126`, `:198-199`).
...
- `packages/agent-provider-anthropic/src/anthropic/provider.ts` (effort → native 파라미터 매핑 또는 문서화된 no-op)
- `packages/agent-provider-openai/src/openai/*` (effort → native 매핑; 기존 `effort?` 타입 재사용)
- `packages/agent-provider-openai-compatible/src/deepseek/provider.ts` · `packages/agent-provider-openai-compatible/src/qwen/provider.ts` (effort → native 매핑/no-op)
