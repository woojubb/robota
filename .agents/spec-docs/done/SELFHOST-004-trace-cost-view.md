...
- [x] TC-01: the extended `summarizeUsageBySource` produces per-source **cost** totals AND the span timeline (from the per-operation span entries on `IInteractiveSessionRecord.history`), **grouping sub-turn spans under their owning turn** — **positionally**: the interactive turn appends its spans in emit order immediately before the turn's `usage-summary` entry (the boundary the reducer flushes on), so the reducer buffers spans and assigns them to the turn whose `usage-summary` closes them. This is equivalent to (and simpler than) carrying an `ownerPath`/owner correlation on each entry because Robota `run()` serializes turns and the per-turn collector captures only that turn's spans; `ISpanEntry` intentionally carries no owner field. Unit test on the reducer asserts the grouping (not just span presence); it stays within its `agent-core` + `agent-interface-transport` deps (NO `agent-plugin` read).
