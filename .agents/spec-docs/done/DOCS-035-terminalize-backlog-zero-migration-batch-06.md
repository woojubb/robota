...
- [x] TC-01: the committed manifest contains exactly three units, five governed source blobs, nine
      final tracked paths, exact mixed dispositions, and zero baseline/carrier changes.
- [x] TC-02: control/residual issues and the CONFIG-002 canonical handoff are read back exactly;
      CONFIG-002 cites that exact comment while both delivered Tasks need no returned issue.
- [x] TC-03: DOCS-028 and HARNESS-122 Tasks become done with byte-identical bodies, CONFIG-002 becomes
      skipped with a byte-identical body, and neither bypassed/stale plan is promoted to done.
- [x] TC-04: both paired plans become rejected with dated truthful evidence; the DOCS-028 paired path
      rekeys to its completed Task, and no historical gate verdict is manufactured.
- [x] TC-05: the exact final changed-path set is the nine approved lifecycle/ledger paths, no excluded
...
- [x] `.agents/tasks/completed/DOCS-035-terminalize-backlog-zero-migration-batch-06.md`
...
- GATE-IMPLEMENT — `.agents/tasks/<ID>.md` has been created: `## Tasks` names `.agents/tasks/DOCS-035-terminalize-backlog-zero-migration-batch-06.md`, which exists
- GATE-IMPLEMENT — Tasks file path is recorded in the `## Tasks` section of the spec document: `## Tasks` names `.agents/tasks/DOCS-035-terminalize-backlog-zero-migration-batch-06.md`, whose basename is the spec's
...
- Planning checkpoint binding: `.agents/tasks/DOCS-035-terminalize-backlog-zero-migration-batch-06.md`; `.agents/spec-docs/active/DOCS-035-terminalize-backlog-zero-migration-batch-06.md`; `SCENARIO DRAFTED: not-applicable | 0`; subject-bound scenario run `r20260828200720`; orchestrator run `r20260828200716`; whole-worktree inventory limited to the exact Task/spec pair and two loop ledgers.
...
- GATE-VERIFY — All tasks in `.agents/tasks/<ID>.md` are marked complete (`[x]`): 5/5 tasks `[x]` in .agents/tasks/DOCS-035-terminalize-backlog-zero-migration-batch-06.md
- GATE-VERIFY — No tasks are blocked or pending: no unticked, blocked, or pending task
- GATE-VERIFY — Build passes for all affected packages (`pnpm build`): build-shaped `pnpm harness:scan:reference-kind-qualified` → exit 0 ( ⏎ ::examined:: 3134 tracked document(s) ⏎ reference-kind-qualified scan passed (1465 unqualified reference(s) at baseline across 275 file(s)). It checks that a reference says WHICH it is, not that the kind it names is correct — deciding that needs a live GitHub read.); all 2 supplied commands exit 0
- GATE-VERIFY — Tests pass for all affected packages (`pnpm test`): test-shaped `pnpm exec vitest run scripts/harness/__tests__/check-task-archival.test.mjs scripts/harness/__tests__/scan-task-path-citations.test.mjs scripts/harness/__tests__/task-lifecycle.test.mjs scripts/harness/__tests__/scan-doc-folder-status-agreement.test.mjs scripts/harness/__tests__/scan-standing-delegation-evidence.test.mjs` → exit 0 ( Duration 460ms (transform 221ms, setup 0ms, collect 371ms, tests 241ms, environment 1ms, prepare 245ms) ⏎ ⏎ 5:11:38 AM [vite] warning: `esbuild` option was specified by "vitest" plugin. This option is deprecated, please use `oxc` instead.); all 2 supplied commands exit 0

### [GATE-COMPLETE: TC-01] — ✅ PASS | 2026-08-29

**Test skipped:** Independent preservation audit measured exactly three manifest units, five governed blobs, nine approved paths, and zero baseline/carrier changes; documentation migration has no executable TC-01 behavior.

### [GATE-COMPLETE: TC-02] — ✅ PASS | 2026-08-29

**Test skipped:** Exact GitHub control/residual issue and canonical comment readback is append-only control-plane evidence; no executable TC-02 behavior exists.

### [GATE-COMPLETE: TC-03] — ✅ PASS | 2026-08-29

**Test skipped:** Normalized Git comparison proves all three Task bodies remain byte-identical while only approved lifecycle frontmatter and paths change; no executable TC-03 behavior exists.

### [GATE-COMPLETE: TC-04] — ✅ PASS | 2026-08-29

**Test skipped:** Folder, status, citation, and evidence comparison proves both truthful planning-document rejections without manufactured gates; no executable TC-04 behavior exists.

### [GATE-COMPLETE: TC-05] — ✅ PASS | 2026-08-29

**Test skipped:** Focused scanners and lifecycle tests passed; full harness scan and CI mirror are reserved for atomic final placement, so there is no separate executable scenario.
...
- GATE-COMPLETE — The spec's `## Tasks` section names the exact active task path under `.agents/tasks/`: `## Tasks` names `.agents/tasks/DOCS-035-terminalize-backlog-zero-migration-batch-06.md`, which exists
- GATE-COMPLETE — That active task exists and is completion-ready: all tasks are `[x]`, with no pending or blocked item: 5/5 tasks `[x]` in .agents/tasks/DOCS-035-terminalize-backlog-zero-migration-batch-06.md
