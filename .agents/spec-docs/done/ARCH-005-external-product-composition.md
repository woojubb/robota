...
| ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| External-preset registration + preset-resolve glue (`loadExternalPresets` result → per-call resolver)                          | **In-kernel** — for external consumers AND for `robota` (ARCH-008); the module-global registry survives only as the in-session `/preset` DISCOVERY surface | pure fold over preset DATA (settings/file read happens in shell, result passed in). **Met by ARCH-008** (superseding ARCH-007's B2 option iii): the shell calls `resolveShellPreset`, which builds agent-preset's per-call instance registry (R8) and resolves over it, then hands `{registry, presetId, context}` into the profile — `assembleProduct` ADOPTS that registry and replays that context, so `product.presets` IS the shell's registry and `product.defaultPreset` IS the shell's resolution. The ordering constraint ARCH-007 cited (the preset is needed before the base command modules are built) is satisfied by handing the registry in, not by a module global. What remains global is DISCOVERY, not resolution: `/preset` lives in `agent-command`, is executed inside the session, and has no handle on the assembled product, so it still reads `listPresets`/`getPreset`/`resolvePreset`. Both surfaces are fed by the one `loadExternalPresets()` call, so they cannot disagree (anti-split test). Retiring the global entirely needs the registry threaded through the command-host context — tracked as ARCH-009 |
