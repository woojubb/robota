...
### [REMOTE REVIEW ROUND 1] — 🔁 RESOLVED | 2026-08-26

- Exact reviewed range: PR #2374 automation reviewed base `8139f845838495a4c191ed3a3af68de342b470d0` and head `a7e56fd4b5a75d7c7ea0614684cbba5f69a405dd`; Claude returned `ACTIONABLE FINDINGS: 0` for that exact range.
- CodeQL disposition: CodeQL alert 484 (`js/incomplete-multi-character-sanitization`) was classified invalid because the replacement result is only a boolean-validation intermediate, any reintroduced `<!--` remains non-whitespace and fails closed, and no replacement output reaches a renderer or other sink. The inline finding received a reasoned reply, its review thread was resolved, and the PR carries `review-findings-acknowledged`; no code change was made for the invalid finding.
- Named red-check ground: the `scans` job failed on GitHub's synthetic PR merge commit because topic replay included both the topic commits and the merge wrapper, making the already recorded endorsement checkpoint appear to be added a second time relative to the wrapper's first parent.
- Resolution: topic replay now detects the exact two-parent synthetic shape whose first parent is the requested base, replays only through the second-parent topic head, and continues to validate persisted state from the merged `HEAD` tree. A deliberate-red temporary-repository fixture reproduces the independent base advance plus no-ff PR merge and proves the wrapper is not replayed as another checkpoint.
- Focused verification: the seven affected harness test files passed 231 tests in 7 files, including the new synthetic-merge regression fixture.
