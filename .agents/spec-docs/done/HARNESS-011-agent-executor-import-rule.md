...
- Backlog closure (done gate): `.agents/tasks/completed/HARNESS-011-ci-green-baseline.md` — `status: done` (line 3), `## Progress update (2026-06-13) — CLOSED` present; User Execution Test Scenarios section states "Not applicable — CI/infra change; evidence is green pipeline runs on the PR" — done-gate satisfied without user execution scenarios per the backlog itself.
