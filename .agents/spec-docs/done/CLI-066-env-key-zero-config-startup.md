...
- Command: same fresh `env-default-provider.test.ts` run (10/10, exit 0).
- Output: `readProviderSettings env-default integration (CLI-066) > TC-03: settings profile wins over the env key` passes (regression held). Corroborated by user-execution evidence in `.agents/tasks/completed/CLI-066-env-key-only-startup-rejected.md` (status: done): profile + env key → profile wins with zero notice lines.
...
- User-execution evidence on real binary recorded in `.agents/tasks/completed/CLI-066-env-key-only-startup-rejected.md` (status: done) — done-gate rule satisfied.
