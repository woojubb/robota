...
- [ ] TC-01: `timeout: 999999999` 전달 시 실제 실행 타임아웃이 600,000ms로 캡됨
- [ ] TC-02: `timeout: 5000` 전달 시 실제 타임아웃이 5,000ms (캡 미만은 변경 없음)
- [ ] TC-03: `timeout` 미전달 시 DEFAULT_TIMEOUT 값이 적용됨

## Test Plan

| TC-ID | Test Type | Tool / Approach                        | Notes                                            |
| ----- | --------- | -------------------------------------- | ------------------------------------------------ |
| TC-01 | unit      | vitest — timeout clamping assertion    | Pass 999999999, verify effectiveTimeout = 600000 |
| TC-02 | unit      | vitest — below-cap timeout passthrough | Pass 5000, verify effectiveTimeout = 5000        |
| TC-03 | unit      | vitest — default timeout applied       | Pass undefined, verify DEFAULT_TIMEOUT used      |

## Tasks

- [ ] `.agents/tasks/CLI-036.md` — 미생성 (GATE-APPROVAL 통과 후 생성)
