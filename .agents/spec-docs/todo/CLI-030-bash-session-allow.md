...
- [ ] TC-01: `pnpm test` 를 `s`(session allow)로 허용한 후 동일 세션에서 재실행 → 프롬프트 없이 실행됨
- [ ] TC-02: 세션 종료 후 재시작 → 동일 명령이 다시 프롬프트를 표시함
- [ ] TC-03: permission 프롬프트에 `[s] Allow for this session` 옵션이 표시됨
- [ ] TC-04: `[a] Allow once`는 기존과 동일하게 동작 (세션 목록에 추가 안 됨)
...
- [ ] `.agents/tasks/CLI-030.md` — 미생성 (GATE-APPROVAL 통과 후 생성)
