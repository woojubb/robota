...
- [ ] TC-01: 동시 stat 호출이 최대 100개로 제한됨 (p-limit 동작 검증)
- [ ] TC-02: 기존 glob 도구 테스트 모두 통과 (mtime 정렬 포함)
- [ ] TC-03: 결과 파일 목록이 mtime 기준으로 올바르게 정렬됨

## Test Plan

| TC-ID | Test Type | Tool / Approach                   | Notes                                            |
| ----- | --------- | --------------------------------- | ------------------------------------------------ |
| TC-01 | unit      | vitest — p-limit concurrency mock | Spy on limit fn, verify max 100 concurrent stat  |
| TC-02 | unit      | vitest — existing glob tests pass | Run existing test suite, verify all pass         |
| TC-03 | unit      | vitest — mtime sort assertion     | Multiple files with different mtime, check order |

## Tasks

- [ ] `.agents/tasks/CLI-043.md` — 미생성 (GATE-APPROVAL 통과 후 생성)
