...
- [ ] TC-08: **NOT met, and cannot be ticked — investigated 2026-08-18.** ARCH-021 (`done` 2026-08-16) delivered the composition half inside its declared area: the child worker composes from the product's own packs rather than reconstructing Robota defaults, proved by a bintest asserting `pack-coding`'s real tool surface reaches the child and that dropping a pack drops its tools. The rest is tracked, not lost — **ARCH-033** (#1784) projecting live owner-bound capability across the process boundary, **ARCH-034** (#1785) in-process vs child-process surface divergence, **ARCH-035** (#1787) no defaults-aggregator leaf so the TOOL axis cannot be cut by a manifest edge, **ARCH-036** (#1788) `deps.builtInAgents` dropped by the child path, and **SEC-009** (#1786) `apiKey` in the IPC start payload. ARCH-036 reached `done` on 2026-08-18; the other four are OPEN (ARCH-033/034/035 `todo`, SEC-009 `in-progress`).

  The re-scope treatment TC-13 received does not transfer here, and the harness said so before a human did. Ticking this criterion was attempted and `scan-resolving-claims` rejected it as `ticked-but-unfinished`: TC-13 closes onto ARCH-031, which is **done**, whereas this would close onto five items of which only ARCH-036 has since finished. A criterion may be re-scoped onto work that finished; it may not be re-scoped onto work that has not started. **SEC-009 carries this criterion's own "keeps credentials parent-side" clause**, so ticking it would have claimed a security property the repository does not have.

  This criterion is met when SEC-009 closes and ARCH-033/034/035 are judged, or when the owner re-scopes it explicitly — the decision TC-13 records the owner making on 2026-08-16.
...
- [x] TC-15: all fourteen child Task records contain current engineering and scenario evidence, reach `done`, and move atomically to `.agents/tasks/completed/` with the agreement projections updated. **Verified 2026-08-18:** all fourteen `completed/` paths resolve; `node scripts/harness/check-done-evidence.mjs` passed (14 superseded references, exit 0); `node scripts/harness/check-task-archival.mjs` passed (123 active, 838 archived, exit 0).
- [ ] TC-16: the assembled integration base passes `pnpm harness:conformance`, affected scoped harness verification, and `pnpm harness:verify-like-ci`, with exact commands and exit evidence recorded. **Run 2026-08-18:**

  | Command                                        | Result                                        |
  | ---------------------------------------------- | --------------------------------------------- |
  | `pnpm harness:conformance`                     | `"conformant": true`, exit **0**              |
  | `node scripts/harness/check-done-evidence.mjs` | passed, exit **0**                            |
  | `node scripts/harness/check-task-archival.mjs` | passed (123 active, 838 archived), exit **0** |
  | `pnpm harness:verify-like-ci`                  | **exit 1** — 3 of 12 stages failed            |

  The three failures were `harness-self-test`, `scan-suite-dist-free` and `scan-suite`, and all three
  had ONE cause: `scan-resolving-claims` rejecting the attempt to tick TC-08 above. With TC-08
  restored to its true state the scan passes. This criterion stays unticked because it must be re-run
  and recorded green on the base that actually closes the initiative — a run that failed and was then
  argued away is the shape this repository's `regression-red-proof` floor exists to reject.
...
Active initiative Task: `.agents/tasks/completed/AGREEMENT-002-complete-august-13-agent-architecture-findings.md`.
...
- Tasks file: `.agents/tasks/completed/AGREEMENT-002-complete-august-13-agent-architecture-findings.md` exists and is
