...
| `packages/agent-framework/src/interactive/interactive-session-execution.ts`            | build `IHistoryEntry<ISpanEntry>` from the `agent-core` span-completion event (mirror `createUsageSummaryEntry`, `:117`) + append to the session record; `extractTurnUsage` resolves the turn's model id + computes turn-granular `costUsd` via `calculateModelCost`, flips `costStatus` from `'unknown'` (owns ONLY `costUsd` — no span-timestamp stamping; NO `agent-plugin` edge)                                                                                                                                                                                                                        |
...
- 2026-07-17 — **iteration 7: RE-REVIEW → REVISE, applied.** Direction endorsed-in-principle; two code-verified
  ...
  **surfaces raw timing** (a span-completion event); the `IHistoryEntry<ISpanEntry>` is built in **`agent-framework`**,
  ...
  today) — v1 timing source = the function-tool + event-service join. Iteration-8 re-review pending.
