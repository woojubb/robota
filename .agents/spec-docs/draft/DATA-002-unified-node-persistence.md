...
- **Phase 3 — SHIPPED (2026-07-05).** Command call sites cut over: `dag node scaffold` now writes a `.dag/nodes/<name>.node.json` manifest + `<name>.dag.node.js` companion (execute-only); `save`/`validate` use the local-aware registry (`createCliNodeRegistryWithLocalNodes`); `run`/`catalog run` resolve `projectDir` by walking up to the nearest `.dag/` (so a workflow in `.dag/workflows/` finds code nodes in `.dag/nodes/`). Tests: scaffold two-file layout (LOCALNODE-005 rewritten) + P3 walk-up run. Live UE end-to-end: `scaffold` → implement companion → `save` (local-aware) → `validate` (valid) → `catalog run` → success. 1005 tests · 45 scans · lint 0-errors · build (Node 24) green. **DATA-002 complete.**
