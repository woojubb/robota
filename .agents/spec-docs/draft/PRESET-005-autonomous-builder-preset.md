...
기능이 무의미하다. 사용자가 요청한 첫 프리셋은 "Fable 5 스타일"이다. 그러나 리서치 결과 Anthropic은
Fable 5의 시스템 프롬프트나 성격 명세를 공개한 적이 없으므로(설계 제안서 §2.1) "프롬프트 복제"는
...
2. **문서화된 Fable 5 작업 스타일 + Claude's Character 원칙으로 페르소나를 구성, generic 식별자 사용.**
...
식별자는 generic(`autonomous-builder`), description에 "Fable 5 작업 스타일에서 영감" 출처 각주 허용
...
- [ ] TC-04: `rg -i "fable|hermes|claude" packages/agent-preset/src/presets/autonomous-builder.ts` 결과에서 `id`/식별자 라인에는 벤더 토큰이 없음(있다면 description 각주 한정) — 식별자 generic 단언
