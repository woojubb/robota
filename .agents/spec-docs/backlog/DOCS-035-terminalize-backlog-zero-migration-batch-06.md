...
| CONFIG-002  | Task `.agents/tasks/CONFIG-002-config-loader-silently-drops-corrupt-settings-and-writers-produce-a-shape-it-rejects.md` @ `1e890999cc72eab64bd9f5fe9398c6be16aa40dd`                                                                                                                                                                       | CLOSED/COMPLETED issue #2023 delivered the loader axis via PR #2285 merge `91f11f7c093f7a8f66077fa0927fc68f872c145a`; no exact OPEN residual owner exists at manifest freeze.                                      | Create an exact OPEN issue for the residual public writer/loader contradiction, append/read back a canonical current-truth handoff, preserve the Task body, and archive it skipped to that exact comment without claiming delivery. |
...
Planned control marker: `backlog-zero:DOCS-035:2c875dd3`. Planned CONFIG-002 residual marker:
`backlog-zero:CONFIG-002-RESIDUAL:2c875dd3`. Both must be unique, OPEN, and unassigned before
approval. Parent issue #2404 remains open for later batches and prevention.
