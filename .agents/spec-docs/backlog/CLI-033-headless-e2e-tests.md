...
- [ ] TC-01: `-p "What is 2+2"` 실행 → 숫자가 포함된 응답 출력
- [ ] TC-02: `-p "Respond" --system-prompt "Always respond in Korean"` 실행 → 한국어 응답 출력
- [ ] TC-03: 성공 시 exit code 0, 오류 시 exit code 1
- [ ] TC-04: `pnpm test:e2e`로 독립 실행 가능하고 CI 파이프라인에서 통과

## Test Plan

| TC-ID | Test Type | Tool / Approach                      | Notes                                          |
| ----- | --------- | ------------------------------------ | ---------------------------------------------- |
| TC-01 | e2e       | vitest + child_process.spawn         | Mock provider returns numeric answer           |
| TC-02 | e2e       | vitest + child_process.spawn         | Mock provider reflects system-prompt directive |
| TC-03 | e2e       | vitest — process exit code assertion | Check exit code from spawn result              |
| TC-04 | e2e       | pnpm test:e2e script execution       | Verify script exists and runs without error    |

## Tasks

- [ ] `.agents/tasks/CLI-033.md` — 미생성 (GATE-APPROVAL 통과 후 생성)
