...
- [ ] TC-01: `BRAVE_API_KEY` 없이 WebSearch 호출 → "API 키가 설정되지 않았습니다" 형태의 안내 메시지 반환 (에러 크래시 없음)
- [ ] TC-02: README 환경변수 표에 `BRAVE_API_KEY` 항목이 추가됨
- [ ] TC-03: README WebSearch 도구 설명에 `BRAVE_API_KEY` 의존성이 명시됨
