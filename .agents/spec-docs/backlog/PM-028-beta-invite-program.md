...
- [ ] TC-01: robota.io/beta 페이지가 로드됨
- [ ] TC-02: 신청 폼 제출 → 확인 메시지 표시
- [ ] TC-03: 신청 폼 제출 시 이름/이메일/use case 데이터가 저장 매체(스프레드시트 또는 DB)에 기록됨
- [ ] TC-04: 수집된 피드백 항목을 GitHub Issues로 변환하는 프로세스가 문서화됨

## Test Plan

| TC-ID | Test Type | Tool / Approach              | Notes                                                                             |
| ----- | --------- | ---------------------------- | --------------------------------------------------------------------------------- |
| TC-01 | e2e       | playwright — page load check | Navigate to /beta, verify page renders                                            |
| TC-02 | e2e       | playwright — form submission | Submit form, verify success message shown                                         |
| TC-03 | manual    | spreadsheet/DB entry check   | Data storage requires human verification of backend storage — no automated test   |
| TC-04 | manual    | GitHub Issues process check  | Process documentation review requires human judgment — no automated test possible |

## Tasks

- [ ] `.agents/tasks/PM-028.md` — 미생성 (GATE-APPROVAL 통과 후 생성)
