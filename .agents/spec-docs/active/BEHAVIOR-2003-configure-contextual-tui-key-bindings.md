...
- [ ] TC-01: The versioned sparse JSON parser accepts documented aliases, uppercase rules, chords,
      per-action `null` unbinding and context reuse, and emits one immutable effective map.
- [ ] TC-02: Parse/schema/unknown/reserved/duplicate/prefix conflicts reject the whole replacement with
      an exact path diagnostic; multiplexer and undeliverable-modifier warnings preserve valid bindings.
- [ ] TC-03: Text contexts never buffer unmodified printable characters as chord prefixes, and chord
      state resets on timeout, mismatch, Escape, context change and snapshot replacement.
- [ ] TC-04: Every current production input context resolves semantic actions through the registry while
      printable text, paste, IME composition and the two-stage Ctrl+C shutdown contract are preserved.
- [ ] TC-05: Every active footer derives keys from the current context's effective actions and omits
      unbound or inactive actions.
- [ ] TC-06: A parent-directory watcher accepts atomic replacement, hot-reloads without restart, retains
      the last valid map on visible invalid replacement, and disposes on all render exit paths.
- [ ] TC-07: `/keybindings` is registered only with the injected capability, atomically creates a schema-
      linked sparse document when absent, never overwrites an existing file and opens the exact path.
- [ ] TC-08: The JSON Schema and documentation enumerate current contexts/actions/defaults, grammar,
      reserved keys, terminal/modal-editor limits and multiplexer warnings.
- [ ] TC-09: The real scripted-provider CLI in a PTY demonstrates context reuse, remapped submit, live
      atomic reload, derived hint refresh and last-valid preservation after an invalid replacement.
- [ ] TC-10: Focused unit/integration/PTY suites, affected package builds, typecheck and harness scans pass.
...
- [ ] `.agents/tasks/BEHAVIOR-2003-configure-contextual-tui-key-bindings.md` — existing paired Task
