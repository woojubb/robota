...
- [x] **TC-01** The issue #1852 case asserts on the set of `Timeout` resources the host armed (tracked by
      identity from `startRuntimeHost` through `shutdown()`), not on a process-wide count, and passes
      on the committed tree.
- [x] **TC-02** Under the mutant that deletes `if (bound !== undefined) clearTimeout(bound);` in
...
- [x] **TC-03** The file passes under a **decoy home** — a temp directory whose
...
- [x] **TC-04** The control case: `createDefaultUserSettingsSources()` and `homedir()` both resolve
      under the isolated home during the test and under the saved home after restore.
- [x] **TC-05** `pnpm --filter @robota-sdk/agent-framework exec vitest run` passes for the package
...
| ----- | ----------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TC-01 | Integration | vitest, `async_hooks` identity tracking around `startRuntimeHost` → `shutdown()`, one `setImmediate` before reading    |                                                                          | `packages/agent-framework/src/runtime/__tests__/runtime-host.test.ts` > `startRuntimeHost (RUNTIME-001 TC-01)` > `shutdown() leaves no timer holding the event loop open (#1852)` |
| TC-02 | Mutation    | delete the `clearTimeout(bound)` line in `runtime-host.ts`, run the file, restore, record                              | hand-run; `git diff --stat` empty after restore                          | Test skipped as a committed case — a mutant cannot be committed; the run is recorded in the fix commit body and the GATE-VERIFY entry                                             |
| TC-03 | Integration | the file under `HOME=<decoy with a planted SessionStart hook>` and under `HOME=$(mktemp -d)/home`, exit codes recorded | the decoy is prepared by a recorded command; reproducible on any machine | Test skipped as a committed case — an environment arm, run by hand and recorded in the fix commit body and the GATE-VERIFY entry                                                  |
| TC-04 | Integration | vitest control case calling `createDefaultUserSettingsSources()` and `homedir()` inside and after the isolation        |                                                                          | same file > `reads settings and plugins from the isolated home, not the developer's (TEST-012 control)`                                                                           |
| TC-05 | Integration | `pnpm --filter @robota-sdk/agent-framework exec vitest run` and `pnpm harness:scan`, exit codes recorded               |                                                                          | package suite plus `harness:scan`; counts and exit codes in the GATE-VERIFY entry                                                                                                 |
...
Every command below was run by this gate against the committed tree (branch `fix/2383-runtime-host-test-measures-the-host`, HEAD `a98e522a1`, `git status --porcelain` empty before and after), not read from the fix commit body.

- Ordering — prior gate: `[GATE-IMPLEMENT] — ✅ PASS | 2026-08-28` above (line 325) carries per-criterion evidence lines (task path, bullet→TC mapping, PLAN outcome, whole-worktree inventory) — not a bare PASS.
- Ordering — input state: frontmatter `status: in-progress`; file under `.agents/spec-docs/active/`; `node scripts/harness/scan-doc-folder-status-agreement.mjs` → `violations=0 result=PASS`, exit 0. Paired Task `.agents/tasks/RUNTIME-007-runtime-host-shutdown-timer-test-measures-the-process-not-the-host.md` frontmatter `status: in-progress`.
- All tasks in the Task file complete: the Task carries no `[x]`/`[ ]` line at all (`grep -c` = 0 — per `.agents/tasks/README.md` § "What this tree is NOT" a Task is the problem record, and checkbox breakdowns are a retired artefact kind), so the tasks are the four `## Test Plan` bullets the GATE-IMPLEMENT entry bound to TC-01..TC-05, and completion is judged on the committed tree: bullet 1 (identity tracking) — `runtime-host.test.ts:130-167` holds the `async_hooks` `seen` map (never shrunk) / `destroyed` set, one `setImmediate` yield at `:155`, `seen.size >= 1` at `:160`, survivors-empty at `:164-167`; bullet 2 (isolation) — `:71-83` sets `HOME`/`USERPROFILE` to a fresh empty dir per test, restored at `:85-92`, labelled `Contained — TEST-012.` at `:71`; bullet 3 (control) — case `reads settings and plugins from the isolated home, not the developer's (TEST-012 control)` at `:170-190` asserts `createDefaultUserSettingsSources()` and `homedir()` inside and after the isolation; bullet 4 — the file, package and `harness:scan` runs recorded below. The literal `[x]` form is absent because the artifact kind has no checkbox to tick; the same shape passed this gate for HARNESS-126 on 2026-08-27. The spec's own `## Tasks` row is `[ ]`, which is correct while the Task is `in-progress` (README: the checkbox marks terminal disposition).
- No task blocked or pending: Task `status: in-progress` (not `blocked`); no bullet lacks a committed artifact or a run below.
- Build passes for affected packages: `git diff --name-only origin/develop...HEAD` → one path under `packages/`: `packages/agent-framework/src/runtime/__tests__/runtime-host.test.ts` (plus four `.agents/` documents). Judged applicable although test-only: the package's `tsconfig.json` has `include: ["src/**/*"]`, so the test file is inside the tsc program. `pnpm --filter @robota-sdk/agent-framework build` → `Build complete in 2316ms`, exit 0; `pnpm --filter @robota-sdk/agent-framework typecheck` (`tsgo --noEmit && tsgo -p tsconfig.examples.json --noEmit`) → exit 0.
- Tests pass — the file, real home: `pnpm --filter @robota-sdk/agent-framework exec vitest run src/runtime/__tests__/runtime-host.test.ts` → `Tests 6 passed (6)`, exit 0, with `~/.claude/settings.json` hooks = `{SessionStart}` (the condition that was red on `develop` `bb4c3626e`).
- Tests pass — TC-03 decoy home: `DECOY=$(mktemp -d)/home; mkdir -p $DECOY/.claude; printf '{"hooks":{"SessionStart":[{"matcher":"","hooks":[{"type":"command","command":"sleep 2","timeout":5}]}]}}' > $DECOY/.claude/settings.json; HOME=$DECOY USERPROFILE=$DECOY <same vitest command>` → `Tests 6 passed (6)`, exit 0. Empty home: `EMPTY=$(mktemp -d)/home; mkdir $EMPTY` (0 entries) `HOME=$EMPTY USERPROFILE=$EMPTY <same>` → `Tests 6 passed (6)`, exit 0. Decoy proven to take effect (control arm): `origin/develop`'s unfixed `runtime-host.test.ts`, written to a probe filename in the same directory and run under the same decoy → `× shutdown() leaves no timer holding the event loop open (#1852) → expected 3 to be less than or equal to 2`, exit 1; probe removed, `git status --porcelain` empty.
- Tests pass — TC-02 mutant re-run (the commit-body record was not taken on trust): `runtime-host.ts:99` `if (bound !== undefined) clearTimeout(bound);` (1 match) deleted with `sed -i`, `git diff --stat` = `1 file changed, 1 deletion(-)`; file run → `× shutdown() leaves no timer … → 1 timer(s) armed during the host's lifetime still hold the event loop after shutdown(): expected [ Array(1) ] to deeply equal []`, creation stack `runtime-host.ts:96:19 | … Object.shutdown (runtime-host.ts:95:9)`, exit 1 — the issue #1852 regression named by identity, other 5 cases green. Restored by copying the saved original back: `sha256` of the working file = `sha256` of `git show HEAD:…/runtime-host.ts` (`66db4fd74fdacd5e…`), `git status --porcelain` empty, `git diff | wc -l` = 0; re-run → `Tests 6 passed (6)`, exit 0.
- Tests pass — package: `pnpm --filter @robota-sdk/agent-framework exec vitest run` → `Test Files 195 passed (195)`, `Tests 1562 passed (1562)`, exit 0.
- Scans: `pnpm harness:scan` → `146 scans passed, 1 skipped (97 declared what they examined)`, exit 0. The skip was read, not counted: `↩ new-rule-declares-enforcement` declares `::examined:: 0 new rule sections ::expected-empty:: this change adds no rule SECTION of the form this floor reads` — this branch adds no rule section, so the declared reason holds. The scan's `dist` advisory (81 packages with `dist/` older than `src/`) is informational and exits 0.

### [GATE-COMPLETE: TC-01] — ✅ PASS | 2026-08-28

**Command:** `pnpm --filter @robota-sdk/agent-framework exec vitest run src/runtime/__tests__/runtime-host.test.ts --reporter=verbose` (real home; `~/.claude/settings.json` hooks = `['SessionStart']`, the condition that was red on `develop` `bb4c3626e`).
**Output:** `Test Files 1 passed (1)`, `Tests 6 passed (6)` — every case listed by name, including `shutdown() leaves no timer holding the event loop open (#1852)` — exit 0.

Checkbox `[x]`. The case (`runtime-host.test.ts:108-167`) asserts on identity: an `async_hooks` hook records every `Timeout` `init` into `seen` (never shrunk) and each `destroy` into `destroyed`; after `shutdown()` one `setImmediate` is awaited; `seen.size >= 1` and the survivors list (`!destroyed && hasRef()`) must be `[]`. No `getActiveResourcesInfo` count remains in the file. **Test reference:** TC-01 row of `## Test Plan` names the file > describe > case.

### [GATE-COMPLETE: TC-02] — ✅ PASS | 2026-08-28

**Action:** cited the GATE-VERIFY entry's independent mutant re-run (not the fix commit body), and cross-checked its restore hash against the tree this gate judges.
**Observed:** GATE-VERIFY deleted `runtime-host.ts:99` `if (bound !== undefined) clearTimeout(bound);`, ran the file → `× shutdown() leaves no timer … 1 timer(s) armed during the host's lifetime still hold the event loop after shutdown(): expected [ Array(1) ] to deeply equal []`, creation stack `runtime-host.ts:96:19`, exit 1, the other 5 cases green; restored to `sha256 66db4fd74fdacd5e…`, `git diff | wc -l` = 0. This gate: `sed -n 99p runtime-host.ts` → that exact line, `grep -c "clearTimeout(bound)"` = 1; `sha256sum` of the working file, of `git show HEAD:…/runtime-host.ts` and of `git show origin/develop:…/runtime-host.ts` are all `66db4fd74fdacd5ed1a7e32385a8ca4d0206e771d9451bef2184347305643c2c`; `git diff --stat -- packages/` empty — the restore is real and the file is unchanged from `develop`.

Checkbox `[x]`. **Test skipped** as a committed case with the reason on the TC-02 row (a mutant cannot be committed; hand-run and recorded) — accepted.

### [GATE-COMPLETE: TC-03] — ✅ PASS | 2026-08-28

**Decoy preparation:** `DECOY=$(mktemp -d)/home; mkdir -p $DECOY/.claude; printf '{"hooks":{"SessionStart":[{"matcher":"","hooks":[{"type":"command","command":"sleep 2","timeout":5}]}]}}' > $DECOY/.claude/settings.json` (`/tmp/tmp.J42QIINvnP/home`, file content re-read and matches the accepted shape).
**Decoy run:** `HOME=$DECOY USERPROFILE=$DECOY pnpm --filter @robota-sdk/agent-framework exec vitest run src/runtime/__tests__/runtime-host.test.ts` → `Tests 6 passed (6)`, exit 0.
**Empty home:** `EMPTY=$(mktemp -d)/home; mkdir $EMPTY` (0 entries). Through the `pnpm` shim this run exits 1/126 before vitest starts — `Volta error: Could not unpack Node v22.14.0 … pnpm shim: corepack not found next to node` — the Volta shim provisions its toolchain under `HOME` and an empty home has no `.volta`; this is the launcher, not the test (the decoy home had acquired a `.volta` by the time it was run). Re-run with the toolchain resolved outside the swapped home: `HOME=$EMPTY USERPROFILE=$EMPTY /home/ubunutu/.volta/tools/image/node/22.14.0/bin/node <node_modules/vitest/vitest.mjs> run src/runtime/__tests__/runtime-host.test.ts` (cwd `packages/agent-framework`) → `Tests 6 passed (6)`, exit 0; home still 0 entries afterwards. The decoy re-run the same direct way → `Tests 6 passed (6)`, exit 0.
**Control arm (the decoy reproduces the unfixed red):** `git show origin/develop:packages/agent-framework/src/runtime/__tests__/runtime-host.test.ts > src/runtime/__tests__/runtime-host.probe-2383.test.ts` (3 `getActiveResourcesInfo` matches), run under the same decoy the same direct way → `× … shutdown() leaves no timer holding the event loop open (#1852) → expected 3 to be less than or equal to 2`, `Tests 1 failed | 4 passed (5)`, exit 1. Probe removed; `git status --porcelain` shows only this spec document modified.
...
**Command:** the TC-01 verbose run above.
**Output:** `✓ … > reads settings and plugins from the isolated home, not the developer's (TEST-012 control) 1ms`, exit 0 — and the case also passed in the decoy and empty-home runs (6/6 each).

Checkbox `[x]`. The case (`runtime-host.test.ts:170-190`) asserts every `createDefaultUserSettingsSources()` path starts with the isolated `home` and `homedir() === home`, then restores `HOME`/`USERPROFILE` by the `afterEach` rule and asserts `homedir() !== home` and no source path under it. **Test reference:** TC-04 row names the file > case.

### [GATE-COMPLETE: TC-05] — ❌ FAIL | 2026-08-28

**Package suite:** `pnpm --filter @robota-sdk/agent-framework exec vitest run` → `Test Files 195 passed (195)`, `Tests 1562 passed (1562)`, exit 0 — met.
**Scans:** `pnpm harness:scan` on the tree under judgement (HEAD `a98e522a1` plus the uncommitted `status: verifying` + GATE-VERIFY entry that the archival commit will land; receipt not reused because the tree is not clean) → `1 of 147 scans failed`, exit 1. The failing scan: `reference-kind-qualified` — `[unfrozen] .agents/spec-docs/active/RUNTIME-007-….md: 1 unqualified reference(s) in a file the baseline does not know. A new document starts at zero`, at line 352: `… exit 1 — the issue #1852 regression named by identity …` — prose inside the uncommitted GATE-VERIFY entry that writes the number without `issue`. The committed tree at `a98e522a1` passed (GATE-VERIFY recorded `146 scans passed, 1 skipped`, exit 0); the branch as it will be committed does not.

Checkbox `[x]` — checked, but the `pnpm harness:scan` exit 0 half of the criterion is not met on the tree being handed forward. Required: `pnpm harness:scan` exit 0 on the branch.
