...
The source hierarchy under issue #1985 still represents typed MCP configuration, client transport, supervision, background calls, and schema projection as duplicate GitHub execution entries even though RULE-023 assigns internal implementation ownership to Tasks. Staging the declared Tasks without a relationship owner makes the repository planning-order guard refuse the multi-Task prelude, while processing them one by one repeats equivalent gates and read-backs.
...
- This exact AGREEMENT Task/spec and 5 child Tasks.
...
Use one atomic AGREEMENT for this group. The larger rollback unit is accepted because every row has the same administrative terminal meaning and exact independent Task owner; in exchange, planning and read-back are performed once for the group. issue #2521 and issue #2528 remain explicitly security/data-sensitive: their proposed absorption is conditioned on owner approval that URL-admission, untrusted-server-data, schema-admission, and projection-correctness constraints remain binding in the exact Tasks and source history. A row that needs an independently meaningful external lifecycle remains an Issue; a security/data flag alone is not silently treated as either retention or absorption authority.
...
- [x] MCP-003 — superseded — `.agents/tasks/completed/MCP-003-add-an-mcp-connection-and-capability-catalog-supervisor.md`
- [x] MCP-004 — done — `.agents/tasks/completed/MCP-004-hand-long-running-mcp-calls-to-background-tasks.md`
- [x] MCP-005 — done — `.agents/tasks/completed/MCP-005-project-mcp-tool-schemas-safely-across-providers.md`
...
- GATE-WRITE — Contains a concrete symptom: PASS — MCP client control-plane source rows under issue #1985 remain duplicate GitHub execution entries, and the repository planning-order guard refuses their multi-Task prelude when it lacks a relationship owner.
- GATE-WRITE — Contains a reproduction condition: PASS — the Problem identifies staging the five declared Tasks under issue #1985 without a relationship owner as the condition that triggers the refusal.
- GATE-WRITE — Does not contain `TBD`, `TODO`, or a vague single-sentence Problem: PASS — the Problem contains neither token and has 465 characters across two specific sentences.
...
- GATE-WRITE — Decision references the trade-off that drove the choice: PASS — it accepts a larger batch rollback unit in exchange for one planning and read-back cycle, while conditioning absorption of security/data issue #2521 and issue #2528 on owner approval and continued URL-admission, untrusted-data, schema-admission, and projection-correctness constraints in MCP-002 and MCP-005.
...
- GATE-WRITE — `## Tasks` section present with placeholder: PASS — the section names the paired AGREEMENT-014 execution record before MCP-001 through MCP-005.
...
**Review fingerprint:** d0168c286f41 (review e2f0b31f, type/tags 8493cd67)
...
- GATE-APPROVAL — No Architecture Review or frontmatter type/tags modified after approval: the `**Review fingerprint:**` recorded at approval (d0168c286f41) equals the document's current fingerprint
- GATE-APPROVAL — Approval is a direct, unambiguous statement directed at this spec document: PASS — the user replied `모두 승인함. 모든 너의 제안에 대해 타당한 근거가 있다면 자동으로 사전 승인함` to the current proposal containing AGREEMENT-014, including its explicit owner-approval condition for the sensitive rows; `모두 승인함` directly approves the presented work, and the stated evidence condition is satisfied by this document's independent GATE-WRITE PASS.
...
- GATE-IMPLEMENT — `.agents/tasks/<ID>.md` has been created: `## Tasks` names `.agents/tasks/AGREEMENT-014-coordinate-the-mcp-client-control-plane-migration.md`, which exists
- GATE-IMPLEMENT — Tasks file path is recorded in the `## Tasks` section of the spec document: `## Tasks` names `.agents/tasks/AGREEMENT-014-coordinate-the-mcp-client-control-plane-migration.md`, whose basename is the spec's
...
```json
{
  ...
  "deliveryMode": "single",
  "sequencedArtifacts": [],
  "taskPath": ".agents/tasks/AGREEMENT-014-coordinate-the-mcp-client-control-plane-migration.md",
  "specPath": ".agents/spec-docs/todo/AGREEMENT-014-coordinate-the-mcp-client-control-plane-migration.md",
  "taskItems": [
    {
      "kind": "tc-id",
      "value": "TC-01"
  ...
  "worktreePaths": [
    ".agents/spec-docs/todo/AGREEMENT-014-coordinate-the-mcp-client-control-plane-migration.md",
    ".agents/tasks/AGREEMENT-014-coordinate-the-mcp-client-control-plane-migration.md"
  ]
}
```
