...
The source hierarchy under issue #2073 still represents handoff authority, refusal policy, orchestration, and protocol codecs as duplicate GitHub execution entries even though RULE-023 assigns internal implementation ownership to Tasks; issue #2090 is its first leaf, not the hierarchy root. Staging the declared Tasks without a relationship owner makes the repository planning-order guard refuse the multi-Task prelude, while processing them one by one repeats equivalent gates and read-backs.
...
Use one atomic AGREEMENT for this group. The larger rollback unit is accepted because every row has the same administrative terminal meaning and exact independent Task owner; in exchange, planning and read-back are performed once for the group. RETAIN-class parent issue #2073 remains an Issue. Included security/data leaf issue #2090, issue #2106, issue #2126, and issue #2127 are internally sequenced session-mobility implementation slices with no separate claimant or release lifecycle; HANDOFF-002 through HANDOFF-005 preserve authority and refusal constraints and mutation remains separately gated.
...
- [ ] TC-04: Observable: canonical parent issue #2073 contains the complete exact owner map; every absorbed row has one `woojubb` marker, no P-priority label, `CLOSED/NOT_PLANNED`, a byte-identical source-body prefix, unchanged hierarchy and dependencies, and no removed assignee or historical URL; replay adds nothing.
...
`.agents/tasks/AGREEMENT-011-coordinate-the-session-mobility-handoff-migration.md`.
...
- GATE-WRITE — Contains a concrete symptom: PASS — session-mobility source rows under issue #2073 remain duplicate GitHub execution entries, and the repository planning-order guard refuses their multi-Task prelude when it lacks a relationship owner.
- GATE-WRITE — Contains a reproduction condition: PASS — the Problem identifies staging the four declared Tasks under true hierarchy root issue #2073, rather than first leaf issue #2090, without a relationship owner as the condition that triggers the refusal.
- GATE-WRITE — Does not contain `TBD`, `TODO`, or a vague single-sentence Problem: PASS — the Problem contains neither token and has 494 characters across two specific sentences.
...
- GATE-WRITE — Research findings or waiver feed Alternatives Considered and Decision: PASS — RULE-023's exact Task ownership model directly yields the serial, atomic, and retain-duplicate alternatives and the selected atomic projection while retaining parent issue #2073.
...
- GATE-WRITE — Decision references the trade-off that drove the choice: PASS — it accepts a larger batch rollback unit in exchange for one planning and read-back cycle; HANDOFF-002 through HANDOFF-005 preserve the authority and refusal constraints of security/data issue #2090, issue #2106, issue #2126, and issue #2127.
...
- GATE-WRITE — At least one criterion exists per distinct feature or sub-item: PASS — TC-01 covers HANDOFF-002 through HANDOFF-005 ownership; TC-02 dependency and prerequisite preservation; TC-03 exact manifest authority; TC-04 terminal mutation, issue #2073's complete parent map, preservation, and replay idempotence; and TC-05 reconciliation and scans.
...
- GATE-WRITE — `## Tasks` section present with placeholder: PASS — the section names the paired AGREEMENT-011 execution record before HANDOFF-002 through HANDOFF-005.
...
**Review fingerprint:** c22168444269 (review b14716ff, type/tags 8493cd67)
...
- GATE-APPROVAL — No Architecture Review or frontmatter type/tags modified after approval: the `**Review fingerprint:**` recorded at approval (c22168444269) equals the document's current fingerprint
- GATE-APPROVAL — Approval is a direct, unambiguous statement directed at this spec document: PASS — the user replied `모두 승인함. 모든 너의 제안에 대해 타당한 근거가 있다면 자동으로 사전 승인함` to the current proposal containing AGREEMENT-011; `모두 승인함` directly approves the presented work, and the stated evidence condition is satisfied by this document's independent GATE-WRITE PASS.
...
- GATE-IMPLEMENT — `.agents/tasks/<ID>.md` has been created: `## Tasks` names `.agents/tasks/AGREEMENT-011-coordinate-the-session-mobility-handoff-migration.md`, which exists
- GATE-IMPLEMENT — Tasks file path is recorded in the `## Tasks` section of the spec document: `## Tasks` names `.agents/tasks/AGREEMENT-011-coordinate-the-session-mobility-handoff-migration.md`, whose basename is the spec's
...
```json
{
  ...
  "deliveryMode": "single",
  "sequencedArtifacts": [],
  "taskPath": ".agents/tasks/AGREEMENT-011-coordinate-the-session-mobility-handoff-migration.md",
  "specPath": ".agents/spec-docs/todo/AGREEMENT-011-coordinate-the-session-mobility-handoff-migration.md",
  "taskItems": [
    {
      "kind": "tc-id",
      "value": "TC-01"
  ...
  "worktreePaths": [
    ".agents/spec-docs/todo/AGREEMENT-011-coordinate-the-session-mobility-handoff-migration.md",
    ".agents/tasks/AGREEMENT-011-coordinate-the-session-mobility-handoff-migration.md"
  ]
}
```
