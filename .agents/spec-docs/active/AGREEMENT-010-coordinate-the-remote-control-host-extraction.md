...
The source hierarchy under issue #2071 still represents remote-control reducer, ports, trust repositories, and CLI host wiring as duplicate GitHub execution entries even though RULE-023 assigns internal implementation ownership to Tasks. Staging the declared Tasks without a relationship owner makes the repository planning-order guard refuse the multi-Task prelude, while processing them one by one repeats equivalent gates and read-backs.
...
- This exact AGREEMENT Task/spec and 4 child Tasks.
...
Use one atomic AGREEMENT for this group. The larger rollback unit is accepted because every row has the same administrative terminal meaning and exact independent Task owner; in exchange, planning and read-back are performed once for the group. RETAIN-class rows remain Issues. Included security/data issue #2086, issue #2105, issue #2119, and issue #2120 form one internally sequenced remote-host extraction chain with no separate claimant or release lifecycle; REMOTE-015 through REMOTE-018 preserve their trust and data constraints and mutation remains separately gated.
...
- [ ] TC-04: Observable: the canonical parent contains the complete exact owner map; every absorbed row has one `woojubb` marker, no P-priority label, `CLOSED/NOT_PLANNED`, a byte-identical source-body prefix, unchanged hierarchy and dependencies, and no removed assignee or historical URL; replay adds nothing.
...
`.agents/tasks/AGREEMENT-010-coordinate-the-remote-control-host-extraction.md`.
...
- GATE-WRITE — Contains a concrete symptom: PASS — remote-host source rows under issue #2071 remain duplicate GitHub execution entries, and the repository planning-order guard refuses their multi-Task prelude when it lacks a relationship owner.
- GATE-WRITE — Contains a reproduction condition: PASS — the Problem identifies staging the four declared Tasks under issue #2071 without a relationship owner as the condition that triggers the refusal.
- GATE-WRITE — Does not contain `TBD`, `TODO`, or a vague single-sentence Problem: PASS — the Problem contains neither token and has 440 characters across two specific sentences.
...
- GATE-WRITE — Research findings or waiver feed Alternatives Considered and Decision: PASS — RULE-023's exact Task ownership model directly yields the serial, atomic, and retain-duplicate alternatives and the selected atomic projection.
...
- GATE-WRITE — Decision references the trade-off that drove the choice: PASS — it accepts a larger batch rollback unit in exchange for one planning and read-back cycle; REMOTE-015 through REMOTE-018 preserve the trust and data constraints of security/data issue #2086, issue #2105, issue #2119, and issue #2120.
...
- GATE-WRITE — At least one criterion exists per distinct feature or sub-item: PASS — TC-01 covers REMOTE-015 through REMOTE-018 ownership; TC-02 dependency and prerequisite preservation; TC-03 exact manifest authority; TC-04 terminal mutation, the complete parent map, preservation, and replay idempotence; and TC-05 reconciliation and scans.
...
- GATE-WRITE — `## Tasks` section present with placeholder: PASS — the section names the paired AGREEMENT-010 execution record before REMOTE-015 through REMOTE-018.
...
**Review fingerprint:** e9616b195aba (review 818aa10d, type/tags 8493cd67)
...
- GATE-APPROVAL — No Architecture Review or frontmatter type/tags modified after approval: the `**Review fingerprint:**` recorded at approval (e9616b195aba) equals the document's current fingerprint
- GATE-APPROVAL — Approval is a direct, unambiguous statement directed at this spec document: PASS — the user replied `모두 승인함. 모든 너의 제안에 대해 타당한 근거가 있다면 자동으로 사전 승인함` to the current proposal containing AGREEMENT-010; `모두 승인함` directly approves the presented work, and the stated evidence condition is satisfied by this document's independent GATE-WRITE PASS.
...
- GATE-IMPLEMENT — `.agents/tasks/<ID>.md` has been created: `## Tasks` names `.agents/tasks/AGREEMENT-010-coordinate-the-remote-control-host-extraction.md`, which exists
- GATE-IMPLEMENT — Tasks file path is recorded in the `## Tasks` section of the spec document: `## Tasks` names `.agents/tasks/AGREEMENT-010-coordinate-the-remote-control-host-extraction.md`, whose basename is the spec's
...
```json
{
  ...
  "deliveryMode": "single",
  "sequencedArtifacts": [],
  "taskPath": ".agents/tasks/AGREEMENT-010-coordinate-the-remote-control-host-extraction.md",
  "specPath": ".agents/spec-docs/todo/AGREEMENT-010-coordinate-the-remote-control-host-extraction.md",
  "taskItems": [
    {
      "kind": "tc-id",
      "value": "TC-01"
  ...
  "worktreePaths": [
    ".agents/spec-docs/todo/AGREEMENT-010-coordinate-the-remote-control-host-extraction.md",
    ".agents/tasks/AGREEMENT-010-coordinate-the-remote-control-host-extraction.md"
  ]
}
```
