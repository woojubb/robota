...
- [ ] TC-04: the daltonized built-ins pass the CVD guard — simulated protanopia and deuteranopia pairwise Lab ΔE over every status and diff colour pair is above the stated threshold — and the guard FAILS on a built-in value it cannot simulate (a named colour) and on a deliberately red/green pair.
