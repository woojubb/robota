...
- GATE-WRITE — Ordering check: entry gate, no prior gate required; `status: draft` in frontmatter and the file sits under `.agents/spec-docs/draft/`; the Evidence Log carried one non-gate record (`### Architecture validation — 2026-09-21`) and no gate entry, so this is the first run; none of the eight `## Affected Files` exist in the tree, so no work this gate authorizes has already happened.
...
- GATE-WRITE — Problem: concrete symptom: names the command `node scripts/harness/scan-user-execution-plan-order.mjs` over 60 topic commits with its findings by ID (undeclared `PUSH-2664`, out-of-order `DATA-2664`/`RULE-2582`/`BEHAVIOR-2663`/`RULE-2665`, `RULE-2326` checkpoint-mix), the merge conflict in `scripts/harness/__tests__/gate-checkpoint-evidence.test.mjs`, and measured counts (43/17/2 ordered pairs; 40/16/2 tree tuples). Verified: `git merge-tree --write-tree origin/develop 4214cb540` reports its first CONFLICT in that exact file.
- GATE-WRITE — Problem: reproduction condition: legacy tip `4214cb540` merged with `origin/develop`; the scanner on the clean historical sync fixture; replacement `720eb5e84` against the equality sentence in `git-branch.md` § Branch Policy, `backlog-execution.md` § Base Branch Workflow, `multi-backlog-initiative/SKILL.md` step 1. Verified: all four named OIDs (`4214cb540`, `720eb5e84`, `1ef05e0ea`, `58f24c1b7`) resolve to commits; the three documents carry the sentence at `git-branch.md:261`, `backlog-execution.md:1168`, `SKILL.md:50` in the two spellings the document describes.
- GATE-WRITE — Problem: no TBD/TODO, not a single-sentence description (2024 chars, 10 sentences) — mechanical PASS.
- GATE-WRITE — Prior Art Research: section present; cites four documentation sources (`git diff-tree`, `git merge-tree`, `git patch-id` reference pages; RFC 8785) — none third-party source code; `scan-spec-research` reports substantiated — 3 mechanical PASS.
- GATE-WRITE — Research feeds Alternatives/Decision: `diff-tree --raw -r -z --no-renames` observing mode/type/empty changes a patch ID cannot → Alternative 2's Con ("patch IDs cannot see mode, type, or empty changes") and the Decision's raw-tuple equality authority; `merge-tree`'s dependence on `merge.*` config and three gitattributes sources → the Decision's deferral of merge own-content to `BRANCH-2664-P2`; `patch-id --stable`'s empty-patch/whitespace blindness → the three-valued diagnostic flag pinned with `--text`; RFC 8785 → canonical bytes + SHA-256 digest. The research section's closing paragraph states this mapping explicitly; the recommendation is derived, not asserted.
- GATE-WRITE — Architecture Review Checklist: 5/5 `[x]`; Sibling scan `[x]` with completion evidence naming the owners measured and the precedent `.agents/evidence/RULE-023-child-issue-migration-manifest.json` (exists, 562,804 bytes); Alternatives Considered has 3 numbered entries each with Pro and Con — 3 mechanical PASS.
- GATE-WRITE — Decision references the trade-off: "Choose alternative 3, and ship only the non-merge verifier here" — Alternative 3's stated Con (a generic manifest/verifier surface plus one evidence file) against Alternative 1's prelude contradiction and Alternative 2's silent pass inside an allowed category; restated as "fail-closed review without the prelude contradiction, at the cost of one generic verifier module"; the merge own-content deferral names its own trade-off (a verifier that never claims completeness it lacks, backed by the measurement that all sixteen #2664 merges are clean with empty own-content).
- GATE-WRITE — New-surface placement (conditional): applicable — one new harness module and a new `.agents/evidence/migrations/` evidence directory. (a) Mirrors `scripts/harness/` scanner/verifier siblings (`verification-budget-runtime.mjs`, `shared.mjs`, `scan-user-execution-plan-order.mjs` whose enumeration it mirrors) and classifies itself as repository-private `INFRA`/`harness` migration verification, with evidence under the existing `.agents/evidence/` layer beside the RULE-023 manifest precedent. (b) Reuse is at the shared-core level: `envWithoutGitVars`/`resolveWorkspaceRoot` (`shared.mjs:437`/`:39`), `isEntryPoint` (`entrypoint.mjs:5`), `createVerificationRuntime`/`takeVerificationCommand` (`verification-budget-runtime.mjs:19`/`:58`) — all verified exports — and nothing else outside `node:`; no dependency on a sibling product. Independent validation is recorded, not self-claimed: fanouts `r20260921090348`, `r20260921092807`, `r20260921094601`, `r20260921102056` are each `terminal: converged` in `.agents/loop-runs/architecture-audit-fanout.jsonl`; `finding-depth-triager` `DEPTH VERDICT: LOCAL` and `proposal-reviewer` REVISE (`d1539c1bd`) → REVISE (`5644a9850`) → `REVIEW VERDICT: ENDORSE` (`3f3cb9733`) are recorded in the paired Task's `## Finding Evidence` under orchestrator run `r20260921104830` (bound to the Task in `backlog-execution-orchestrator.jsonl`) and mirrored in this Evidence Log's validation record.
- GATE-WRITE — Completion Criteria: every item `TC-NN:` prefixed (5) — mechanical PASS.
- GATE-WRITE — At least 1 criterion per distinct feature/sub-item: Solution 1 (module: schema, tuples, dispositions, merge records, canonical bytes/digest, budget, port, adapter, code set, CLI) → TC-01 and TC-03; Solution 2 (hermetic test + `HERMETIC_TEST_FILES` entry) → TC-01, TC-03, and TC-05's hermetic tier; Solution 3 (generalised prelude fixture, eight-children case) → TC-02; Solution 4 (one sentence, two pointers, owner-documents test) → TC-04; scans and tier runner → TC-05. All eight `## Affected Files` map to at least one TC.
- GATE-WRITE — Command form or Observable behavior form: TC-01 to TC-04 are `Observable:` with named codes, functions, return values, and fixtures; TC-05 is `Commands:` naming four exact commands and `exit 0`; no vague phrasing.
...
- GATE-IMPLEMENT — `.agents/tasks/<ID>.md` has been created: `## Tasks` names `.agents/tasks/MANIFEST-2664-verify-legacy-base-divergence-with-a-closed-manifest-and-bound-receipts.md`, which exists
- GATE-IMPLEMENT — Tasks file path is recorded in the `## Tasks` section of the spec document: `## Tasks` names `.agents/tasks/MANIFEST-2664-verify-legacy-base-divergence-with-a-closed-manifest-and-bound-receipts.md`, whose basename is the spec's
...
- GATE-IMPLEMENT — The tasks file includes a `## Test Plan` (or `## Testing` / `## 검증`) section with ≥50 chars — the `test-plans`: Task `## Test Plan` is 1154 chars
...
```json
{
  ...
  "deliveryMode": "single",
  "sequencedArtifacts": [],
  "taskPath": ".agents/tasks/MANIFEST-2664-verify-legacy-base-divergence-with-a-closed-manifest-and-bound-receipts.md",
  "specPath": ".agents/spec-docs/todo/MANIFEST-2664-verify-legacy-base-divergence-with-a-closed-manifest-and-bound-receipts.md",
  "taskItems": [
    {
      "kind": "tc-id",
      "value": "TC-01"
  ...
  "worktreePaths": [
    ...
    ".agents/spec-docs/todo/MANIFEST-2664-verify-legacy-base-divergence-with-a-closed-manifest-and-bound-receipts.md",
    ".agents/tasks/MANIFEST-2664-verify-legacy-base-divergence-with-a-closed-manifest-and-bound-receipts.md"
  ]
}
```
