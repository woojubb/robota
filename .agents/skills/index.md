...
| [pr-finding-resolution-loop](pr-finding-resolution-loop/SKILL.md)           | RESOLVE loop: local review before the push, then resolve what the PR's review automation reported, until `ACTIONABLE FINDINGS: 0` (progress-detection escape only), then the gated merge path (HARNESS-018) |
