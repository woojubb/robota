...
| `in-progress`, spec § Decision sequences delivery, and `git log <base>..HEAD -- .agents/spec-docs/active/<ID>.md` is empty (no checkpoint of this pair on this branch) | L2   | Invoke `backlog-gate-guard` subagent: GATE-IMPLEMENT (continuation) — `gate.mjs judge` has no continuation mode yet (issue #2422), so the guard judges the whole criteria list | `in-progress` (no transition) |
...
1. Run `node scripts/harness/gate.mjs judge --gate <GATE> --doc <PATH>`; on exit 0 with no semantic
   ...
   The one exception is the continuation row above: `gate.mjs judge` has no continuation mode (issue #2422) and fails closed on ordering for an `in-progress` input — outside `--dry-run` it would write a
   FAIL entry — so that row dispatches `backlog-gate-guard` directly and the script is not run.
