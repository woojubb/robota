...
description: Route completed code changes through applicable SPEC, verification, PR, publish, and documentation steps.
...
4. **README.** Update each modified package's `README.md` to match the SPEC changes (create it for
   new packages).
5. **Commit + PR.** After the verified implementation tree is clean, commit and push. Keep one coherent
   work-unit in ONE multi-commit PR per the PR Batching policy and ship per
   [git-branch.md](../../rules/git-branch.md).
