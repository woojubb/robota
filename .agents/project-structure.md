...
├── agent-transport/             # Transport-neutral session bridge, wire protocol, codecs and delivery helpers; browser-safe root/client plus Node-only admission and handoff helpers under ./node (STRUCT-012 S3)
├── agent-transport-protocol/    # Transitional empty tombstone retained only until STRUCT-012 S5 removes the retired package identity
├── agent-ui-*/                  # Presentation family: agent-ui-terminal (React/Ink terminal UI) and agent-ui-web (React GUI presentation core — session reducer + view components + desktop shell + SessionMonitor; private, consumed by apps/agent-app, packages/agent-cli-web, and agent-transport-webrtc-web). UI packages depend on shared contracts/runtime substrates, never on an agent-ui sibling.
├── agent-transport-*/           # Per-concern transport implementations: agent-transport-webrtc-web (browser WebRTC peer — native RTCPeerConnection answerer + RemoteClient + useRtcSession over agent-ui-web), -ws (WebSocket), -http (Hono), -mcp (MCP), and -webrtc (P2P RTCDataChannel, optional werift peer dep, REMOTE-001). -ws/-http/-mcp are contract-pure (deps: interface-transport + agent-transport only). -webrtc additionally depends on agent-remote-pairing (REMOTE-008): the pairing gate must live in wireChannel, where the DTLS fingerprints and pre-session channel frames are visible.
...
| `apps/agent-app`                      | Electron + React (Vite) | Desktop GUI shell rendering the shared GUI core `agent-ui-web`                            |
| `packages/agent-ui-web`               | React                   | Shared GUI presentation core — `SessionMonitor` + session reducer                         |
...
- **The dependency graph's ground truth is the manifests, never a document.** The workspace-internal edge set is the set of workspace-scoped entries in each package's `dependencies` + `peerDependencies`; `scripts/harness/check-dependency-direction.mjs` enforces the direction rules over that derived graph. When a document and the manifests disagree, the manifests are the fact and the document is the drift — which is the direction every conformance audit must diff in.
...
`ProgrammaticInteractionChannel` (in `agent-framework/src/transport-host`) is the concrete production implementation.
