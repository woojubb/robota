...
item is archived**. The reconciling agent on 2026-07-26 owned only `.agents/tasks/**`, so it could
...
> Status Invariants (spec-doc lifecycle vs `.agents/tasks/` item placement). They share the tokens
...
`.agents/tasks/completed/`. Two of the three are `.agents/rules/**` edits beyond this increment's
