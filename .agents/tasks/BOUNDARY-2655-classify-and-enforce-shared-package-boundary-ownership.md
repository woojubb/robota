...
- [x] TC-03: Migrate all classified owner-local material, including recording, hook, session migration and private PTY tooling, preserving behavior. Evidence: historical owner-local migration/scenario results below plus final-head build and TUI jobs in the closeout evidence.
...
- [x] TC-06: Verify focused and affected scope plus final remote CI and prepare per-criterion source/parent delivery evidence. Evidence: final-head CI, owner-landing record and Hume's revalidated MERGE VERIFIED PASS in the closeout addendum; parent/source projection remains Main's delivery action.
