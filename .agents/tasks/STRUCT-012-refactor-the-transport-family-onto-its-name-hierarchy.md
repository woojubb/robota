...
The new CLI-owned record was generated by the real Linux scenario recorder, not copied from history.
Its normalized stdout and stderr match the historical payload; stdout SHA256 is
`c1e2491a6b646ad43cfaca380bfec390ed4df5cb1e908052970d2088d3e699ae`.

Linux verification used Node 22.14.0 / pnpm 8.15.4 and image
`node@sha256:e5ddf893cc6aeab0e5126e4edae35aa43893e2836d1d246140167ccc2616f5d7`.
Source provenance is HEAD `f8ca20610787b8b26e68536fce4f8fdb379c1fc8` plus the dirty-source
snapshot archive SHA256 `80daf41f7621b542204d90a9235f9569adf6b7e035c14df8b5a008471184f6fa`,
with a path/mode/hash manifest. Product sources, scripts and manifests remained unchanged during
execution; stale, already-delivered PROC-034 governance copies were separately preserved and removed.
Frozen install, full build, scenario verify/record/verify and the CLI owner verifier passed.
Build and execution used isolated Linux outputs; runtime verification had no network access.
The owner command was `pnpm harness:verify -- --scope packages/agent-cli --include-scenarios --base-ref HEAD`
without skipping record checks. Its container-only synthetic Git HEAD is a fixture comparison base,
not source provenance or a planning checkpoint. Earlier filtered-build and missing-Git failures are
preserved; the successful retry used the full build and explicit scope, not relaxed checks.
