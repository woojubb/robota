...
- [ ] T2 (TC-01): Extract `startRuntimeHost()` into an **ink-free module** (NOT reached through `cli.ts`'s
...
- [ ] T3 (TC-02): Add the `robota --serve` headless entry (an ink-free entry module) that runs
...
- [ ] T4 (TC-03): Refactor `TuiInteractionChannel` — no longer constructs its own `InteractiveSession` and no
...
- [ ] T5 (TC-04): `apps/agent-app/electron/sidecar.ts` spawns `robota --serve` (extraArgs/command) — NOT the
...
- [ ] T6 (TC-05): dependency-direction (runtime surface presentation-free, no cycle); affected typecheck +
