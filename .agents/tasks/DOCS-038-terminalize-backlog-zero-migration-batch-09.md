...
- [ ] TC-01 — preserve the exact three-unit/ten-path manifest, governed input/postimage blobs,
      skipped/rejected dispositions, and excluded-scope boundary.
- [ ] TC-02 — preserve exact readback of control/owner issues and all three canonical handoffs; put
      only those exact comment URLs on the skipped Tasks and rejected plans.
- [ ] TC-03 — preserve all three Task bodies; rekey exactly three SEC-016 and one STRUCT-011 Task
      citations inside the rejected plans and reproduce plan postimages `4d092597...`/`ca8278c6...`.
- [ ] TC-04 — rekey only the SEC-016 standing-delegation baseline path from active to rejected,
      preserving cardinality 218 and every other entry/order; change no excluded path.
- [ ] TC-05 — keep the exact final changed-path set to ten approved lifecycle/ledger/baseline paths
