...
각 패키지의 번들링 상태를 감사하여 번들 사이즈가 적절한지, 불필요한 요소가 포함되지 않았는지, 패키지 간 의존 방향이 올바르게 번들되는지 확인. AWS Lambda, Cloudflare Dynamic Worker Loader 등 서버리스 환경에서 배포될 수 있으므로 번들 사이즈 관리가 중요.
