...
- **All 14 spun-out capabilities are archived complete.** `ls .agents/tasks/completed/ | grep -c '^SELFHOST-'`
  → **14**, and `grep -h '^status:' .agents/tasks/completed/SELFHOST-0*.md | sort | uniq -c` → **14
...
- **The three open follow-ups are separate live files**, so closing the index loses nothing:
  `.agents/tasks/SELFHOST-003-P4-embedding-vector-backend.md`,
  `.agents/tasks/SELFHOST-008-P5-concrete-semantic-backend.md`,
  `.agents/tasks/SELFHOST-011-P3-P4-evals-followups.md`.
- **Archiving repairs 14 dangling links.** Every child writes `Part of [SELFHOST-000](SELFHOST-000-self-hosting-capability-roadmap.md)`
  ...
  (`grep -rn '](SELFHOST-000-self-hosting-capability-roadmap.md)' .agents/tasks/completed | wc -l` →
  **14**). Every one of those resolves to `.agents/tasks/completed/SELFHOST-000-…md`, which did not
