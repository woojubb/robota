...
priority: high
urgency: now
area: packages/agent-cli-web, packages/agent-transport
depends_on: []
---

# INFRA-161: contain the browser bundle break in the CLI web package

Superseded by STRUCT-012 S3. That unit moved the protocol substrate into `agent-transport`, published
browser-safe root and `./client` entries, isolated Node-only code under `./node`, and rewired the GUI
consumer. The CLI web build and browser-subpath scan pass without the private alias proposed here, so
the original defect and the proposed containment no longer exist as independent work.
...
`.agents/spec-docs/rejected/INFRA-161-contain-the-browser-bundle-break-in-the-cli-web-package.md`
