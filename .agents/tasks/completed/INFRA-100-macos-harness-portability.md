...
- [x] TC-03: use one `canonicalTemporaryDirectory()` owner for both repository-check dispatch and
      direct tier execution, then unit-test path identity and both environment boundaries.
- [x] TC-04: run the complete `scripts/harness/__tests__` tier through `harness:verify` without an
      ad-hoc `TMPDIR` override.
- [x] TC-05: rerun provider-replay verification and the framework-owned public replay scenario with
...
- Added a failing canonical-temp-root unit test, then made repository-check dispatch and direct tier
  execution share the realpath-canonical `TMPDIR`; the focused launcher/helper tests pass.
...
- Canonicalize the temporary root at both harness-test process boundaries rather than mechanically
...
Vitest files after each minimal fix, then run `harness-scripts.test.mjs`,
`canonical-temporary-directory.test.mjs`, and `harness-test-tiers.test.mjs` for the shared canonical
temp-root contract. Finish with both complete harness tiers and the framework-owned public replay
scenario. Every command must run without a caller-supplied `TMPDIR` override and exit 0.
...
All five completion criteria are implemented and verified on stock macOS Bash 3.2. Repository-check
dispatch and direct tier execution now share the canonical temporary root, and the framework-owned
ARCH-014 scenario matches the checked-in combined offline scenario record; lint reports nine
pre-existing boundary-typing warnings and zero errors.
