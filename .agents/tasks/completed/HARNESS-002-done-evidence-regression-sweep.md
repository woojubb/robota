...
priority: high
urgency: soon
area: .agents/tasks, scripts/harness
depends_on: []
---
...
2. `harness:scan:done-evidence` — extract test-file paths referenced in
   `.agents/tasks/completed/*.md` evidence sections and fail when a referenced file no longer
   exists.
...
- `scripts/harness/check-done-evidence.mjs` shipped: validates repo-file references in the
  EVIDENCE regions of `.agents/tasks/completed/*.md`; `<!-- evidence-superseded: <reason> -->`
