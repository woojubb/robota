...
that declares no `test:pty` script. The 14 `*.ptytest.ts` suites do not run and the outcome reads as
...
Spec: `.agents/spec-docs/done/HARNESS-2660-a-filter-must-name-a-package-that-declares-the-script.md`

## Plan

- [x] Fix the run instruction in `packages/agent-transport-tui/vitest.pty.config.ts`
- [x] Export `check-ghost-package-refs.mjs`' immutable-historical-record predicate so the new scan
      reuses it instead of forking it
- [x] Add `scripts/harness/scan-filter-script-resolves.mjs`
- [x] Register it in `run-all-scans.mjs`, `scan-guard-scope-fail-closed.mjs` and the three
      registration baselines
- [x] Add `scripts/harness/__tests__/scan-filter-script-resolves.test.mjs`
- [x] Prove the guard bites: RED on the original filter, GREEN after the fix
