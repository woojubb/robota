...
`node scripts/examples/hook-timeout-demo.mjs` <!-- evidence-superseded: BOUNDARY-2655 moved this example to packages/agent-core/examples/hook-timeout-demo.mjs; the recorded command and output remain historical. -->
...
BOUNDARY-2655 relocated the example to `packages/agent-core/examples/hook-timeout-demo.mjs`;
the current owner-local invocation is `node examples/hook-timeout-demo.mjs` from `packages/agent-core`.
The historical transcript and elapsed times above are unchanged, not a current contract assertion.
BOUNDARY-2655 separately records the explicit timeout diagnostic and successful completion result;
reading the default constant is not runtime proof and a timeout is not currently described as exit 1.
