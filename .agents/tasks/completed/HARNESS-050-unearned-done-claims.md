...
`.agents/tasks/completed/*.md` still resolve**. That guards evidence _decay_ — a real artifact that
...
`.agents/tasks/INFRA-055-*.md` with a terminal status, which `check-backlog-placement.mjs` does
...
analysis over `.agents/tasks/**` items whose frontmatter says `status: done`.
...
`git show 0ba361d2d:.agents/tasks/INFRA-055-vacuous-required-checks-on-main-prs.md` (verified by
...
`.agents/tasks/completed/INFRA-055-vacuous-required-checks-on-main-prs.md` — zero findings. That
...
`.agents/tasks/completed/` (676 of which carry `status: done`, this item included): **0 findings**.
...
`.agents/tasks/completed/**` was outside the authorised paths for this change, so they are
...
- [x] No false positive across the existing `.agents/tasks/completed/` corpus — run it over every
...
- **Back-fill the 58 `LEGACY_EVIDENCE_DEBT` items** and drive the set to empty.
  `.agents/tasks/completed/**` was outside the authorised paths for this change.
