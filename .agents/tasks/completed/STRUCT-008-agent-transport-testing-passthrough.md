...
Superseded by STRUCT-012 S2. That merged unit removed the entire `agent-transport/testing` surface,
rewired its consumers to their actual owners, and left no pass-through export to remove. The planned
change therefore no longer exists as an independent work item.
