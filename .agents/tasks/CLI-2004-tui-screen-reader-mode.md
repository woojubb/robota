...
- [ ] Affected-set regression: `run-all-scans.mjs --affected --context pr` exits 0 — TC-15 — **red by design, 2 of 124 scans**, re-run on this head over develop `a5f363f7ed`: `user-execution-plan-order` (no `DONE-GATE-STAGE-1` PASS bound to the planning checkpoint, and that checkpoint carries the scenario ledger line) and `work-run-measurement` (no closure receipt — `work-run ready` requires the Task terminalized first). Both are the recorded USER DISPOSITION below, not defects in this change. Everything else passes, including the three findings develop's newly-landed scans raised on the rebase (`orphan-exports`, `loop-run-records`, `filter-script-resolves`), all fixed here.
...
| TC-15 | `node scripts/harness/run-all-scans.mjs --affected --context pr --skip dist --skip build-contracts`                                                         | red — exactly 2 of 124, both the disposition below (`user-execution-plan-order`, `work-run-measurement`); see the Plan item |
