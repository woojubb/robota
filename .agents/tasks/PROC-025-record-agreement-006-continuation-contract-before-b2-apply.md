...
- [ ] TC-01: Add exactly one six-path `Continuation artifacts` declaration to AGREEMENT-006 and verify
      the checkpoint-evidence parser returns the exact declared set.
- [ ] TC-02: Align the AGREEMENT-006 Task/spec pair at `in-progress` while preserving every prior
      GATE-IMPLEMENT PASS byte and the recorded 2026-08-31 continuation FAIL.
- [ ] TC-03: Pass staged/history plan-order and affected scans while proving the five named live Issues
      remain unchanged.
