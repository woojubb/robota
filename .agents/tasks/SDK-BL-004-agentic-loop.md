...
- Add focused CLI tests for non-interactive exit-code behavior and session persistence toggling.
- Add SDK tests that confirm bare mode skips context/plugin loading while preserving config loading.
- Run `pnpm --filter @robota-sdk/agent-cli test`, `pnpm --filter @robota-sdk/agent-sdk test`, and targeted builds before promoting implementation work.
