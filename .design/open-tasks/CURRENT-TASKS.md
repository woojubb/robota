...
### 최신 통합 규칙(요약)
- `workflow-event-rules.mdc` (Event + Workflow 규칙 통합)
- `type-ssot-rules.mdc` (Type + SSOT 통합)
- `execution-caching-rules.mdc` (캐싱 규칙 통합, example 용어 최소화)
- `git-operations-rules.mdc` (Git 운영 규칙 통합)
- `language-rules.mdc` (언어 규칙 통합)
- `development-architecture-rules.mdc` (개발/아키텍처 규칙 통합)
- `project-structure-rules.mdc` (구조/빌드 규칙 통합)
- `execution-safety-rules.mdc` (No‑Fallback + No‑Dedup 통합)
- `skills-link-rules.mdc` (스킬 링크 규칙 통합)
- `build-and-resolution-rules.mdc` (빌드 무결성 + 문제 해결 통합)
