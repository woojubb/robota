...
현재 `agent-cli`는 중립적인 단일 에이전트 하나를 조립한다. 이를 확장해, **미리 튜닝된 여러 "개성 있는" 프리셋**을 준비하고 사용자가 실행 시 선택할 수 있게 한다. 각 프리셋은 시스템 프롬프트(페르소나)·기본 모델/effort·권한/신뢰 프로파일·명령 모듈 구성을 조금씩 다르게 묶는다. 첫 프리셋은 **reference profile의 문서화된 작업 스타일**을 모방한다.
...
- → **프리셋 식별자는 generic 아키타입 명만 사용**한다. `reference`/`hermes` 같은 식별자 금지. 스타일은 모방하되 이름은 행동을 기술한다.
...
interface IPreset {
  ...
  // (3) 모델/effort (reference profile #1·#3 — thinking 상시성은 모델 고유, effort는 핀 가능한 메커니즘)
  model?: string; // 프리셋이 모델을 핀(예: reference 계열). always-on thinking은 모델 고유(프리셋이 만들지 못함)
  ...
  // (6) 실행 능력 (reference profile #4·#5·#8 — framework/executor seam을 켜는 메커니즘, 페르소나 텍스트 아님)
...
구현 완료)가 모든 프리셋을 균일하게 처리한다. reference profile 프리셋은 그 위의 **한 항목**일 뿐이다.
...
| PRESET-013 | 전환 시 모델/effort 라이브 재적용                                   | FLOW   | medium   | 011,008                      | `model`/`effort`/`temperature`/`maxOutputTokens`를 `robota.setModel` seam(세션 `applyModelOptions`)으로 재적용. (초기 가정한 `provider-hot-swap-requested` effect는 `profileName`만 운반 → 부적합, 정정. effort는 `IModelConfig`/`setModel`에 채널 추가.)                                                   |
| PRESET-014 | 전환 시 페르소나/시스템 프롬프트 라이브 재적용                      | FLOW   | low      | 011,003                      | 시스템 프롬프트 재합성(persona 섹션 교체) → `updateSystemMessage`. 기존 `rebuildSystemMessage` 클로저(persona+합성 입력 보존, staleness refresh 선례)에 mutable persona를 추가하고 `ICommandHostContext.applyPersona?` seam으로 노출. **FEASIBLE(기존 seam 확장)**                                          |
| PRESET-015 | 전환 시 명령 모듈 라이브 재선택                                     | FLOW   | low      | 011,004                      | `enabledCommandModules`/`disabledCommandModules` 재선택. 세션이 **전체 미필터 모듈 셋**을 보존하고, `SystemCommandExecutor.replaceCommands` + skill router `reapplyCommandModuleSelection` + `ICommandHostContext.applyCommandModuleSelection?` seam으로 재필터·재구성. **FEASIBLE(가산적 seam, 저위험)**   |
| PRESET-016 | 전환 시 `enableParallelSubagents` 라이브 게이팅                     | FLOW   | low      | 011                          | 런타임 mutable 게이트 + dispatch 시점 검사. **always-construct 금지**(기본 경로 자원 비용 회귀) — agent runtime이 조립 시 구성된 세션에서만 게이트 토글이 의미 있고, 미구성 세션은 다음 세션부터 반영(문서화). `ICommandSessionRuntime.setParallelSubagentsEnabled?` seam                                   |
| PRESET-017 | `selfVerification` 실행 단계 구현 + 라이브 게이팅 (**미설계 기능**) | FLOW   | low      | 011                          | `selfVerification`은 현재 **어디서도 소비되지 않는** 타입 전용 플래그 → 라이브 재적용이 무의미. 실제 "작업 후 자기검증 단계"를 executor에 구현해야 함(검증 대상/방식은 **제품 설계 결정**). 기능 구현 후에야 라이브 토글이 의미. 별도 기능 백로그                                                           |
| PRESET-006 | 프리셋 발견/관리 UX — `/preset` 명령 + 목록 + TUI 활성 표시(재범위) | SCREEN | medium   | 011 (전환 효과: 012,013,014) | `/preset` 목록 + active 마커(`getActivePresetId` 읽기), `/preset <id>` 전환(검증 + `applyPresetToSession` → 012~014 엔진 트리거), 상태 표시줄 active 표시. 명령/표시는 011만 있으면 착지 가능; 라이브 효과는 권한(012)+모델/effort(013)+페르소나(014)까지 채워지며, 모듈/실행능력 전환은 015(연기)까지 보류 |

**라이브 전환 의존 순서:** 011 → {012, 013, 014} → 006 → (015 연기). 012/013/014는 011 위에서 서로
독립이며 옵션 그룹별로 점진 착지한다. PRESET-006의 `/preset` 명령은 011 착지 후 목록/표시/전환-상태변경을
제공하고, 012~014가 누적될수록 전환의 **관찰 가능한 행동 효과**가 권한 → 모델/effort → 페르소나 순으로
채워진다. 명령 모듈/실행능력 전환(015)은 아키텍처 선행 작업이 필요해 별도 에픽으로 연기한다(피드백:
범위가 크면 별도 FLOW 백로그로 분산).

> **PRESET-014 분해 근거(피드백 §7.1 정정):** 원래 PRESET-014는 페르소나+모듈+실행능력을 한 백로그에
> 묶었으나, 타당성 조사 결과 (A) 페르소나/시스템 프롬프트 재합성만 기존 `rebuildSystemMessage` seam으로
> **즉시 가능**하고, (B) 명령 모듈 재선택과 (C) 실행능력(`enableParallelSubagents`/`selfVerification`)
> 재적용은 세션/agent runtime 재조립 또는 미구현 로직이 필요해 **현 아키텍처에서 불가**하다. 따라서
> PRESET-014를 페르소나 전용으로 재범위하고, B/C는 PRESET-015(연기 에픽)로 분리한다.
