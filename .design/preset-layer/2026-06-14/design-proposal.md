...
| PRESET-015 | 전환 시 명령 모듈 + 실행능력 라이브 재적용 (**연기/별도 에픽**)     | FLOW   | low      | 011,004                      | `enabledCommandModules`/`disabledCommandModules` 재선택 + `enableParallelSubagents`/`selfVerification` 재적용. **현 아키텍처에서 미지원** — 모듈은 init 시 systemCommands로 평탄화되어 레지스트리 변경 seam 없음; `enableParallelSubagents`는 조립 시 agent runtime에 구워짐(부분 재조립 필요); `selfVerification`은 아직 executor 소비 미구현. 레지스트리 mutability + 런타임 재조립 + selfVerification 구현이 선행. 별도 에픽으로 분리 |
