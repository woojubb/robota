...
| `autonomous-builder` | 능동·철저·자기검증·고자율 (reference profile 작업 스타일 모방) | **첫 의견 프리셋 (PRESET-005)** |
| `careful-reviewer`   | 읽기 중심, write/exec마다 ask, 근거 장황             | 후속                            |
| `neutral-executor`   | 얇은 페르소나, 지시에 충실, 최소 편집 (Hermes식)     | 후속                            |

> 식별자에 벤더명 없음. `autonomous-builder`의 description/문서에 "reference profile 작업 스타일에서 영감" 정도의 출처 각주는 허용(사용자 확인됨: generic 식별자).
