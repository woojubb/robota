...
사용자 결정: **전 영역 무제한 Fable-5 우선** (충돌 시 Fable-5가 이김). 소스:
`_SOURCE-fable5.md` (1,597줄).

## 결과물

| 백로그       | 영역                                                 | 보고서                          |
| ------------ | ---------------------------------------------------- | ------------------------------- |
| **RULE-001** | 거버넌스/우선순위 메타 (Fable-5 = conduct authority) | (이 문서)                       |
...
전부 `.agents/spec-docs/draft/` 에 `status: draft`. 우선순위 체인: **user > Fable-5 conduct > 기타 하네스 규칙 > 기본 동작.**

## 영역별 핵심

- **RULE-002 소통**: 하네스에 소통/포매팅 규칙이 거의 전무 → 12개 원칙 신규 채택(prose 우선, 볼드·불릿 최소, 응답당 질문 ≤1, 거절 시 불릿 금지 등). **유일한 실질 충돌**: Fable-5 "기술문서 prose, 불릿 금지" ↔ 하네스의 기계 파싱 구조물(SPEC 섹션·backlog frontmatter·scan 표). 무제한 우선이므로 Fable-5 채택하되, **구조화된 레포 산출물은 scope-separation**(대화/서술형=prose, 계약 구조=유지)으로 해결 권고 — GATE-APPROVAL 확인 필요.
- **RULE-003 태도**: 7개 신규(과한 사과 없는 책임성, 건설적 반박, 반대관점 제시, 단정 자제 등). **직접 충돌**: 하네스 "설계 컨펌 필수"·"단독결정 금지"(PLG-002) ↔ Fable-5 "명확하면 실행·과도한 deferral 금지". Fable-5 우선으로 해결 → develop 오염/단독결정 재발이 최상위 Implementation Risk.
...
- **RULE-005 안전**: 하네스에 안전 규칙 0 → 충돌 없음. 수백 줄 복붙은 base-model 중복이라 거부, 대신 **"Fable-5가 안전 권위"라는 한 문단 포인터**만 권고. 비포터블 항목(end_conversation, thumbs-down, 특정 핫라인, 분류기)은 제외.
