...
- [ ] **1.1 ActionTrackingEventService 구현**
  - [ ] `packages/agents/src/services/enhanced-event-service.ts` 생성
  - [ ] `ActionTrackingEventService` 클래스 구현 (EventService 인터페이스 준수)
  - [ ] 실행 계층 추적을 위한 `executionHierarchy: Map<string, ExecutionNode>` 구현
  - [ ] `emitWithContext()` 자동 계층 정보 추가 메서드 구현
  - [ ] Duck Typing 패턴으로 기존 EventService와 완벽 호환

- [ ] **1.2 ExecutionNode 및 관련 타입 정의**
  - [ ] `ExecutionNode` 인터페이스 정의 (id, parentId, level, children)
  - [ ] `trackExecution()` 메서드로 새 실행 추가
  - [ ] `findParent()` 메서드로 부모 자동 찾기
  - [ ] `createBoundEmit()` 메서드로 context가 바인딩된 emit 함수 생성

- [ ] **1.3 기존 EventService 인터페이스 확장**
  - [ ] `packages/agents/src/services/event-service.ts` 수정
  - [ ] `emitWithContext()` 메서드 추가 (optional, 기존 호환성 유지)
  - [ ] TypeScript 타입 정의 업데이트

### **📋 Phase 2: ToolExecutionService 통합 (30분)**

- [ ] **2.1 ToolExecutionService에서 Enhanced EventService 활용**
  - [ ] `packages/agents/src/services/tool-execution-service.ts` 수정
  - [ ] `executeTool()` 메서드에서 ActionTracker 감지 로직 추가
  - [ ] Duck Typing으로 Enhanced EventService 자동 감지
  - [ ] 감지 시 `createBoundEmit()` 활용하여 도구에 바인딩된 emit 전달

- [ ] **2.2 Zero-Configuration 구현**
  - [ ] ExecutionService가 ActionTracker 존재를 모르도록 구현
  - [ ] `eventService.trackExecution?.(executionId, parentId)` 패턴 사용
  - [ ] 기존 코드 100% 호환성 보장

### **📋 Phase 3: Playground Enhanced EventService 통합 (30분)**

- [ ] **3.1 PlaygroundExecutor에서 ActionTrackingEventService 주입**
  - [ ] `apps/web/src/lib/playground/robota-executor.ts` 수정
  - [ ] 기존 `PlaygroundEventService` 대신 `ActionTrackingEventService` 사용
  - [ ] `PlaygroundEventService`를 base로 감싸서 주입
  - [ ] Zero Breaking Change 보장

- [ ] **3.2 기존 PlaygroundEventService 리팩터링**
  - [ ] `PlaygroundEventService`를 pure EventService 구현체로 변환
  - [ ] ActionTracking 로직 제거하고 단순 emit만 담당
  - [ ] ActionTrackingEventService의 base로 활용
...
- [ ] **5.2 실제 팀 실행 검증 (핵심 테스트)**
  - [ ] **5.2.1 검증용 예제 파일 생성**
    ...
    - [ ] Playground와 동일한 환경 구성 (ActionTrackingEventService 사용)
