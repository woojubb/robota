...
**목표**: React-Flow와 Mermaid 기능을 agents 패키지에서 분리
**현재 진행상황**: 🔴 **시작 예정** (최우선 아키텍처 개선)

**🚨 분리 필요 이유**:
- **외부 의존성 분리**: `@xyflow/react`는 UI 라이브러리로 순수 비즈니스 로직 패키지인 `agents`에 부적합
- **도메인 명확화**: React-Flow는 시각화/UI 도메인, agents는 순수 비즈니스 로직 도메인
- **향후 확장성**: React-Flow와 Mermaid는 별도 패키지로 분리 예정이므로 미리 준비
- **의존성 최소화**: agents 패키지의 외부 의존성 최소화 원칙 준수
