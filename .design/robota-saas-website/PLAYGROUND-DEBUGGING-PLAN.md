...
### **📋 Phase 1: EventService 인터페이스 확장 (Zero Breaking Change)**

- [ ] **1.1 EventService 인터페이스 확장**
  - [ ] `packages/agents/src/services/event-service.ts` 파일 열기
  - [ ] `EventService` 인터페이스에 `emitWithContext` 메서드 추가 (optional)
  - [ ] 기존 `emit` 메서드는 그대로 유지 (호환성 보장)
  - [ ] `emitWithContext` 메서드로 자동 계층 정보 추출 기능 구현

- [ ] **1.2 SilentEventService와 DefaultEventService 업데이트**  
  - [ ] `SilentEventService`에 `emitWithContext` 구현 (빈 메서드)
  - [ ] `DefaultEventService`에 `emitWithContext` 구현 (console 출력)
  - [ ] 기존 코드 호환성 확인

- [ ] **1.3 타입 정의 확인 및 보완**
  - [ ] `ServiceEventData` 인터페이스에 필요한 계층 필드 확인
  - [ ] `ToolExecutionContext` 타입 임포트 확인
  - [ ] 타입 안전성 검증

### **📋 Phase 2: AgentDelegationTool 핵심 문제 해결**

- [ ] **2.1 toolExecutionId undefined 문제 해결**
  - [ ] `packages/team/src/tools/agent-delegation-tool.ts` 파일 열기
  - [ ] Line 93-117의 `executeWithHooks` 메서드 확인
  - [ ] `toolExecutionId` 생성 로직 검증 및 수정
  - [ ] `context?.executionId` fallback 로직 개선

- [ ] **2.2 HierarchyTracker 연결 수정**
  - [ ] `hierarchyTracker.registerToolExecution` 호출부 확인
  - [ ] `executionId`와 `parentId` 올바른 매핑 확인  
  - [ ] 정확한 계층 정보 등록 보장

- [ ] **2.3 Context 전달 체인 검증**
  - [ ] `enhancedContext` 생성 로직 확인 (Line 128-140)
  - [ ] `parentExecutionId` 설정이 올바른지 확인
  - [ ] `assignTask` 함수로 정확한 context 전달 보장

### **📋 Phase 3: TeamContainer 이벤트 발생 개선**

- [ ] **3.1 assignTask 메서드 이벤트 발생 수정**
  - [ ] `packages/team/src/team-container.ts` 파일 열기
  - [ ] Line 300-320의 이벤트 발생 코드 확인
  - [ ] `context?.executionId`를 `parentExecutionId`로 잘못 사용하는 부분 수정
  - [ ] 올바른 계층 정보 사용하도록 수정

- [ ] **3.2 emitWithContext 메서드 적용**
  - [ ] 기존 `this.eventService.emit` 호출을 `emitWithContext`로 변경
  - [ ] `ToolExecutionContext`를 두 번째 파라미터로 전달
  - [ ] 자동 계층 정보 추출 기능 활용

- [ ] **3.3 모든 팀 이벤트 타입 검증**
  - [ ] `team.analysis_start`, `team.analysis_complete` 이벤트 확인
  - [ ] `agent.creation_start`, `agent.creation_complete` 이벤트 확인  
  - [ ] `agent.execution_start`, `agent.execution_complete` 이벤트 확인
  - [ ] `task.aggregation_start`, `task.aggregation_complete` 이벤트 확인
...
- [ ] **5.2 실제 팀 실행 검증 (핵심 테스트)**
  - [ ] **5.2.1 검증용 예제 파일 생성**
    - [ ] `apps/examples/22-eventservice-team-test.ts` 파일을 복사하여 `apps/examples/23-hierarchy-verification-test.ts` 생성
    - [ ] Playground와 동일한 환경 구성 (PlaygroundEventService 사용)
    - [ ] 계층 구조 검증을 위한 이벤트 수집 및 분석 로직 추가
  ...
  - [ ] **5.2.3 팀 실행 시나리오 설정**
    - [ ] 간단한 2개 태스크 시나리오: "카페 메뉴 개발" + "마케팅 전략"
    - [ ] 각 태스크당 예상 이벤트 수: 8-10개 (team.analysis → agent.creation → agent.execution → task.aggregation)
    - [ ] 총 예상 이벤트 수: 16-20개 (기존 4개 대비 400-500% 증가)
  ...
  - [ ] **5.2.4 검증 실행 및 결과 분석**
    ...
    ```
    - [ ] 실행 전: 4개 평면 이벤트 확인
    - [ ] 실행 후: 16-20개 계층적 이벤트 확인
    - [ ] 각 이벤트의 `parentExecutionId`, `executionLevel`, `executionPath` 검증
    - [ ] 이벤트 트리 구조 시각적 출력 및 확인
...
- [ ] **5.3 계층 구조 상세 검증**
  ...
  - [ ] **5.3.3 성공 기준 정의**
    - [ ] ✅ **이벤트 수 증가**: 4개 → 16+ 개 (400% 이상 증가)
    - [ ] ✅ **계층 구조 형성**: `parentExecutionId` 연결이 올바른 트리 구조 형성
    - [ ] ✅ **실행 레벨 정확성**: conversation(0) → tool(1) → team(2) 순서
    - [ ] ✅ **이벤트 타입 다양성**: 8가지 이상 이벤트 타입 확인
    - [ ] ✅ **Zero Breaking Change**: 기존 예제 코드 변경 없이 동작
