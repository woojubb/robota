...
| **agent-transport**           | Lean transport core (pure TS, zero React/Ink): headless (`/headless`) + testing (`/testing`) only. HTTP, WebSocket, and MCP are standalone `agent-transport-{http,ws,mcp}` packages (split out in beta.76) | Transport    |
