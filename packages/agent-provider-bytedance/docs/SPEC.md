...
Owns ByteDance and BytePlus media provider integration for Robota SDK. Implements `IVideoGenerationProvider` from `@robota-sdk/agent-core` for Seedance video generation, including video task creation, polling, and cancellation via the BytePlus/ModelArk HTTP API.
