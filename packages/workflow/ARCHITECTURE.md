...
class MyCustomHandler implements EventHandler {
  ...
  async handle(eventType: string, eventData: unknown): Promise<WorkflowUpdate[]> {
    // EventService를 활용한 커스텀 처리 로직
    const childService = (this.eventService as any).createContextBoundInstance
      ? (this.eventService as any).createContextBoundInstance(this)
      : this.eventService;
