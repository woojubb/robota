...
When a call selects `toolChoice: none`, the provider omits its configured server-side web search.
This applies to both ordinary and streaming calls, including text-only forced summaries; otherwise
a server tool could still execute even when the core passed no local tool schemas.
