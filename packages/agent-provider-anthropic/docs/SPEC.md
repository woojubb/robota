...
4. **Provider definition** (`provider-definition.ts`) -- exposes `createAnthropicProviderDefinition()` with setup defaults, credential requirements, and provider construction through the common `IProviderDefinition` contract.
