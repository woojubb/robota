...
const stream = await agent.runStream(
  'Write a comprehensive analysis of quantum computing principles',
);
...
```typescript
import { RetryPlugin, ErrorHandlingPlugin } from '@robota-sdk/agent-core';
...
const agent = new Robota({
  ...
  plugins: [
    ...
    new ErrorHandlingPlugin({
      ...
      fallbackResponses: {
        overloaded_error:
          "I apologize, but I'm experiencing high demand. Please try again in a moment.",
      },
    }),
  ],
});
...
- **[@robota-sdk/agent-provider-openai](../openai/)**: OpenAI GPT provider
