...
| remote-control | `createRemoteControlCommandModule` | `RemoteControlCommandSource` | `executeRemoteControlCommand` (REMOTE-008: `/remote-control` enable/stop/status — a declarative trigger returning enable/stop effects; status via the injected `remoteControl` adapter)                    |
