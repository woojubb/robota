...
import { hasValidRunDraft, hasValidRunResult } from '../client/designer-api-client.js';
import type { IRunDraft, IRunResult } from '@robota-sdk/dag-core';
...
describe('hasValidRunDraft contract validation', () => {
  ...
  it('accepts a valid run draft', () => {
    expect(hasValidRunDraft(draft)).toBe(true);
  });

  it('rejects a draft without separate node state map', () => {
    const invalid = {
      ...draft,
      nodeStateMap: undefined,
    } as unknown as IRunDraft;

    expect(hasValidRunDraft(invalid)).toBe(false);
  });
});
