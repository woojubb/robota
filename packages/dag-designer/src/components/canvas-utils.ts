...
export function toNode(
  ...
  const nodeWithPorts = enrichNodeWithPorts(nodeDefinition, objectInfo);
  const inputs = nodeWithPorts.inputs ?? [];
  const outputs = nodeWithPorts.outputs ?? [];
...
export function hasSameCanvasNodeState(currentNodes: Node[], nextNodes: Node[]): boolean {
  ...
  return currentNodes.every((currentNode, index) => {
    ...
    const currentExecutionStatus =
      (currentNode.data as { executionStatus?: TNodeExecutionStatus } | undefined)
        ?.executionStatus ?? 'idle';
    const nextExecutionStatus =
      (nextNode.data as { executionStatus?: TNodeExecutionStatus } | undefined)?.executionStatus ??
      'idle';
    const currentSelected =
      (currentNode.data as { isSelected?: boolean } | undefined)?.isSelected ?? false;
    const nextSelected =
      (nextNode.data as { isSelected?: boolean } | undefined)?.isSelected ?? false;
    const currentTraceSignature = (currentNode.data as { traceSignature?: string } | undefined)
      ?.traceSignature;
    const nextTraceSignature = (nextNode.data as { traceSignature?: string } | undefined)
      ?.traceSignature;
    const currentHandles = (
      currentNode.data as { inputHandlesByPortKey?: Record<string, string[]> } | undefined
    )?.inputHandlesByPortKey;
    const nextHandles = (
      nextNode.data as { inputHandlesByPortKey?: Record<string, string[]> } | undefined
    )?.inputHandlesByPortKey;
    const currentInputs = (currentNode.data as { inputs?: IPortDefinition[] } | undefined)?.inputs;
    const nextInputs = (nextNode.data as { inputs?: IPortDefinition[] } | undefined)?.inputs;
    const currentOutputs = (currentNode.data as { outputs?: IPortDefinition[] } | undefined)
      ?.outputs;
    const nextOutputs = (nextNode.data as { outputs?: IPortDefinition[] } | undefined)?.outputs;
    const handlesChanged = JSON.stringify(currentHandles) !== JSON.stringify(nextHandles);
    const portsChanged =
      JSON.stringify(currentInputs) !== JSON.stringify(nextInputs) ||
      JSON.stringify(currentOutputs) !== JSON.stringify(nextOutputs);
    return (
...
function findManifestForNode(
  nodeType: string,
  manifests?: INodeManifest[],
): INodeManifest | undefined {
  return manifests?.find((manifest) => manifest.nodeType === nodeType);
}

/**
 * Adds runtime-owned port definitions for view and validation use. The runtime
 * catalog wins over stale node-local port snapshots. Node-local ports are only
 * a compatibility fallback when no catalog source is available.
 */
export function enrichNodeWithPorts(
  node: IDagNode,
  objectInfo?: TObjectInfo,
  manifests?: INodeManifest[],
): IDagNode {
  ...
  if (info) {
    return {
      ...node,
      inputs: deriveInputPorts(info),
      outputs: deriveOutputPorts(info),
    };
  }

  const manifest = findManifestForNode(node.nodeType, manifests);
  if (manifest) {
    return {
      ...node,
...
export function enrichDefinitionWithPorts(
  ...
  return {
    ...definition,
    nodes: definition.nodes.map((node) => enrichNodeWithPorts(node, objectInfo, manifests)),
  };
}

export function stripNodePortDefinitions(node: IDagNode): IDagNode {
  const strippedNode: IDagNode = {
    nodeId: node.nodeId,
  ...
  if (node.triggerPolicy) {
    strippedNode.triggerPolicy = node.triggerPolicy;
  }
  if (node.retryPolicy) {
    strippedNode.retryPolicy = node.retryPolicy;
  }
  if (typeof node.timeoutMs === 'number') {
    strippedNode.timeoutMs = node.timeoutMs;
  }
  if (node.costPolicy) {
    strippedNode.costPolicy = node.costPolicy;
  }
  return strippedNode;
}

export function stripDefinitionNodePortDefinitions(definition: IDagDefinition): IDagDefinition {
  return {
    ...definition,
    nodes: definition.nodes.map((node) => stripNodePortDefinitions(node)),
  };
}
...
export function compactListBindings(definition: IDagDefinition): IDagDefinition {
  ...
  const nextEdges = definition.edges.map((edge) => {
    ...
    const nextBindings = edge.bindings.map((binding) => {
      ...
      const directListPort = (targetNode.inputs ?? []).find(
        (port) => port.key === binding.inputKey && port.isList,
      );
      const listPortKey = directListPort ? directListPort.key : listHandle?.portKey;
      if (!listPortKey) {
        return binding;
      }
      const listPort = (targetNode.inputs ?? []).find(
        (port) => port.key === listPortKey && port.isList,
      );
      if (!listPort) {
        return binding;
      }
