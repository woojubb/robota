...
const CATEGORY_COLORS: Record<string, { accent: string; dim: string }> = {
  'ai-inference': {
    accent: 'var(--studio-accent-violet, #a78bfa)',
...
const DEFAULT_CATEGORY_COLOR = {
  accent: 'var(--studio-text-muted, #5c5c7a)',
  dim: 'rgba(92, 92, 122, 0.15)',
};
...
export function DagNodeView(props: NodeProps<TDagCanvasNode>): ReactElement {
  ...
  const rootStyle = useMemo((): CSSProperties => {
    const base: CSSProperties = {
      ...
      fontSize: 13,
      transition:
        'background-color 500ms ease-out, border-color 500ms ease-out, box-shadow 500ms ease-out',
    };
    ...
    if (isSelected) {
      base.boxShadow =
        '0 0 0 2px var(--studio-accent-violet, #a78bfa), 0 0 24px rgba(167, 139, 250, 0.25)';
    }
    ...
    <div className="transform-gpu" style={rootStyle}>
      ...
      <div className="grid grid-cols-2 gap-3 py-3">
        <div className="nodrag flex flex-col gap-2">
          ...
          {inputs.length === 0 ? (
            <div className="px-3 text-xs" style={{ color: 'var(--studio-text-muted, #5c5c7a)' }}>
              No inputs
            </div>
          ) : (
            inputs.map((port) => {
              const inputHandles = port.isList
                ? (props.data.inputHandlesByPortKey?.[port.key] ?? [
                    buildListPortHandleKey(port.key, 0),
                  ])
                : [port.key];
              return (
                    ...
                    return (
                      <div key={`${port.key}:${handleId}`} className="relative pl-6">
                        ...
                        <div className={`leading-tight ${isPendingSlot ? 'opacity-50' : ''}`}>
                          ...
                          >
                            {port.isList ? handleId : port.key} · {port.type} ·{' '}
