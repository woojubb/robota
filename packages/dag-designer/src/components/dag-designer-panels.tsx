...
export function DagDesignerNodeConfig(props: IDagDesignerNodeConfigProps): ReactElement {
  ...
  const selectedNode = useMemo(
    () =>
      context.definitionWithRuntimePorts.nodes.find(
        (node) => node.nodeId === context.selectedNodeId,
      ),
    [context.definitionWithRuntimePorts.nodes, context.selectedNodeId],
  );
  ...
  return (
    <div className={props.className ?? ''}>
      <NodeConfigPanel
        node={selectedNode}
        definition={context.definitionWithRuntimePorts}
...
export function DagDesignerRunProgressSummary(
  ...
  return <div className={`text-xs ${props.className ?? ''}`}>{summaryText}</div>;
}
