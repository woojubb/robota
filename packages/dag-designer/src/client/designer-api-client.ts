...
import type {
  ...
  IDesignerGetRunDraftInput,
  IDesignerOverwriteRunDraftNodeResultInput,
  IDesignerResetRunDraftNodeResultInput,
  IDesignerSaveRunDraftInput,
...
export function hasValidRunDraft(draft: IRunDraft): boolean {
  return (
    ...
    draft.input !== null &&
    typeof draft.nodeStateMap === 'object' &&
    draft.nodeStateMap !== null &&
    typeof draft.createdAt === 'string' &&
    typeof draft.updatedAt === 'string' &&
    (typeof draft.runResult === 'undefined' || hasValidRunResult(draft.runResult))
  );
}
...
export class DesignerApiClient implements IDesignerApiClient {
  ...
  public async listDefinitions(
    ...
    return {
      ok: false,
      error: [createContractViolationProblem(HTTP_OK, path)],
    };
  }
  ...
  public async listObjectInfo(): Promise<TResult<TObjectInfo, IProblemDetails[]>> {
    ...
    return { ok: false, error: [createContractViolationProblem(HTTP_OK, path)] };
  }
  ...
  public async saveRunDraft(
    input: IDesignerSaveRunDraftInput,
  ): Promise<TResult<IRunDraft, IProblemDetails[]>> {
    return this.requestRunDraft(
      '/v1/dag/run-drafts',
      'POST',
      JSON.stringify({
  ...
  public async getRunDraft(
    input: IDesignerGetRunDraftInput,
  ): Promise<TResult<IRunDraft, IProblemDetails[]>> {
    return this.requestRunDraft(
      `/v1/dag/run-drafts/${encodeURIComponent(input.draftId)}`,
      'GET',
      undefined,
      input.correlationId,
    );
  ...
  public async resetRunDraftNodeResult(
    input: IDesignerResetRunDraftNodeResultInput,
  ): Promise<TResult<IRunDraft, IProblemDetails[]>> {
    return this.requestRunDraft(
      `/v1/dag/run-drafts/${encodeURIComponent(input.draftId)}/nodes/${encodeURIComponent(input.nodeId)}/reset`,
      'PUT',
      JSON.stringify({}),
      input.correlationId,
    );
  ...
  public async overwriteRunDraftNodeResult(
    input: IDesignerOverwriteRunDraftNodeResultInput,
  ): Promise<TResult<IRunDraft, IProblemDetails[]>> {
    return this.requestRunDraft(
      `/v1/dag/run-drafts/${encodeURIComponent(input.draftId)}/nodes/${encodeURIComponent(input.nodeId)}/result`,
      'PUT',
      JSON.stringify({
        input: input.input,
        output: input.output,
  ...
  private async requestRunDraft(
    ...
    const draft = payloadResult.value.data?.draft;
    if (draft && hasValidRunDraft(draft)) {
      return {
        ok: true,
        value: draft,
      };
  ...
  private async requestPayload(
    ...
    if (
      ...
      hasValidProblemDetails(payload.errors)
    ) {
      return {
        ok: false,
