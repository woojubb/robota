import { Robota, type IEventService } from '@robota-sdk/agents';
import type {
  IAgentConfig,
  IAIProvider,
  IToolSchema,
...
import { FunctionTool } from '@robota-sdk/tools';
import { RelayMcpTool } from '@robota-sdk/tool-mcp';
import { bindWithOwnerPath } from '@robota-sdk/event-service';
...
function validateTemplateList(data: unknown): ITemplateEntry[] {
  ...
  for (const entry of data) {
    if (
      ...
      throw new Error(
        '[relay-assign-task] Invalid template entry: missing required fields (id, name, provider, model, systemMessage)',
      );
    }
  }
 ...
 */
export const listTemplateCategoriesTool = new FunctionTool(
  listTemplateCategoriesSchema,
  async () => {
    const categories = [
      {
        id: 'default',
 ...
 */
export const listTemplatesTool = new FunctionTool(
  listTemplatesSchema,
 ...
 */
export const getTemplateDetailTool = new FunctionTool(
 ...
 */
export function createAssignTaskRelayTool(
  ...
  return new RelayMcpTool({
    ...
    run: async (params: TToolParameters, ctx): Promise<IToolResult> => {
      ...
      const parentOwnerPath: IOwnerPathSegment[] = Array.isArray(ctx.ownerPath)
        ? ctx.ownerPath
            .filter(
              (s: IOwnerPathSegment): s is IOwnerPathSegment =>
                s !== null &&
                typeof s === 'object' &&
