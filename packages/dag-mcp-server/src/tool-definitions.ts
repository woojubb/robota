...
export const DAG_MCP_TOOL_DEFINITIONS: readonly IDagMcpToolDefinition[] = [
  ...
  },
  {
    name: 'dag_build',
    description:
      'Build an IDagDefinition from a declarative pipeline spec. Each stage is either a single node or a { parallel: [...] } fan-out. Returns the definition, nodeCount, edgeCount, and any warnings.',
    inputSchema: objectSchema(
      {
        pipeline: {
          type: 'array',
          description:
            'Ordered pipeline stages. Each item is { nodeType, id?, config?, fromPort?, toPort? } or { parallel: [...] }.',
        },
        dagId: { type: 'string', description: 'Optional DAG ID. Auto-generated if omitted.' },
      },
      ['pipeline'],
    ),
  },
  {
    name: 'dag_validate',
    description:
      'Validate an IDagDefinition against the registered node catalog. Returns { valid, errors }.',
    inputSchema: objectSchema({ definition: { type: 'object' } }, ['definition']),
  },
];
