...
class FakeDagClient implements IDagOrchestrationPort {
  ...
  public async buildDag(input: object) {
    this.calls.push({ method: 'buildDag', payload: { input } });
    return { ok: true, status: 200, payload: { ok: true, status: 200, data: {} } };
  }

  public async validateDag(definition: object) {
    this.calls.push({ method: 'validateDag', payload: { definition } });
    return {
      ok: true,
      status: 200,
      payload: { ok: true, status: 200, data: { valid: true, errors: [] } },
