...
const handlers: Record<string, TToolHandler> = {
  ...
  dag_build: async (args, client) => {
    const pipeline = requireObject(args, 'pipeline');
    if (!pipeline.ok) return usageError(pipeline.detail);
    const dagId = optionalString(args, 'dagId');
    const input: IDagBuildInput = {
      pipeline: pipeline.value as unknown as IDagBuildInput['pipeline'],
      ...(typeof dagId === 'string' ? { dagId } : {}),
    };
    return toMcpResult(await client.buildDag(input));
  },
  dag_validate: async (args, client) => {
    const definition = requireObject(args, 'definition');
    if (!definition.ok) return usageError(definition.detail);
    return toMcpResult(await client.validateDag(definition.value as object as IDagDefinition));
  },
};
