...
function makeToolUseResponse(toolId: string, name: string, input: Record<string, unknown>): Anthropic.Message {
    return {
        id: 'msg_test',
        type: 'message',
        role: 'assistant',
...
describe('AnthropicProvider', () => {
    ...
    describe('chat — validation', () => {
        ...
        it('should throw when messages is not an array', async () => {
            await expect(provider.chat('not-an-array' as unknown as TUniversalMessage[], {}))
        ...
        it('should throw when model is not specified', async () => {
            ...
            await expect(provider.chat(messages, {}))
                .rejects.toThrow('Model is required in chat options');
        });
    });
    ...
    describe('chat — direct execution', () => {
        ...
        it('should throw when response has no content', async () => {
            ...
            await expect(provider.chat(messages, { model: 'claude-3-opus-20240229' }))
        ...
        it('should throw when response has unsupported content type', async () => {
            ...
            await expect(provider.chat(messages, { model: 'claude-3-opus-20240229' }))
                .rejects.toThrow('Unsupported content type: image');
        });
    ...
    describe('chat — executor delegation', () => {
        it('should delegate to executor when configured', async () => {
            ...
            expect(executor.executeChat).toHaveBeenCalledWith(expect.objectContaining({
                provider: 'anthropic',
                model: 'claude-3-opus-20240229'
            }));
        });
