...
/** JSON-safe value type for parsed settings files */
type TJsonValue = string | number | boolean | null | TJsonValue[] | { [key: string]: TJsonValue };

/**
 * Read and parse a JSON file. Returns undefined if the file does not exist.
 * Throws on parse errors.
 */
function readJsonFile(filePath: string): TJsonValue | undefined {
  if (!existsSync(filePath)) {
    return undefined;
  }
  const raw = readFileSync(filePath, 'utf-8');
  return JSON.parse(raw) as TJsonValue;
}
