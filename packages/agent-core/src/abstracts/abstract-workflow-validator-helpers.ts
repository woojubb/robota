...
export function createValidationResultHelper(
  ...
  return {
    ...
    metadata: {
      ...
      dataStats: getDataStats(extractSimpleData(data)),
      version: validatorVersion,
      options: extractSimpleOptions(options),
    },
  };
}
...
export function updateStatisticsHelper(stats: IValidatorStats, result: IValidationResult): void {
  if (result.isValid) stats.successfulValidations++;
  else stats.failedValidations++;
  ...
  for (const issue of result.issues) {
    ...
    stats.issuesBySeverity.set(
      issue.severity,
      (stats.issuesBySeverity.get(issue.severity) || 0) + 1,
    );
  }
}

/** @internal */
export function buildValidatorStatsOutput(
  stats: IValidatorStats,
  ruleConfigs: Map<string, IValidationRuleConfig>,
): {
  ...
  mostCommonIssues: Array<{ rule: string; count: number; severity: ValidationSeverity }>;
  lastValidationAt?: Date;
} {
  const mostCommonIssues = Array.from(stats.issuesByRule.entries())
    .sort(([, a], [, b]) => b - a)
    .slice(0, TOP_ISSUES_COUNT)
    .map(([rule, count]) => ({
      rule,
      count,
      severity: ruleConfigs.get(rule)?.severity || ValidationSeverity.ERROR,
    }));
  return {
    ...
    averageProcessingTime:
      stats.totalValidations > 0 ? stats.totalProcessingTime / stats.totalValidations : 0,
    averageIssueCount:
      stats.totalValidations > 0 ? stats.totalIssueCount / stats.totalValidations : 0,
    mostCommonIssues,
...
/** @internal */
export function filterIssuesHelper(
  issues: IValidationIssue[],
  options: IValidationOptions,
): IValidationIssue[] {
  let filtered = [...issues];
  if (!options.includeWarnings)
    filtered = filtered.filter((i) => i.severity !== ValidationSeverity.WARNING);
  if (!options.includeInfo)
    filtered = filtered.filter((i) => i.severity !== ValidationSeverity.INFO);
...
export function getEnabledRulesHelper(
  ...
  let rules = availableRules.filter((r) => {
    const c = ruleConfigs.get(r);
...
export function extractSimpleData(data: IWorkflowData): Record<string, string | number | boolean> {
  ...
  if (data && typeof data === 'object') {
    ...
    if (Array.isArray(nodes)) result.nodes = nodes.length;
    const edges = obj['edges'];
    if (Array.isArray(edges)) result.edges = edges.length;
    const node = obj['node'];
    if (Array.isArray(node)) result.node = node.length;
    const connections = obj['connections'];
    if (Array.isArray(connections)) result.connections = connections.length;
  }
  return result;
}

function extractSimpleOptions(
  options: IValidationOptions,
): string | number | boolean | string[] | Date {
  if (typeof options === 'object' && options !== null)
    return `${JSON.stringify(options).substring(0, PREVIEW_LENGTH)}...`;
