 ...
 */
export type TSchemaValidationInput =
  | Record<
      string,
      string | number | boolean | Date | string[] | number[] | boolean[] | null | undefined
    >
  | string
  | number
  | boolean
 ...
 */
export const AgentTemplateMetadataSchema = z
  .object({
...
export const AgentTemplateSchema = z
  .object({
    name: z
      ...
      .regex(
        /^[a-z0-9_-]+$/,
        'Template name must contain only lowercase letters, numbers, underscores, and hyphens',
      ),
    ...
    llm_provider: z
      ...
      .regex(
        /^[a-z0-9_-]+$/,
        'Provider name must contain only lowercase letters, numbers, underscores, and hyphens',
      ),
    ...
    maxTokens: z.number().min(1).max(MAX_TOKENS_LIMIT).optional(),
