...
describe('ConversationSession', () => {
  ...
  describe('High-level API data integrity', () => {
    it('should preserve assistant message with null content and tool calls', () => {
      ...
      expect(isAssistantMessage(messages[1]) && messages[1].toolCalls?.[0].id).toBe(
        'call_calculate_123',
      );
    ...
    it('should preserve assistant message with meaningful content and tool calls', () => {
      ...
      expect(isAssistantMessage(assistantMessage) && assistantMessage.toolCalls).toHaveLength(1);
      expect(
        isAssistantMessage(assistantMessage) && assistantMessage.toolCalls?.[0].function.name,
      ).toBe('calculate');
    });
    ...
    it('should preserve multimodal parts for user, assistant, system, and tool messages', () => {
      ...
      session.addAssistantMessage('done', undefined, undefined, [
        { type: 'text', text: 'generated result' },
        { type: 'image_inline', mimeType: 'image/png', data: 'cmVzdWx0LWltYWdl' },
      ]);
      session.addSystemMessage('system with part', undefined, [
        { type: 'text', text: 'system-part' },
      ]);
      session.addToolMessageWithId('{"ok":true}', 'tool-call-parts', 'testTool', undefined, [
...
describe('ConversationHistory', () => {
  ...
  describe('Tool execution loop prevention integration test', () => {
    it('should prevent infinite tool execution by preserving conversation history correctly', () => {
      ...
      expect(
        isAssistantMessage(assistantToolCallMessage) && assistantToolCallMessage.toolCalls,
      ).toHaveLength(1);

      // The tool result should be properly linked
      const toolResultMessage = messages[2];
      expect(toolResultMessage.role).toBe('tool');
      expect(isToolMessage(toolResultMessage) && toolResultMessage.toolCallId).toBe(
        'call_calculate_abc123',
      );
