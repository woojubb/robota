...
export async function initializePluginHelper(
  ...
  lifecycleEvents: IPluginLifecycleEvents,
  logger: ILogger,
): Promise<void> {
  ...
  if (status.initialized) {
    logger.debug(`Plugin "${pluginName}" already initialized`);
    return;
  }
  try {
    ...
    logger.error(`Failed to initialize plugin "${pluginName}"`, {
...
export async function destroyPluginHelper(
  ...
  const status = plugin.getStatus();
  if (!status.initialized) {
    logger.debug(`Plugin "${pluginName}" not initialized, skipping destroy`);
    return;
  }
  try {
    ...
    logger.error(`Failed to destroy plugin "${pluginName}"`, {
      error: error instanceof Error ? error.message : String(error),
    });
    const normalized = error instanceof Error ? error : new Error(String(error));
    if (lifecycleEvents.onError) await lifecycleEvents.onError(plugin, normalized);
...
export async function validateDependenciesHelper(
  ...
  for (const dep of dependencies) {
    if (dep.required && !plugins.has(dep.name))
    ...
    if (dep.minVersion) {
      ...
      if (depPlugin && depPlugin.version) {
        if (compareSemver(depPlugin.version, dep.minVersion) < 0)
          throw new ConfigurationError(
            `Dependency "${dep.name}" version ${depPlugin.version} is less than required ${dep.minVersion}`,
          );
      }
    }
  }
...
export function resolveDependencyOrderHelper(
  pluginNames: string[],
  pluginOptions: Map<string, IPluginRegistrationOptions>,
): string[] {
  ...
  const visit = (name: string) => {
    ...
    if (opts?.dependencies) {
      for (const dep of opts.dependencies) {
        if (pluginNames.includes(dep.name)) visit(dep.name);
      }
    }
  ...
  const sorted = pluginNames.sort((a, b) => {
    const pa = pluginOptions.get(a)?.priority || 0;
    const pb = pluginOptions.get(b)?.priority || 0;
    return pb - pa;
  });
