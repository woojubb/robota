...
/**
 * The span ID a provider call is exported under, derived from the call ID core mints before the
 * call. One derivation for both the exporter and the outgoing header, so the span a vendor sees as
 * its parent is the span the trace contains.
 */
export function providerCallSpanId(callId: string): string {
  return callId.replaceAll('-', '').slice(0, 16);
}
...
export function outboundTraceContextFor(
  ...
  const traceparent = buildTraceparent(traceContext.traceId, providerCallSpanId(callId));
