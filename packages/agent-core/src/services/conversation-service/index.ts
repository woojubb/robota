...
export class ConversationService implements IConversationService {
  ...
  validateContext(context: IConversationContext): { isValid: boolean; errors: string[] } {
    ...
    if (
      ...
      (typeof context.maxTokens !== 'number' || context.maxTokens <= 0)
    )
  ...
  createUserMessage(
    ...
    return createUserMessageStatic(content, metadata);
  }
  createAssistantMessage(
    response: IConversationResponse,
    metadata?: Record<string, string | number | boolean>,
  ): IAssistantMessage {
    return createAssistantMessageStatic(response, metadata);
  }
  createSystemMessage(
    content: string,
    metadata?: Record<string, string | number | boolean>,
  ): ISystemMessage {
    return createSystemMessageStatic(content, metadata);
  }
  createToolMessage(
    toolCallId: string,
    ...
    return createToolMessageStatic(toolCallId, result, metadata);
  }
