 ...
 *
 * Usage: node packages/agent-core/examples/hook-timeout-demo.mjs
 */

import { readFileSync } from 'node:fs';
import { runHooks } from '../dist/node/index.js';

const EXECUTOR_SRC = new URL('../src/hooks/executors/command-executor.ts', import.meta.url);
...
check('blocked === false (timeout diagnostic is not an explicit deny)', s1result.blocked === false);
check(
  'timeout diagnostic is present',
  s1result.errors?.some((error) => error.kind === 'timeout'),
);
