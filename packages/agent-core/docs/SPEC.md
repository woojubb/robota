...
| `IProviderDefinition`                 | `interfaces/provider-definition.ts` | Provider assembly contract. Provider packages expose definitions with display metadata, compatibility aliases, defaults, setup prompts, credential requirements, model catalog fallback metadata, optional provider-owned catalog refresh hooks, probe hooks, and `createProvider()` factories. |
...
| `IProviderConfig`                     | `interfaces/provider-definition.ts` | Normalized provider configuration consumed by provider definitions, including `apiKey`, optional `authToken`, and a provider-owned `options` bag that generic layers pass through without interpreting.                                                                                         |
...
| `IProviderDefinition`                   | interface      | Provider assembly definition, including optional setup display metadata, compatibility aliases, defaults, credential requirements, model catalog fallback metadata, provider-owned refresh hooks, and provider-owned options |
...
| `IProviderCredentialRequirement`        | interface      | Provider-owned credential requirement, such as accepting either `apiKey` or `authToken`                                                                                                                                      |
