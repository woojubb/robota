...
- **Resume semantics**: on resume, the live `config.systemMessage` is authoritative and replaces any persisted system prompt in the restored history (a staleness refresh — the rebuilt prompt reflects the current cwd/AGENTS.md/CLAUDE.md and tool inventory). A restored system message survives only as a fallback when there is no live `config.systemMessage` at all. The restored conversation's user/assistant/tool messages are always preserved; restore keys off the presence of conversation content, not the system head (so a system prompt applied before the first turn does not block restore).
