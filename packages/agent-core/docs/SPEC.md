...
One evaluation order serves every caller. The interactive session, background tasks and subagents differ only in the context they pass, never in the order, because two orders let the same call be decided differently depending on who made it; a background policy therefore contributes a ceiling, an ask-everything flag and the task's own lists, and a turn driven by another agent session contributes where that peer runs, not a resolver of its own. A peer never has more authority than the operator: another host uses no tool, the same host may use a read whose every named location resolves inside the workspace and is no credential, and a change is refused unless the operator enabled peer changes. The reply to the peer exists only in a peer turn and asks whenever what the model sees holds a tool's output — a tool result from this turn or any earlier one, the operator's included, or a compaction summary that may restate one — because what it carries may then come from the machine rather than from the model; a reply in a conversation with no tool output goes out directly, and an earlier reply's own result, which says only whether it was delivered, is not tool output. Tool output means the results of tool calls: what the operator placed in a prompt, attached files included, is the operator's own input and is not counted. A provider's own hosted tools run at the vendor and never reach this evaluator, so a peer turn's requests carry none.

Invariants the order keeps:

- **Nothing later softens a denial.** A matching deny rule, a caller's ceiling and what a peer's origin does not allow outrank every ask, every allow and every mode, bypass included. A call outside the ceiling is refused rather than offered to a person.
- **Some calls always reach a person, every time.** A deny or ask rule that cannot be evaluated, an explicit ask rule, a removal whose loss would be catastrophic, and a write into a location that widens what later turns, child sessions or the user's shell will trust all ask in every mode, as does every ask in a peer turn — and a consent remembered from an earlier call never answers them, because a remembered scope is wide enough to cover the next such call too. Bypass is not exempt, unlike Claude Code: here bypass also reaches unattended subagents and SDK runs, where a silent write to configuration would widen trust for later turns and child sessions. Only what a tool's declared argument shows can be judged this way; a shell command writing into those locations is the containment seam's concern, not the gate's.
