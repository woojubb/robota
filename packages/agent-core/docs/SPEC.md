...
| `IProviderDefinition`                   | interface      | Provider assembly definition, including optional setup display metadata, compatibility aliases, defaults, model catalog fallback metadata, provider-owned refresh hooks, and provider-owned options |
