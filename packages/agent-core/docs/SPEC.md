...
One evaluation order serves every caller. The interactive session, background tasks and subagents differ only in the context they pass, never in the order, because two orders let the same call be decided differently depending on who made it; a background policy therefore contributes a ceiling, an ask-everything flag and the task's own lists, and a turn driven by another agent session contributes where that peer runs, not a resolver of its own. A peer never has more authority than the operator: another host uses no tool, the same host may use a read whose every named location resolves inside the workspace and is no credential, and a change is refused unless the operator enabled peer changes. The reply to the peer exists only in a peer turn and asks once the turn used any tool, because what it carries may then come from the machine rather than from the model; a reply with no tool use goes out directly, and it can carry whatever the conversation already holds. A provider's own hosted tools run at the vendor and never reach this evaluator, so a peer turn's requests carry none.
