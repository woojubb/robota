...
| `command` | today's glob (`*` any run) over ONE command                                                   | as written; then, when the pattern carries a wildcard, a separator (`;` `&` `\|` `&&` `\|\|` newline) outside quotes or a substitution (`$(` backtick `<(` `>(`) outside single quotes is NO match — `Bash(git *)` matches `git status` and not `git status; rm -rf /` (issue #2427); `2>&1` and `&>` are redirections; a pattern without a wildcard is the whole line, verbatim | never                                                                                                                                                                                                                                                                                                                                      |
