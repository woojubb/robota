...
Beyond the one primary argument, a deny or ask rule may name any other top-level parameter (`Tool(name:value)`). Whether `name:` is a parameter is decided by the tool's declared parameter names, not by the text's shape, so a URL or command pattern containing a colon keeps its meaning; an omitted parameter never matches, and a value that is not a scalar is unevaluable. Allow rules may not name parameters, and may glob the tool name only after a literal server prefix: in an allow list either would grant more than the operator could see. A rule naming the primary field is reported and asks on every call rather than being silently ignored, because an ignored deny fails open. A deny that names a tool outright — bare, `(*)`, or a name glob — withholds it from the model instead of offering it and refusing every call.
