...
A pattern or argument that cannot be evaluated in its declared kind is a distinct outcome from "did not match," and `evaluatePermission` routes an unevaluable **deny** or **ask** pattern to an ask rather than falling through to the allow list — the same fail-closed principle as the command-kind asymmetry above, generalized to every kind. Ask patterns are read in the deny direction for the same reason: both mean stop and check. For the same reason a rule naming a tool's primary field as a parameter asks rather than being ignored, since an ignored deny fails open. Whether `name:` in a pattern names a parameter is decided by the tool's declared parameters, not by the text's shape, so a URL or command pattern containing a colon keeps its meaning. Allow rules are held to a narrower grammar than deny and ask rules — no parameter rules and no tool-name glob without a literal server prefix — because in an allow list either form would grant tools or calls the operator never saw. A tool denied outright is withheld from the model rather than offered and refused on every call.
...
One evaluation order serves every caller. The interactive session, background tasks and subagents differ only in the context they pass, never in the order, because two orders let the same call be decided differently depending on who made it; a background policy therefore contributes a ceiling, an ask-everything flag and the task's own lists. A message from another agent session is text from an untrusted third party that carries no authority, so the turn it starts contributes nothing but whether the reply to that peer exists and whether a tool that acts only for the operator is withheld: everything the model does in it is decided by the same rules, mode and remembered consent as the session's own work, and nothing about the peer — where it runs, what it asked for — widens or narrows that. The reply exists only in such a turn, and there it is decided like any call that sends something off this machine — rules, mode and remembered consent apply to it as to the operator's own calls — never by inferring from the conversation whether what it carries came from the machine, because no reading of the history tells what the text contains; when it asks, the operator sees the full text and the peer it goes to. A provider's own hosted tools run at the vendor and never reach this evaluator; the owner's turns carry them because the owner enabled them, but in a turn a peer's message started no step of the ordinary permissions could decide them, so its requests carry none.
...
- **Some calls always reach a person, every time.** A deny or ask rule that cannot be evaluated, an explicit ask rule, a removal whose loss would be catastrophic, and a write into a location that widens what later turns, child sessions or the user's shell will trust all ask in every mode — and a consent remembered from an earlier call never answers them, because a remembered scope is wide enough to cover the next such call too. Bypass is not exempt, unlike Claude Code: here bypass also reaches unattended subagents and SDK runs, where a silent write to configuration would widen trust for later turns and child sessions. Only what a tool's declared argument shows can be judged this way; a shell command writing into those locations is the containment seam's concern, not the gate's.
...
A run carries W3C trace context only when its host supplies it; this package never adopts ambient trace state. Every call actually handed to a provider — the forced summary included, cache hits and refused preflights excluded — carries a `traceparent` whose span is derived from that call's own ID by the same function the exporter uses, so the parent a vendor sees is the span the trace contains. It is resolved at invocation, after the request was announced, so no replay event contains it. A provider that does not affirmatively declare it can propagate receives nothing and, when the host listed origins, the host is told which provider, by ID only; a run that hands its trace only to subprocesses has no provider to report. Every tool body is given its own minted ID, because a vendor's tool call ID is neither unique nor hex; in a run with trace context the body receives the `traceparent` whose span is derived from that ID, so a tool that forwards it names the span exported for that body. A tool body needs no capability declaration, since a tool that makes no outbound request simply ignores it. An adapter or tool sends the header only when its effective origin exactly equals a host-listed origin, because the header discloses trace identifiers to whoever receives it. Child processes are a separate grant, derived apart from the origins so either can be given alone: a class the host enables receives `TRACEPARENT` in a fresh copy of the environment — never by modifying the process's own, which every other child would inherit — naming the body's span for the shell and the prompt root for command hooks, with the ambient `TRACESTATE` dropped because it belongs to another parent. A command hook is handed that value apart from its input, so it never appears in the stdin JSON, in an HTTP hook body, or to any other hook type, and a `TRACEPARENT` the hook's own configuration sets wins over it.
