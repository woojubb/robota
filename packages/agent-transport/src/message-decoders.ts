 ...
 *
 * `wire-messages.ts` owns the TypeScript unions; this file owns turning `JSON.parse` output into one
...
export const SERVER_MESSAGE_SHAPES: Readonly<Record<TServerMessage['type'], TVariantShape>> = {
  ...
  personal_usage_report_error: {
    requestId: isNonEmptyString,
    code: oneOf(['not_available', 'report_failed']),
    message: isString,
  },
