 ...
 * `createSessionMessageHandler`), `seq` does not reset when a reconnect builds a new channel, and gap output is not lost.
 * The WS localhost path does NOT use this bridge — it keeps `createSessionMessageHandler` unchanged.
 */
...
import { handleClientMessage, parseClientMessage } from './session-message-handler.js';
