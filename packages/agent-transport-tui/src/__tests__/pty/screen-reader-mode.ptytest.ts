...
describe('CLI-2004 screen-reader mode through the real binary', () => {
  ...
  it('TC-11: --screen-reader emits no box-drawing chrome, role labels, and OSC 133 marks', async () => {
    session = spawnTui({
      projectDir,
    ...
    await session.waitFor(/REPLAYED_ANSWER_42/, 30_000);
    await session.waitFor(/Type a message or \/help/, 30_000);
  ...
  it('TC-11: the SAME fixture without the flag still renders the bordered UI (default-off)', async () => {
    session = spawnTui({
      projectDir,
      homeDir: join(projectDir, 'home'),
  ...
  it('TC-12: ROBOTA_SCREEN_READER=1 with no flag announces the env channel first', async () => {
