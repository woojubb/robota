...
rather than looping. Four timeouts — `startupMs`, `perCallMs`, `globalDefaultMs`, `idleMs` — are
distinct typed settings. A `list_changed` notification marks that domain stale and refreshes it over
...
| `MCPConnectionSupervisor`          | class     | Open / reuse / close, bounded retry, `list_changed` refresh without reconnecting, last-known-good with identity, four typed timeouts, `shutdown`                             |
...
| `IMCPTimeouts`                     | interface | `startupMs`, `perCallMs`, `globalDefaultMs`, `idleMs` — four independent settings                                                                                            |
...
- `IMCPConnectionSupervisorOptions.timeouts` / `.backoff` / `.clock` / `.onStateChange` /
  `.discovery.maxPages` — four independent timeouts, the bounded backoff policy, an injectable clock,
