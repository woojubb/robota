...
function StatusPanel({ children }: { children: React.ReactNode }): React.ReactElement {
  return (
    <div className="flex min-h-64 items-center justify-center rounded-2xl bg-card px-8 text-center text-[15px] text-muted-foreground">
      {children}
    </div>
  );
}
...
export function UsageHeader({
  ...
  return (
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div>
        <div className="text-[13px] font-medium text-accent">Local analytics</div>
        <h1 className="mt-1 text-[28px] font-semibold tracking-[-0.02em]">Personal usage</h1>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className="rounded-lg bg-raised px-3 py-2 text-[13px] font-medium text-foreground hover:bg-hover"
        >
...
export function CurrentSessionPanel({
  ...
  if (state.currentSessionUsageStatus === 'loading') {
    content = <p className="mt-3 text-[14px] text-muted-foreground">Loading trace…</p>;
          ...
          return (
            ...
            >
              <div className="relative flex h-32 w-full items-end">
                <div
                  className="mx-auto w-full max-w-10 rounded-md bg-accent/75 transition-colors group-hover:bg-accent"
                  style={{ height: `${height}%` }}
                  title={`${day.date}: ${number.format(day.totals.totalTokens)} tokens`}
                />
              </div>
              <span className="truncate text-[12px] tabular-nums text-subtle">
...
function DailyChartHeader({ report }: { report: TPersonalUsageReport }): React.ReactElement {
  return (
    <div className="flex items-center justify-between gap-3">
      <div>
        <h2 className="text-[15px] font-semibold">Daily usage</h2>
