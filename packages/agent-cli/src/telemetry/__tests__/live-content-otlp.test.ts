...
describe('live content OTLP projection', () => {
  it('exports redacted text as a captured record on the prompt root', () => {
    const records = projectLiveContentLogs(contentBatch([
  ...
  it('keeps the post-redaction bodies of one batch within the budget, counting what it dropped', () => {
    const big = 'x'.repeat(16384);
    const items = Array.from({ length: 70 }, () => item(big));
    const records = projectLiveContentLogs(contentBatch(items), new Date(ENDED_AT), resource, policy,
      (text) => ({ text, truncated: false }));
    const captured = records.filter((record) => record.eventName === 'robota.content.captured');
    const bytes = captured.reduce((sum, record) => sum + Buffer.byteLength(String(record.body)), 0);
    expect(bytes).toBeLessThanOrEqual(LIVE_CONTENT_BATCH_BODY_BUDGET);
    expect(captured).toHaveLength(48);
    const omitted = records.at(-1)!;
    expect(omitted.eventName).toBe('robota.content.omitted');
    expect(omitted.body).toBeUndefined();
    expect(omitted.attributes).toEqual({ 'robota.telemetry.omitted_content_items': 22 });
  });

  it('never exports more than 64 items, even small ones', () => {
    const records = projectLiveContentLogs(contentBatch(Array.from({ length: 70 }, () => item('a'))),
      new Date(ENDED_AT), resource, policy, (text) => ({ text, truncated: false }));
    expect(records.filter((record) => record.eventName === 'robota.content.captured')).toHaveLength(64);
    expect(records.at(-1)!.attributes).toEqual({ 'robota.telemetry.omitted_content_items': 6 });
  });

  it('drops only the item whose redaction threw', () => {
    const records = projectLiveContentLogs(contentBatch([item('boom'), item('fine')]),
      new Date(ENDED_AT), resource, policy, (text) => {
        if (text === 'boom') throw new Error('redactor bug');
        return { text, truncated: false };
      });
    expect(records.map((record) => [record.eventName, record.body])).toEqual([
  ...
  it('exports only the omission count when the batch has no redactor', () => {
    const records = projectLiveContentLogs(contentBatch([item('secret text'), item('more')]),
      new Date(ENDED_AT), resource, policy, undefined);
    expect(records).toHaveLength(1);
    expect(records[0]!.eventName).toBe('robota.content.omitted');
    expect(JSON.stringify(records)).not.toContain('secret text');
  });
});
...
describe('live content OTLP delivery', () => {
  ...
  it('bounds its own queue independently of the content-free logs', async () => {
    ...
    // The worker takes the first batch at once, so eight more fill the queue.
    for (let index = 0; index < 9; index += 1) content.enqueue(contentBatch([item('x')]));
