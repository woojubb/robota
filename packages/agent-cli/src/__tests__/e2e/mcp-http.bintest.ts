...
describe('robota mcp serve loopback HTTP binary', () => {
  it('issues a private token, serves a real client, and cleans up on SIGTERM', async () => {
    ...
    const child = spawn(
      process.execPath,
      [ROBOTA_BIN, 'mcp', 'serve', '--http-token-file', tokenFile,
        '--allowed-tools', 'Read', '--session-log', FIXTURE, '--no-session-persistence'],
      { cwd, env: { HOME: home, PATH: process.env['PATH'] ?? '' }, stdio: ['pipe', 'pipe', 'pipe'] },
    );
    let diagnostics = '';
    let stdout = '';
    ...
    try {
      await vi.waitFor(() => expect(diagnostics).toMatch(/MCP HTTP listening at http:\/\/127\.0\.0\.1:\d+\/mcp/), { timeout: 15000 });
      ...
      const result = await client.callTool({
        name: 'Read', arguments: { filePath: join(cwd, 'message.txt') },
      });
