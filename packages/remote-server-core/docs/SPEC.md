...
Owns reusable Remote API server route registration and OpenAPI documentation for Robota. Provides a single `registerRemoteServerRoutes` function that mounts `RemoteServer` from `@robota-sdk/remote/server` onto an Express application, configures Swagger UI, and returns a runtime status handle.
