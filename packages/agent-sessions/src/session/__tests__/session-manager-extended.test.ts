...
vi.mock('@robota-sdk/agent-core', async (importOriginal) => {
  const original = await importOriginal<typeof import('@robota-sdk/agent-core')>();
  ...
  class MockAgentFactory {
    async createAgent(AgentClass: unknown, _config: Partial<IAgentConfig>): Promise<unknown> {
...
describe('SessionManager - extended coverage', () => {
  ...
  describe('createChat', () => {
    ...
    it('should throw when session not found', async () => {
      await expect(manager.createChat('nonexistent', createDefaultChatOptions())).rejects.toThrow(
  ...
  describe('switchChat', () => {
    ...
    it('should activate target chat and set activeChatId on session', async () => {
      const chatId1 = await manager.createChat(
        sessionId,
        createDefaultChatOptions({ name: 'Chat 1' }),
      );
      const chatId2 = await manager.createChat(
        sessionId,
        createDefaultChatOptions({ name: 'Chat 2' }),
      );
    ...
    it('should update session lastUsedAt', async () => {
      ...
      expect(manager.getSession(sessionId)!.lastUsedAt.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
    });
  });
