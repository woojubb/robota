...
- **DAG definition authoring or validation** -- owned by `dag-core`.
...
| `RunOrchestratorService.createRun`       | Method    | `(input: IStartRunInput) => Promise<TResult<ICreateRunResult, IDagError>>`                                                       |
