      ...
      try {
        ...
        if (!provider || typeof provider !== 'string') {
          res
            ...
            .json({ error: 'Missing or invalid field: provider must be a string' });
          return;
        }
        if (!model || typeof model !== 'string') {
          res
            .status(HTTP_BAD_REQUEST)
            .json({ error: 'Missing or invalid field: model must be a string' });
          return;
        }
        if (!messages || !Array.isArray(messages)) {
          res
            .status(HTTP_BAD_REQUEST)
            .json({ error: 'Missing or invalid field: messages must be an array' });
          return;
        }
...
/** Configuration options for RemoteServer initialization. */
