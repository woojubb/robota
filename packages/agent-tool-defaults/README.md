...
`@robota-sdk/agent-tools`. It implements no tool itself. Every file tool it returns is contained by
the `cwd` you pass, and adapter-gated tools appear only when you supply their adapter. Import it at
a composition root (the place that assembles an agent), not from library code.
...
`createDefaultTools()` is synchronous and performs no I/O. It returns a new `FunctionTool[]` on every
call, and the same options always produce the same set of tools.
...
sandbox path. When the sandbox shares the host filesystem, file tools stay on the host, contained
by `cwd`.
...
| ------------------ | ------------------- | ----------------------------------------------------------------------------------------------------------- |
| `cwd`              | `string`            | Required. The root every file tool is contained by, and the default working directory for `Shell` / `Bash`. |
