...
- **Plugin common APIs**: `agent-sdk/command-api/plugin/` owns plugin command metadata constants, subcommand projection helpers, `ICommandPluginAdapter`, and the `plugin-tui-requested` effect factory. `/plugin` command behavior lives in `@robota-sdk/agent-command-plugin` and consumes these APIs as an external command module while hosts keep concrete plugin storage/UI wiring.
...
- **Product-composed built-in command modules**: `/plugin` is provided by `@robota-sdk/agent-command-plugin`. It reuses SDK plugin command common APIs, sends host UI opening through `plugin-tui-requested`, and performs install/uninstall/enable/disable/marketplace operations through a host-provided `ICommandPluginAdapter`.
...
- **`/rewind`**: User-invocable product-composed code checkpoint command. `rewind list` lists prompt-turn checkpoints; `rewind restore <checkpoint-id>` and `rewind code <checkpoint-id>` restore files to the selected checkpoint. It is not model-invocable by default.
