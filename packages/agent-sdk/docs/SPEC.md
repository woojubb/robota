...
- **Permission mode common APIs**: `agent-sdk/command-api/permissions/` owns permission-mode constants, descriptor subcommands, validation, and command-facing adapter resolution. `/mode` command behavior lives in `@robota-sdk/agent-command-mode` and consumes these APIs as an external command module.
...
- **SDK-default built-in commands**: `help`, `clear`, `cost`, `permissions`, `memory`, `rewind`, `resume`, `background`, `rename`, `reset`
