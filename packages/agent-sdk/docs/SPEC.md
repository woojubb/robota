...
- **Session common APIs**: `agent-sdk/command-api/session/` owns command-facing session-history helpers, session-name parsing, and effect factories for host-rendered history/name/picker state. `/clear`, `/rename`, and `/resume` command behavior lives in `@robota-sdk/agent-command-session` and consumes these APIs as an external command module.
...
- **SDK-default built-in commands**: `help`, `cost`, `memory`, `rewind`, `background`, `reset`
...
- **Product-composed built-in command modules**: `/clear`, `/rename`, and `/resume` are provided by `@robota-sdk/agent-command-session`. `/clear` reuses SDK session command common APIs to clear SDK session history and emits `conversation-history-cleared` so hosts clear rendered history through their own UI state. `/rename` reuses SDK session command common APIs to normalize the requested name and emits `session-renamed` so hosts update title/status/persistence through their own adapters. `/resume` emits `session-picker-requested` so hosts display saved-session picker UI through their own adapters.
