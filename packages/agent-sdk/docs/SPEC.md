...
- **Context/compact common APIs**: `agent-sdk/command-api/context/` owns command-facing context-state reads, automatic compact policy reads, and manual compact host-facade helpers. `/compact` command behavior lives in `@robota-sdk/agent-command-compact` and consumes these APIs as an external command module.
...
- **SDK-default built-in commands**: `help`, `clear`, `mode`, `model`, `language`, `cost`, `context`, `permissions`, `memory`, `rewind`, `resume`, `rename`, `reset`
- **Product-composed built-in command modules**: `/compact` is provided by `@robota-sdk/agent-command-compact` and declares blocking lifecycle metadata through the same `ISystemCommand` contract.
