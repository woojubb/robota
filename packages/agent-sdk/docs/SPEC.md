...
- **Session common APIs**: `agent-sdk/command-api/session/` owns command-facing session-history helpers and effect contracts for host-rendered history state. `/clear` command behavior lives in `@robota-sdk/agent-command-session` and consumes these APIs as an external command module.
...
- **SDK-default built-in commands**: `help`, `cost`, `memory`, `rewind`, `resume`, `background`, `rename`, `reset`
...
- **Product-composed built-in command modules**: `/clear` is provided by `@robota-sdk/agent-command-session`, reuses SDK session command common APIs to clear SDK session history, and emits `conversation-history-cleared` so hosts clear rendered history through their own UI state.
