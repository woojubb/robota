...
import type { IDagDefinition, TObjectInfo } from '@robota-sdk/dag-core';
...
describe('Dag design flow E2E', () => {
  ...
  it('returns validation errors for invalid definition in validate flow', async () => {
    ...
    expect(validated.errors.some((error) => error.code === 'DAG_VALIDATION_EMPTY_NODES')).toBe(
      true,
    );
  ...
  it('blocks validate for invalid binding without fallback', async () => {
    ...
    expect(
      validated.errors.some((error) => error.code === 'DAG_VALIDATION_BINDING_INPUT_NOT_FOUND'),
    ).toBe(true);
  });
  ...
  it('lists node catalog from injected service', async () => {
    ...
    const objectInfo: TObjectInfo = {
      'image-source': {
        ...
        output_name: ['image'],
        output_node: false,
        description: 'Image source node',
      },
    };
    const nodeCatalogService: INodeCatalogService = {
      hasNodeType: async (nodeType: string) => ({
        ok: true,
        value: Object.hasOwn(objectInfo, nodeType),
      }),
    ...
    expect(listed.data['image-source']?.display_name).toBe('Image Source');
    expect(listed.data['image-source']?.category).toBe('Test');
  });

  it('fails validate when node type is not in node catalog', async () => {
    const storage = new InMemoryStoragePort();
    const nodeCatalogService: INodeCatalogService = {
      hasNodeType: async (nodeType: string) => ({ ok: true, value: nodeType === 'image-source' }),
    ...
    expect(
      validated.errors.some((error) => error.code === 'DAG_VALIDATION_NODE_TYPE_NOT_REGISTERED'),
    ).toBe(true);
  });
});
