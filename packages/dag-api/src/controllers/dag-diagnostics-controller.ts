...
export class DagDiagnosticsController {
  public constructor(
    ...
    private readonly dlqReinject: IDiagnosticsDeadLetterReinjectPort,
   ...
   */
  public async rerun(request: IRerunRequest): Promise<
    TDiagnosticsApiResponse<{
  ...
  public async reinjectDeadLetter(
    ...
    const reinjected = await this.dlqReinject.reinjectOnce(
      request.workerId,
      request.visibilityTimeoutMs,
    );
    if (!reinjected.ok) {
      const problem = toRuntimeProblemDetails(
        reinjected.error,
