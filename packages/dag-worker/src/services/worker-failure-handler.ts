...
export async function handleRetry(
  ...
  try {
    ...
    return failAfterAck(
      queue,
      message.messageId,
      buildDispatchError('DAG_DISPATCH_ENQUEUE_RETRY_FAILED', 'Failed to enqueue retry message', {
        taskRunId,
        attempt: nextAttempt,
...
export async function failAfterAck(
  queue: IQueuePort,
  messageId: string,
  error: IDagError,
): Promise<TResult<IWorkerLoopResult, IDagError>> {
...
export async function successAfterAck(
  queue: IQueuePort,
  messageId: string,
  taskRunId: string,
  retried: boolean,
