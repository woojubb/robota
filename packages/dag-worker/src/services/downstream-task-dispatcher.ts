...
export async function dispatchDownstreamReadyTasks(
  ...
  const downstreamNodes = definition.nodes.filter((node) =>
  ...
  for (const downstreamNode of downstreamNodes) {
    const result = await dispatchSingleDownstreamNode(
      dagRun,
      definition,
      downstreamNode,
      allTaskRuns,
...
async function dispatchSingleDownstreamNode(
  ...
  const ready = downstreamNode.dependsOn.every((upstreamNodeId) =>
    allTaskRuns.some(
      (taskRun) => taskRun.nodeId === upstreamNodeId && taskRun.status === 'success',
    ),
  );
  if (!ready) {
    return { ok: true, value: undefined };
