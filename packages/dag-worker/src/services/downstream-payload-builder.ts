...
function resolveUpstreamOutput(
  ...
  const upstreamTask = taskRuns.find(
    (taskRun) => taskRun.nodeId === fromNodeId && taskRun.status === 'success',
  );
...
function applySingleBinding(
  ...
  const directInputPort = (downstreamNode.inputs ?? []).find(
    (port) => port.key === binding.inputKey,
  );
  ...
  if (listHandle) {
    return applyIndexedListBinding(
      payload,
      listHandle,
      outputValue,
      binding.inputKey,
...
function compactListPorts(payload: TPortPayload, inputs: IPortDefinition[]): void {
