...
address the pair, the two directions never overwrite each other, and a record of one purpose never opens as another. Every value that anyone but the user's own relay can see rotates by
