...
`skills` follows the same command-module layering as other built-in commands. Models invoke it through the SDK-projected `robota_command_skills` tool with skill arguments in `args`. When this model-invocable command module is composed, skill metadata is listed in the SDK-composed system prompt `Skills` section; full skill content is loaded only after SDK activation.
