...
/**
 * Await an OpenAI `create()` call, reading the server's own request ID off `.withResponse()` when
 * the client exposes it. A test double or older client that resolves to a plain value has no
 * `withResponse`, so this falls back to the awaited value with no ID — never a fabricated one.
 */
...
export function readOpenAIRequestId(response: unknown): string | undefined {
