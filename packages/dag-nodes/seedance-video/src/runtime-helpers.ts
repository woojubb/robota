...
import type { IVideoJobSnapshot, TImageInputSource } from '@robota-sdk/agent-core';
 ...
 */
export function toOutputVideo(
  output: IVideoJobSnapshot['output'],
  ...
  const mimeType =
    typeof output.mimeType === 'string' && output.mimeType.trim().length > 0
      ? output.mimeType
      : 'video/mp4';
