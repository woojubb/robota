...
vi.mock('./runtime-helpers.js', () => ({
    ...
    resolveImageInputSource: vi.fn(async (image: { uri?: string }) => ({
        ok: true,
        value: { kind: 'uri', uri: image.uri ?? 'https://img.example.com/img.png', mimeType: 'image/png' }
    })),
    toOutputVideo: vi.fn((output: Record<string, unknown>) => ({
        ok: true,
        value: { kind: 'video', mimeType: 'video/mp4', uri: output?.uri ?? 'https://cdn.example.com/video.mp4' }
    }))
}));

function createBaseRequest(overrides: Partial<ISeedanceGenerateVideoRequest> = {}): ISeedanceGenerateVideoRequest {
    return {
...
describe('SeedanceVideoRuntime', () => {
    ...
    describe('image input resolution', () => {
            ...
            expect(mockCreateVideo).toHaveBeenCalledWith(
                expect.objectContaining({
                    inputImages: expect.arrayContaining([
                        expect.objectContaining({ kind: 'uri' })
                    ])
                })
            );
        });
        ...
        it('returns error when image resolution fails', async () => {
            ...
            const images = [
                { kind: 'image' as const, mimeType: 'image/png', uri: 'asset://missing' }
            ];
            const result = await runtime.generateVideo(createBaseRequest({ inputImages: images }));
            expect(result.ok).toBe(false);
            if (!result.ok) {
    ...
    describe('polling loop', () => {
        ...
        it('polls multiple times before success', async () => {
            ...
            const result = await runtime.generateVideo(createBaseRequest({ pollIntervalMs: 1, pollTimeoutMs: 5000 }));
            expect(result.ok).toBe(true);
        ...
        it('returns retryable error on polling timeout', async () => {
            ...
            const result = await runtime.generateVideo(createBaseRequest({ pollIntervalMs: 1, pollTimeoutMs: 1 }));
            expect(result.ok).toBe(false);
            if (!result.ok) {
