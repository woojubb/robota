...
import type { IVideoGenerationProvider, TImageInputSource } from '@robota-sdk/agent-core';
import {
  resolveImageInputSource,
...
export class SeedanceVideoRuntime {
  ...
  private resolveConfig(): {
    ...
    const allowedModelsRaw =
      this.explicitAllowedModels ?? parseCsv(process.env.DAG_SEEDANCE_ALLOWED_MODELS);
    const defaultModel =
      typeof defaultModelRaw === 'string' && defaultModelRaw.trim().length > 0
        ? defaultModelRaw.trim()
        : DEFAULT_SEEDANCE_MODEL;
    const allowedModels =
      Array.isArray(allowedModelsRaw) && allowedModelsRaw.length > 0
        ? allowedModelsRaw
        : [defaultModel];
  ...
  public async generateVideo(
    request: ISeedanceGenerateVideoRequest,
    ...
    const selectedModel =
      request.model.trim().length > 0 ? request.model.trim() : config.defaultModel;
    ...
    for (;;) {
      ...
      if (snapshot.status === 'failed') {
        const failedMessage =
          snapshot.error?.message ?? 'Seedance job failed without explicit error message';
        return this.mapTaskError('DAG_TASK_EXECUTION_SEEDANCE_JOB_FAILED', failedMessage, false, {
