 ...
 */
export class ImageSourceNodeDefinition extends AbstractNodeDefinition<
  typeof ImageSourceConfigSchema
> {
  ...
  protected override async executeWithConfig(
    ...
    const mimeTypeValue =
      typeof config.mimeType === 'string' ? config.mimeType : reference.mediaType();
    const mimeType =
      typeof mimeTypeValue === 'string' && mimeTypeValue.trim().length > 0
        ? mimeTypeValue
        : 'image/png';
    ...
    return {
      ...
      value: {
        ...
        _agentSummary: `Image source resolved. Type: ${mimeType}.`,
      },
    };
  }
}
