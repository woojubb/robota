...
const LlmTextAnthropicConfigSchema = z.object({
  model: z.string().default(DEFAULT_ANTHROPIC_LLM_MODEL),
  temperature: z.number().default(DEFAULT_TEMPERATURE),
  maxTokens: z.number().int().positive().optional(),
  baseCredits: z.number().default(0),
});

export interface ILlmTextAnthropicNodeDefinitionOptions {
  defaultModel?: string;
  allowedModels?: string[];
}

export class LlmTextAnthropicNodeDefinition extends AbstractNodeDefinition<
  typeof LlmTextAnthropicConfigSchema
> {
  public readonly nodeType = 'llm-text-anthropic';
  public readonly displayName = 'LLM Text Anthropic';
  ...
  public readonly configSchemaDefinition = LlmTextAnthropicConfigSchema;

  private readonly apiKeyEnvName = 'ANTHROPIC_API_KEY';
  private readonly defaultModel: string;
  private readonly allowedModels: string[];

  public constructor(options?: ILlmTextAnthropicNodeDefinitionOptions) {
    super();
    this.defaultModel =
  ...
  private resolveProvider(): AnthropicProvider | undefined {
    ...
    if (this.allowedModels.length > 0 && !this.allowedModels.includes(selectedModel)) {
      return {
        ...
        error: buildValidationError(
          ...
          'Selected model is not allowed for llm-text-anthropic node',
          { model: selectedModel },
        ),
      };
    }
  ...
  protected override async validateInputWithConfig(
    ...
    config: z.output<typeof LlmTextAnthropicConfigSchema>,
  ...
  protected override async executeWithConfig(
    ...
    if (!provider) {
      return {
        ok: false,
        error: buildValidationError(
          'DAG_VALIDATION_ANTHROPIC_API_KEY_REQUIRED',
          [
          ...
          { action: 'set_env_var', suggestion: 'export ANTHROPIC_API_KEY=sk-ant-...' },
          {
            ...
            retryable: false,
            action: 'SET_ENV_VAR',
            actionDetail: 'Add ANTHROPIC_API_KEY to .env and reload the process',
          },
        ),
      };
    }
    ...
    const agent = new Robota({
      name: 'DagLlmTextAnthropicNodeAgent',
    ...
    try {
      ...
      return {
        ...
        error: buildTaskExecutionError(
          ...
          retryable,
          { provider: 'anthropic', model: modelResult.value, errorCode: code },
        ),
      };
    }
  }
