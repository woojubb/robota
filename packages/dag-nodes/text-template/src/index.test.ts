...
describe('TextTemplateNodeDefinition', () => {
  ...
  it('substitutes %s with input text', async () => {
    const node = new TextTemplateNodeDefinition();
    const result = await node.taskHandler.execute({ text: 'world' }, createContext('Hello, %s!'));
  ...
  it('uses default template when none specified', async () => {
    const node = new TextTemplateNodeDefinition();
    const result = await node.taskHandler.execute({ text: 'passthrough' }, createContext('%s'));
    expect(result.ok).toBe(true);
    if (result.ok) {
  ...
  it('substitutes {{text}} handlebars syntax with input text', async () => {
    const node = new TextTemplateNodeDefinition();
    const result = await node.taskHandler.execute(
      { text: 'world' },
      createContext('Hello, {{text}}!'),
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.text).toBe('Hello, world!');
  ...
  it('supports both %s and {{text}} in the same template', async () => {
    const node = new TextTemplateNodeDefinition();
    const result = await node.taskHandler.execute(
      { text: 'Bob' },
      createContext('Dear {{text}}, %s here.'),
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.text).toBe('Dear Bob, Bob here.');
    }
  });

  it('%%s escape is preserved when using {{text}} syntax', async () => {
    const node = new TextTemplateNodeDefinition();
    const result = await node.taskHandler.execute(
      { text: 'value' },
      createContext('{{text}} has %%s placeholder'),
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
