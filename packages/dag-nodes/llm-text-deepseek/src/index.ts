...
const COST_PER_TOKEN_USD = 0.0001;
...
const LlmTextDeepSeekConfigSchema = z.object({
  model: z.string().default(DEFAULT_DEEPSEEK_LLM_MODEL),
  temperature: z.number().default(DEFAULT_TEMPERATURE),
  maxTokens: z.number().int().positive().optional(),
  baseCredits: z.number().default(0),
});

export interface ILlmTextDeepSeekNodeDefinitionOptions {
  defaultModel?: string;
  allowedModels?: string[];
}

export class LlmTextDeepSeekNodeDefinition extends AbstractNodeDefinition<
  typeof LlmTextDeepSeekConfigSchema
> {
  public readonly nodeType = 'llm-text-deepseek';
  public readonly displayName = 'LLM Text DeepSeek';
  ...
  public readonly configSchemaDefinition = LlmTextDeepSeekConfigSchema;

  private readonly apiKeyEnvName = 'DEEPSEEK_API_KEY';
  private readonly defaultModel: string;
  private readonly allowedModels: string[];

  public constructor(options?: ILlmTextDeepSeekNodeDefinitionOptions) {
    super();
    this.defaultModel =
  ...
  private resolveProvider(): DeepSeekProvider | undefined {
    const apiKey = process.env[this.apiKeyEnvName];
    if (typeof apiKey === 'string' && apiKey.trim().length > 0) {
      return new DeepSeekProvider({ apiKey: apiKey.trim() });
    }
    return undefined;
  }
    ...
    if (this.allowedModels.length > 0 && !this.allowedModels.includes(selectedModel)) {
      return {
        ...
        error: buildValidationError(
          ...
          'Selected model is not allowed for llm-text-deepseek node',
          { model: selectedModel },
        ),
      };
    }
  ...
  protected override async validateInputWithConfig(
    ...
    config: z.output<typeof LlmTextDeepSeekConfigSchema>,
  ...
  protected override async executeWithConfig(
    ...
    if (!provider) {
      return {
        ok: false,
        error: buildValidationError(
          'DAG_VALIDATION_DEEPSEEK_API_KEY_REQUIRED',
          [
            'DEEPSEEK_API_KEY is not set.',
            "Fix: echo 'DEEPSEEK_API_KEY=sk-...' >> .env",
            'Then: dag run <file> --env-file .env',
            'Get key: https://platform.deepseek.com/',
          ].join('\n'),
          {},
          { action: 'set_env_var', suggestion: 'export DEEPSEEK_API_KEY=sk-...' },
        ),
      };
    }
    ...
    const agent = new Robota({
      name: 'DagLlmTextDeepSeekNodeAgent',
    ...
    try {
      ...
      return {
        ...
        error: buildTaskExecutionError(
          ...
          retryable,
          { provider: 'deepseek', model: modelResult.value, errorCode: code },
        ),
      };
    }
  }
