...
export class InputNodeDefinition extends AbstractNodeDefinition<typeof InputNodeConfigSchema> {
  ...
  protected override async executeWithConfig(
    ...
    // Runtime payload takes priority over build-time config — enables dag_run_definition.inputs
    const text = typeof input['text'] === 'string' ? input['text'] : config.text;
    io.setOutput('text', text);
    io.setOutput('_agentSummary', `Input: ${text.length} chars.`);
