...
function isImageBinary(input: TPortPayload): boolean {
  ...
  return (
    image.kind === 'image' && typeof image.mimeType === 'string' && typeof image.uri === 'string'
  );
}
...
export class OkEmitterNodeDefinition extends AbstractNodeDefinition<typeof OkEmitterConfigSchema> {
  ...
  public override readonly defaultInputPort = 'image';
  ...
  protected override async executeWithConfig(
    ...
    io.setOutput('_agentSummary', 'Image validated. Status: ok.');
    return {
      ok: true,
      value: io.toOutput(),
    };
