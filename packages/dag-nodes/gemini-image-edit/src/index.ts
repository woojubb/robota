 ...
 */
export class GeminiImageEditNodeDefinition extends AbstractNodeDefinition<
  typeof GeminiImageEditConfigSchema
> {
  ...
  public readonly inputs: IDagNodeDefinition['inputs'] = [
    ...
    { key: 'text', label: 'Text', order: 1, type: 'string', required: true },
  ];
  ...
  protected override async executeWithConfig(
    ...
    if (!textInputResult.ok || textInputResult.value.trim().length === 0) {
      return {
        ...
        error: buildValidationError(
          ...
          'Gemini image node requires non-empty text input',
          { nodeId: context.nodeDefinition.nodeId },
        ),
      };
    }
    ...
    io.setOutput('_agentSummary', `Image edited with Gemini. Model: ${config.model}.`);
    return {
      ok: true,
      value: io.toOutput(),
    };
 ...
 * @extends AbstractNodeDefinition
 */
export class GeminiImageComposeNodeDefinition extends AbstractNodeDefinition<
  typeof GeminiImageComposeConfigSchema
> {
  ...
  protected override async executeWithConfig(
    ...
    if (!textInputResult.ok || textInputResult.value.trim().length === 0) {
      return {
        ...
        error: buildValidationError(
          ...
          'Gemini image compose node requires non-empty text input',
          { nodeId: context.nodeDefinition.nodeId },
        ),
      };
    }
    ...
    io.setOutput(
      '_agentSummary',
      `${imagesInputResult.value.length} images composed with Gemini. Model: ${config.model}.`,
    );
    return {
      ok: true,
      value: io.toOutput(),
