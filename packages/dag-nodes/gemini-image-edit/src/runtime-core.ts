 ...
 */
export function isImageBinaryValue(
  value: Partial<IPortBinaryValue> | null | undefined,
): value is IPortBinaryValue {
  if (!value) {
    return false;
  }
  if (value.kind !== 'image') {
  ...
  return typeof value.mimeType === 'string' && typeof value.uri === 'string';
}
...
export class GeminiImageRuntime {
  ...
  private mapProviderFailure(
    ...
    return {
      ok: false,
      error: buildTaskExecutionError(code, message, false, { code: providerCode, model }),
    };
  }
  ...
  public async editImage(
    request: IGeminiImageEditRequest,
  ...
  public async composeImages(
    request: IGeminiImageComposeRequest,
  ): Promise<TResult<IPortBinaryValue, IDagError>> {
    const defaultModelResult = this.resolveDefaultModel();
    if (!defaultModelResult.ok) return defaultModelResult;
    const allowedModels = this.resolveAllowedModels();
    const resolvedProvider = this.resolveProvider(allowedModels);
    const providerCapabilityResult = this.getImageProviderCapability(
      resolvedProvider,
      'composeImage',
    );
    if (!providerCapabilityResult.ok) {
