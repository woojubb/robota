...
vi.mock('@robota-sdk/google', () => ({
    GoogleProvider: vi.fn().mockImplementation((options: { apiKey: string; imageCapableModels: string[] }) => ({
...
describe('isImageBinaryValue edge cases', () => {
    ...
    it('returns false when mimeType is non-string', () => {
        const value = { kind: 'image', mimeType: 123, uri: 'test' } as unknown as Partial<IPortBinaryValue>;
        expect(isImageBinaryValue(value)).toBe(false);
    });

    it('returns false when uri is non-string', () => {
        const value = { kind: 'image', mimeType: 'image/png', uri: 123 } as unknown as Partial<IPortBinaryValue>;
        expect(isImageBinaryValue(value)).toBe(false);
    });
});
