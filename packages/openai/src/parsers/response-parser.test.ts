...
function createChatCompletion(overrides: Partial<OpenAI.Chat.ChatCompletion> = {}): OpenAI.Chat.ChatCompletion {
    return {
        id: 'chatcmpl-test',
...
function createStreamChunk(overrides: Record<string, unknown> = {}): OpenAI.Chat.ChatCompletionChunk {
    return {
        id: 'chatcmpl-chunk',
...
describe('OpenAIResponseParser', () => {
    ...
    describe('parseResponse', () => {
        ...
        it('should parse response with tool calls', () => {
            ...
            const assistantResult = result as { toolCalls: Array<{ id: string; type: string; function: { name: string; arguments: string } }> };
    ...
    describe('parseStreamingChunk', () => {
        ...
        it('should parse chunk with tool calls', () => {
            ...
            expect(result!.role).toBe('assistant');
            expect(result!.content).toBe('');
            const withToolCalls = result as unknown as { toolCalls: Array<{ id: string; function: { name: string; arguments: string } }> };
