...
function expectSuccess(
  result: IToolResult,
  label: string,
): asserts result is IToolResult & { success: true } {
  if (!result.success) {
...
async function main(): Promise<void> {
  const categoriesResult = await listTemplateCategoriesTool.execute(
    {},
    createContext('listTemplateCategories', {}),
  );
  ...
  const templatesResult = await listTemplatesTool.execute(
    templatesParams,
    createContext('listTemplates', templatesParams),
  );
  ...
  const detailResult = await getTemplateDetailTool.execute(
    detailParams,
    createContext('getTemplateDetail', detailParams),
  );
