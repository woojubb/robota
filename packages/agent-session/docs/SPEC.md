...
| `SESSION_RECORD_ENVELOPE_VERSION`           | Constant             | SELFHOST-014: the version of the `{ schemaVersion, record }` envelope AND the record it wraps — one number, read by the artifact path, the stores and the codec (TRANS-006 retired the codec's duplicate; #2185 named it for the pair; no alias) |
