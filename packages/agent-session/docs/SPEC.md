...
| `ISessionOptions`                           | Interface | `session-types.ts`                         | Constructor options for Session (tools, provider, systemMessage, effort, providerTimeout, optional sessionId) |
...
For the informational `PreModelCall` and `PostModelCall` hooks, the session includes the selected
model-effort value as `IHookInput.effort` for every provider-call round. The value is taken from the
assembled request when present, otherwise from the session's live model configuration; an unset value
is `auto` so the provider adapter, not the session, chooses a documented model default. These hooks
remain fire-and-forget and cannot block or mutate the call.
...
| `getProviderId`               | `() => string`                                                                                                                   | Returns the active provider identifier from the configured provider. Analytics projections use this accessor instead of guessing a provider from the model name.                                                                                                                                                                                                                                                                                                                                                                                    |
| `getModelId`                  | `() => string`                                                                                                                   | Returns the active model identifier, including model changes applied at runtime.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `getModelEffort`              | `() => TModelEffortSelection`                                                                                                    | Returns the current reasoning-effort selection; an unset core model value resolves to `auto`, which preserves provider-default selection.                                                                                                                                                                                                                                                                                                                                                                                                           |
| `getOfferedToolSchemas`       | `() => IToolSchema[]`                                                                                                            | CLI-1990: the schemas the next request will carry — resident tools plus loaded deferred ones — read through `ICommandSessionTools` by `/context`.                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `applyModelOptions`           | `(options: { model?: string; effort?: TModelEffortSelection; temperature?: number; maxOutputTokens?: number }) => Promise<void>` | Re-applies model options to the live session. Awaits `robota.ensureReady()` first so it works on a cold (never-run) session — `setModel` requires full initialization, which otherwise only happens lazily on the first `run()`. Then calls `robota.setModel` so the next call reflects the options. `auto` is preserved for the provider boundary rather than converted to a framework default. Maps `maxOutputTokens` → the agent's `maxTokens` channel; updates `this.model` so `getModelId()` stays accurate. Absent fields are left untouched. |
| `withScopedModelEffort`       | `(effort: TModelEffort, operation: () => Promise<T>) => Promise<T>`                                                              | Applies a temporary effort, awaits the operation, and restores the entry effort in `finally`, including rejection, cancellation, and nested scopes.                                                                                                                                                                                                                                                                                                                                                                                                 |
