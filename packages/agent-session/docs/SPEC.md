...
| `applyModelOptions`           | `(options: { model?: string; effort?: TModelEffort; temperature?: number; maxOutputTokens?: number }) => void` | Re-applies model options to the live session via `robota.setModel` so the next call reflects them. Maps `maxOutputTokens` → the agent's `maxTokens` channel; updates `this.model` so `getModelId()` stays accurate. Absent fields are left untouched. |
