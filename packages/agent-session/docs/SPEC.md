...
permission hooks of its tool calls, its model-call hooks, and the pre-compaction hook of a
compaction the turn itself triggers. A hook fired with no prompt in flight — session start and end,
a user-requested compaction, the post-compaction hook — receives nothing, because there is no root
for it to name. The tool-use hooks take the root from the tool call's context, apart from the body's
