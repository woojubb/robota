...
import type { PlaygroundExecutor, IPlaygroundAgentConfig, IPlaygroundExecutorResult, TPlaygroundMode, IConversationEvent, IVisualizationData } from '../lib/playground/robota-executor';
...
export function playgroundReducer(state: IPlaygroundState, action: TPlaygroundReducerAction): IPlaygroundState {
    switch (action.type) {
        case 'SET_EXECUTOR':
        ...
        case 'ADD_AGENT_CONFIG':
            return { ...state, agentConfigs: [...state.agentConfigs, action.payload], currentAgentConfig: action.payload, mode: 'agent' };
        case 'UPDATE_AGENT_CONFIG':
            return { ...state, agentConfigs: state.agentConfigs.map((cfg, i) => i === action.payload.index ? action.payload.config : cfg) };
        ...
        case 'SET_AUTH':
            return { ...state, userId: action.payload.userId || null, sessionId: action.payload.sessionId || null, authToken: action.payload.authToken || null };
        case 'SET_LOADING':
        ...
        case 'ADD_TOOL_TO_AGENT_OVERLAY': {
            ...
            return { ...state, addedToolsByAgent: { ...state.addedToolsByAgent, [agentId]: [...prev, toolId] } };
        }
        default:
            return state;
    }
