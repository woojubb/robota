  ...
  return new FunctionTool(
    {
      name: 'getCurrentTime',
      description:
        'Get current date and time information. Can provide time in different formats and timezones.',
      parameters: {
        type: 'object',
        properties: {
          timezone: {
            type: 'string',
            description:
              'Timezone identifier (e.g., "Asia/Seoul", "America/New_York", "UTC"). Defaults to local timezone.',
            default: 'local',
          },
          format: {
            type: 'string',
            ...
            description:
              'Output format: "iso" (ISO 8601), "readable" (human readable), "timestamp" (Unix timestamp), "detailed" (comprehensive info)',
            default: 'readable',
          },
          includeWeekday: {
            type: 'boolean',
