...
export class ProjectManager {
    ...
    getAllProjects(): IPlaygroundProject[] {
        return Array.from(this.projects.values()).sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime())
    }
    ...
    createProject(name: string, description: string = '', settings: IProjectSettings): IPlaygroundProject {
        ...
        const project: IPlaygroundProject = {
            ...
            createdAt: now, updatedAt: now, version: CURRENT_VERSION
        }
    ...
    importProject(projectData: TUniversalValue): IPlaygroundProject {
        ...
        if (typeof name !== 'string' || typeof code !== 'string') throw new Error('Invalid project data: missing required fields')
        if (!isPlaygroundProvider(provider)) throw new Error('Invalid project data: invalid provider')
        if (typeof config !== 'object' || config === null || Array.isArray(config) || config instanceof Date) throw new Error('Invalid project data: invalid config')
        ...
        const now = new Date()
        const project: IPlaygroundProject = {
            id, name: `${name} (Imported)`,
            description: typeof description === 'string' ? description : undefined,
            code, provider,
            config: { ...configRecord, model: configRecord.model as string, temperature: configRecord.temperature as string },
            createdAt: now, updatedAt: now, version: CURRENT_VERSION
        }
    ...
    saveProject(projectData: Omit<IPlaygroundProject, 'id' | 'createdAt' | 'updatedAt' | 'version'>): string {
        const now = new Date()
        const id = this.generateId()
        this.projects.set(id, { ...projectData, id, createdAt: now, updatedAt: now, version: CURRENT_VERSION })
        this.saveToStorage()
        return id
    }
    ...
    listProjects(): IProjectMetadata[] {
        return Array.from(this.projects.values())
            .map(project => ({
                ...
                provider: project.provider, linesOfCode: project.code.split('\n').length
            }))
    ...
    duplicateProject(id: string): string | null {
        ...
        const now = new Date()
        this.projects.set(newId, { ...original, id: newId, name: `${original.name} (Replica)`, createdAt: now, updatedAt: now })
        this.saveToStorage()
        return newId
    }
...
function parseProjectEntries(parsed: TUniversalValue[]): Array<[string, IPlaygroundProject]> {
    ...
    for (const item of parsed) {
        if (typeof item !== 'object' || item === null || Array.isArray(item) || item instanceof Date) continue
        ...
        if (typeof id !== 'string' || typeof name !== 'string' || typeof code !== 'string' || typeof version !== 'string') continue
        if (!isPlaygroundProvider(provider)) continue
        if (typeof createdAt !== 'string' || typeof updatedAt !== 'string') continue
        if (typeof config !== 'object' || config === null || Array.isArray(config) || config instanceof Date) continue
        ...
        projects.push([id, {
            ...
            createdAt: new Date(createdAt), updatedAt: new Date(updatedAt), version,
        }])
    }
    return projects
}
