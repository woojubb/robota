...
import type { IAIProvider, IEventService, IExecutor, IToolExecutionContext, IToolSchema, TLoggerData, TToolParameters, TUniversalMessage, TUniversalValue } from '@robota-sdk/agents';
import { OpenAIProvider } from '@robota-sdk/openai';
import { AnthropicProvider } from '@robota-sdk/anthropic';
import { FunctionTool } from '@robota-sdk/agents';
import { PlaygroundHistoryPlugin, type IVisualizationData } from './plugins/playground-history-plugin';
...
export type { IPlaygroundTool, IPlaygroundPlugin, IPlaygroundAgentConfig, IPlaygroundExecutorResult, TPlaygroundMode, TPlaygroundUiErrorKind, IPlaygroundUiError, IVisualizationData, IConversationEvent } from './robota-executor-types';

import type { IPlaygroundTool, IPlaygroundAgentConfig, IPlaygroundExecutorResult, TPlaygroundMode, IPlaygroundToolHookFlags } from './robota-executor-types';
...
export class PlaygroundExecutor {
    ...
    constructor(private serverUrl: string, private authToken: string, options: { eventService: IEventService; logger?: ILogger }) {
        this.logger = options.logger || SilentLogger;
        this.historyPlugin = new PlaygroundHistoryPlugin({ maxEvents: 1000, enableVisualization: true, logger: this.logger });
        this.statisticsPlugin = new PlaygroundStatisticsPlugin({ enabled: true, collectUIMetrics: true, collectBlockMetrics: true, trackResponseTime: true, trackExecutionDetails: true, maxEntries: 1000, slowExecutionThreshold: 3000, errorRateThreshold: 10 });
    ...
    async createAgent(config: IPlaygroundAgentConfig): Promise<void> {
        ...
        this.currentAgent = new Robota({ name: config.name, aiProviders, defaultModel: config.defaultModel, tools: normalizedTools, eventService: this.eventService });
        this.agentRegistry.set(rootAgentId, this.currentAgent);
        this.agentToolsRegistry.set(rootAgentId, normalizedTools);
        this.setMode('agent');
        await this.statisticsPlugin.recordUIInteraction('agent_create', { agentName: config.name, provider: config.defaultModel.provider, model: config.defaultModel.model });
    }

    async updateAgentTools(agentId: string, tools: IPlaygroundTool[]): Promise<{ version: number }> {
        if (!agentId || typeof agentId !== 'string') throw new Error('updateAgentTools: invalid agentId');
    ...
    async getAgentConfiguration(agentId: string): Promise<{ version: number; tools: Array<{ name: string; parameters?: string[] }>; updatedAt: number; metadata?: Record<string, unknown> }> {
        if (!agentId || typeof agentId !== 'string') throw new Error('getAgentConfiguration: invalid agentId');
        const agent = this.agentRegistry.get(agentId);
    ...
    async run(prompt: string): Promise<IPlaygroundExecutorResult> {
        ...
        try {
            ...
            const executionResult: IPlaygroundExecutorResult = { success: true, response: result.content || 'No response', duration, visualizationData: this.getVisualizationData(), uiError: undefined };
            await this.recordStats({ success: true, duration, streaming: false });
            return executionResult;
        } catch (error) {
            const duration = Date.now() - startTime;
            const executionResult: IPlaygroundExecutorResult = { success: false, response: 'Execution failed', duration, error: error instanceof Error ? error : new Error(String(error)), visualizationData: this.getVisualizationData(), uiError: toPlaygroundUiError(error instanceof Error ? error : String(error)) };
            await this.recordStats({ success: false, duration, streaming: false, error: error instanceof Error ? error.message : String(error) });
            return executionResult;
        }
    }
    ...
    async execute(prompt: string, onChunk?: (chunk: string) => void): Promise<IPlaygroundExecutorResult> {
        ...
        try {
            ...
            return { success: true, response: result, duration, visualizationData: this.getVisualizationData(), uiError: undefined };
        } catch (error) {
            const duration = Date.now() - startTime;
            this.logger.error('Playground execution failed', { error: error instanceof Error ? error.message : String(error) });
            return { success: false, response: 'Execution failed', duration, error: error instanceof Error ? error : new Error(String(error)), visualizationData: this.getVisualizationData(), uiError: toPlaygroundUiError(error instanceof Error ? error : String(error)) };
        }
    }
    ...
    async *runStream(prompt: string): AsyncGenerator<string, IPlaygroundExecutorResult> {
        ...
        try {
            ...
            yield fullResponse;
            const duration = Date.now() - startTime;
            const executionResult: IPlaygroundExecutorResult = { success: true, response: fullResponse, duration, visualizationData: this.getVisualizationData() };
            await this.recordStats({ success: true, duration, streaming: true });
            return executionResult;
        } catch (error) {
            const duration = Date.now() - startTime;
            const executionResult: IPlaygroundExecutorResult = { success: false, response: 'Streaming execution failed', duration, error: error instanceof Error ? error : new Error(String(error)), visualizationData: this.getVisualizationData() };
            await this.recordStats({ success: false, duration, streaming: true, error: error instanceof Error ? error.message : String(error) });
            return executionResult;
        }
    }

    getPlaygroundStatistics(): IPlaygroundMetrics { return this.statisticsPlugin.getPlaygroundStats().metrics; }
    ...
    getVisualizationData(): IVisualizationData { return this.historyPlugin.getVisualizationData(); }
    getPlaygroundEvents() { return this.historyPlugin.getAllEvents(); }
    ...
    clearHistory(): void { this.historyPlugin.clearEvents(); }

    async dispose(): Promise<void> {
        try {
            if (this.currentAgent) { await this.currentAgent.destroy(); this.currentAgent = undefined; }
            if (this.websocketClient) { await this.websocketClient.disconnect(); this.websocketClient = undefined; }
    ...
    getLastExecutionId(): string | null { return 'agent-execution-' + Date.now(); }
    ...
    private async recordStats(opts: { success: boolean; duration: number; streaming: boolean; error?: string }): Promise<void> {
        await this.statisticsPlugin.recordPlaygroundExecution({ success: opts.success, duration: opts.duration, provider: 'openai', model: 'gpt-4', mode: this.mode || 'agent', streaming: opts.streaming, timestamp: new Date(), error: opts.error });
    }
    ...
    private createRemoteExecutor(): IExecutor {
        if (!this.serverUrl || !this.authToken) throw new Error('Server URL and auth token required for remote executor');
        const apiUrl = this.serverUrl.replace(/\/ws$/, '').replace(/^ws/, 'http') + '/api/v1/remote';
        return new RemoteExecutor({ serverUrl: apiUrl, userApiKey: this.authToken, timeout: 30000, enableWebSocket: false });
    }

    private createProvidersWithExecutor(): IAIProvider[] {
        const remoteExecutor = this.createRemoteExecutor();
        return [new OpenAIProvider({ executor: remoteExecutor, model: 'gpt-4o-mini' }), new AnthropicProvider({ executor: remoteExecutor })];
    }

    private normalizeTools(tools: IPlaygroundTool[]): FunctionTool[] {
        return tools.map(tool => tool instanceof FunctionTool ? tool : this.buildFunctionTool(tool));
    }

    private buildFunctionTool(tool: IPlaygroundTool): FunctionTool {
        const schema: IToolSchema = { name: tool.name, description: tool.description || `Playground tool: ${tool.name}`, parameters: { type: 'object', properties: { value: { type: 'string', description: 'Value to echo' } } } };
        return new FunctionTool(schema, async (params: TToolParameters, context?: IToolExecutionContext): Promise<TUniversalValue> => tool.execute(params));
    }

    private setMode(mode: TPlaygroundMode): void { this.mode = mode; }
}
