...
| Command system contracts       | `command-contracts.ts`          | `ICommand`, `ICommandSource`, `ICommandResult`, `TCommandInvocationSource` (REMOTE-003), plugin-adapter + status-line command settings contracts                                                                                                                                                                                           |
...
| Interactive-session contracts  | `session-contracts.ts`          | `IInteractiveSession`, `IInteractiveSessionEvents`, `IExecutionResult`, `IToolState`/`Summary`, `IInteractiveSessionStore`; REMOTE-014 E5 co-drive attribution: `TDriverId`, `ISubmitOptions`, and the runtime driver-id constants `OWNER_DRIVER_ID` / `AGENT_DRIVER_ID` (display-only attribution, never authorization — OWNER PRINCIPLE) |
...
| Command system (`command-contracts`)                          | `ICommand`, `ICommandSource`, `ICommandResult`, `TCommandInvocationSource` (REMOTE-003), plugin-adapter + status-line command settings contracts                                                                                                |
