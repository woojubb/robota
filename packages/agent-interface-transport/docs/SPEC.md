...
| Interactive-session contracts          | `session-contracts.ts`          | `IInteractiveSession` (whose `executeCommand` carries the optional CMD-004 command-origin driver id), `IInteractiveSessionEvents` (incl. the CMD-004 `ui_intent` + `session_renamed` events), `IExecutionResult`, `IToolState`/`Summary`, `IInteractiveSessionStore`                                                                                                                       |
...
| Command system (`command-contracts`)                          | `ICommand`, `ICommandSource`, `ICommandResult` (+ CMD-004 `hostActions`/`uiIntents`), `TCommandInvocationSource` (REMOTE-003), `TCommandHostAction`, `TCommandUiIntent`, legacy `TCommandEffect`, plugin-adapter + status-line command settings contracts |
...
| Interactive session (`session-contracts`)                     | `IInteractiveSession`, `IInteractiveSessionEvents` (incl. `ui_intent`/`session_renamed`), `IExecutionResult`, `IToolState`/`Summary`, `IInteractiveSessionStore`                                                                                          |
