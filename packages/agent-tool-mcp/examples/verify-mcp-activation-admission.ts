...
function runStatusScenario(): void {
  ...
  const status = service.inspect(
    request({ workspace: { repositoryKey: 'example-repository', trustState: 'untrusted', generation: 1 } }),
  );
  assertCondition(status.status === 'untrusted' && !status.allowed, 'untrusted request was admitted');
...
function runLifecycleScenario(): void {
  ...
  assertCondition(revoked.status === 'revoked' && !revoked.allowed, 'revoked definition remained admitted');
