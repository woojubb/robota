...
export class GoogleProvider extends AbstractAIProvider implements IImageGenerationProvider {
    ...
    public async generateImage(request: IImageGenerationRequest): Promise<TProviderMediaResult<IImageGenerationResult>> {
        if (request.prompt.trim().length === 0) {
            return { ok: false, error: { code: 'PROVIDER_INVALID_REQUEST', message: 'Image generation requires a non-empty prompt.' } };
        }
        if (request.model.trim().length === 0) {
            return { ok: false, error: { code: 'PROVIDER_INVALID_REQUEST', message: 'Image generation requires a non-empty model.' } };
        }
    ...
    public async editImage(request: IImageEditRequest): Promise<TProviderMediaResult<IImageGenerationResult>> {
        if (request.prompt.trim().length === 0) {
            return { ok: false, error: { code: 'PROVIDER_INVALID_REQUEST', message: 'Image edit requires a non-empty prompt.' } };
        }
        if (request.model.trim().length === 0) {
            return { ok: false, error: { code: 'PROVIDER_INVALID_REQUEST', message: 'Image edit requires a non-empty model.' } };
        }
    ...
    public async composeImage(request: IImageComposeRequest): Promise<TProviderMediaResult<IImageGenerationResult>> {
        if (request.prompt.trim().length === 0) {
            return { ok: false, error: { code: 'PROVIDER_INVALID_REQUEST', message: 'Image compose requires a non-empty prompt.' } };
        }
        if (request.model.trim().length === 0) {
            return { ok: false, error: { code: 'PROVIDER_INVALID_REQUEST', message: 'Image compose requires a non-empty model.' } };
        }
        if (request.images.length < 2) {
            return { ok: false, error: { code: 'PROVIDER_INVALID_REQUEST', message: 'Image compose requires at least two input images.' } };
        }
    ...
    private async executeDirect(messages: TUniversalMessage[], options?: IChatOptions): Promise<TUniversalMessage> {
        if (!this.client) {
        ...
        const genConfig = buildGenerationConfig(
            messages, this.options.defaultResponseModalities, this.options.imageCapableModels, options
        );
        ...
        const responseModalities = buildResponseModalities(
            messages, this.options.defaultResponseModalities, options?.google?.responseModalities
        );
    ...
    private async *executeDirectStream(messages: TUniversalMessage[], options?: IChatOptions): AsyncIterable<TUniversalMessage> {
        const responseModalities = buildResponseModalities(
            messages, this.options.defaultResponseModalities, options?.google?.responseModalities
        );
    ...
    private async runImageRequest(
        ...
        try {
            ...
            if (outputs.length === 0) {
                return { ok: false, error: { code: 'PROVIDER_UPSTREAM_ERROR', message: 'Google image response did not include image output parts.' } };
            }
