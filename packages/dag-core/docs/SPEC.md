...
| `IDagExecutionLineage`              | `types/node-lifecycle.ts`                 | Root and immediate-parent run identity, nested DAG depth, and ordered composite-node ancestry for in-process child runs; no HTTP representation                        |
...
| `IDagRuntimeResult`                 | `types/runtime-provider.ts`               | Final result of a single runtime DAG execution                                                                                                                         |
