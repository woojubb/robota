...
export class DagDefinitionValidator {
  public static validate(definition: IDagDefinition): TResult<IDagDefinition, IDagError[]> {
    ...
    if (definition.dagId.trim().length === 0) {
      errors.push(buildValidationError('DAG_VALIDATION_EMPTY_DAG_ID', 'dagId must not be empty'));
    }
    ...
    for (const node of definition.nodes) {
      ...
      if (nodeIdSet.has(node.nodeId)) {
        errors.push(
          buildValidationError('DAG_VALIDATION_DUPLICATE_NODE_ID', 'nodeId must be unique', {
            nodeId: node.nodeId,
          }),
        );
        continue;
