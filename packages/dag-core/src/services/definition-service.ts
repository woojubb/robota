...
export class DagDefinitionService {
  ...
  public async getDefinitionByDagId(
    dagId: string,
    version?: number,
  ...
  public async updateDraft(
    ...
    if (!existing) {
      return {
        ok: false,
        error: [
          buildValidationError('DAG_VALIDATION_DEFINITION_NOT_FOUND', 'Definition does not exist', {
  ...
  public async publish(
    ...
    const byDagId = await this.storage.listDefinitionsByDagId(dagId);
    const latestVersion = byDagId.reduce(
      (currentMax, definition) =>
