...
function hasPortCatalog(ports: IPortDefinition[] | undefined): ports is IPortDefinition[] {
  return Array.isArray(ports) && ports.length > 0;
}

function resolveBindingInputIdentity(
  inputPorts: IPortDefinition[] | undefined,
  bindingInputKey: string,
): string {
  if (!hasPortCatalog(inputPorts)) {
...
export function validateEdgesAndBindings(
  ...
  for (const edge of definition.edges) {
    ...
    for (const binding of edge.bindings) {
      const outputPort = hasPortCatalog(outputPorts)
        ? findPort(outputPorts, binding.outputKey)
        : undefined;
      if (hasPortCatalog(outputPorts) && !outputPort) {
        errors.push(
          buildValidationError(
      ...
      const resolvedInput = hasPortCatalog(inputPorts)
        ? resolveInputPort(inputPorts, binding.inputKey)
        : undefined;
      const inputPort = resolvedInput?.port;
      if (hasPortCatalog(inputPorts) && !inputPort) {
        errors.push(
          buildValidationError(
  ...
  for (const edge of definition.edges) {
    ...
    for (const binding of edge.bindings) {
      const toNode = nodeById.get(edge.to);
      const bindingInputIdentity = resolveBindingInputIdentity(toNode?.inputs, binding.inputKey);
