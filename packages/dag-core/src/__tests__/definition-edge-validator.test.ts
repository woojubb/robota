...
describe('validateEdgesAndBindings', () => {
  ...
  it('accepts bound edges when node port catalogs are omitted from the definition', () => {
    const def = makeDefinition({
      nodes: [
        {
          nodeId: 'A',
  ...
  it('keeps exact list handle identities distinct when no input catalog is present', () => {
    const def = makeDefinition({
      nodes: [
        {
          nodeId: 'A',
      ...
      edges: [
        { from: 'A', to: 'C', bindings: [{ outputKey: 'out', inputKey: 'images[0]' }] },
        { from: 'B', to: 'C', bindings: [{ outputKey: 'out', inputKey: 'images[1]' }] },
      ],
    });
  ...
  it('allows binary ports when one side has no binaryKind', () => {
    const def = makeDefinition();
    def.nodes[0].outputs = [{ key: 'out', type: 'binary', required: true }];
    def.nodes[1].inputs = [{ key: 'in', type: 'binary', required: true, binaryKind: 'image' }];
    def.edges = [{ from: 'A', to: 'B', bindings: [{ outputKey: 'out', inputKey: 'in' }] }];
    const errors = validateEdgesAndBindings(def, nodeIdSet(def), nodeById(def));
    expect(errors).toHaveLength(0);
  });
  ...
  it('resolves list port handle keys in bindings', () => {
    const def = makeDefinition();
    def.nodes[1].inputs = [{ key: 'images', type: 'binary', required: true, isList: true }];
