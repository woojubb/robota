...
function findErrorCode(
  result: ReturnType<typeof DagDefinitionValidator.validate>,
  code: string,
): boolean {
...
describe('DagDefinitionValidator', () => {
  describe('valid definitions', () => {
    ...
    it('should accept connected definitions that omit runtime-owned port catalogs', () => {
  ...
  describe('dagId validation', () => {
    it('should fail when dagId is empty', () => {
      const result = DagDefinitionValidator.validate(createValidDefinition({ dagId: '' }));
  ...
  describe('version validation', () => {
    it('should fail when version is zero', () => {
      const result = DagDefinitionValidator.validate(createValidDefinition({ version: 0 }));
      expect(findErrorCode(result, 'DAG_VALIDATION_INVALID_VERSION')).toBe(true);
    });

    it('should fail when version is negative', () => {
      const result = DagDefinitionValidator.validate(createValidDefinition({ version: -1 }));
      expect(findErrorCode(result, 'DAG_VALIDATION_INVALID_VERSION')).toBe(true);
    });
  });
  ...
  describe('node validation', () => {
    ...
    it('should fail when duplicate nodeIds exist', () => {
      const result = DagDefinitionValidator.validate(
        createValidDefinition({
          nodes: [createMinimalNode('dup', 'custom'), createMinimalNode('dup', 'custom')],
          edges: [],
        }),
      );
  ...
  describe('edge validation', () => {
    ...
    it('should fail when edge has no bindings', () => {
      const result = DagDefinitionValidator.validate(
        createValidDefinition({
          edges: [{ from: 'node-a', to: 'node-b', bindings: [] }],
        }),
      );
