...
import { TestStoragePort } from './test-storage-port.js';
...
describe('DagDefinitionService extended', () => {
  it('createDraft rejects duplicate version', async () => {
    const storage = new TestStoragePort();
...
describe('DagDefinitionValidator extended', () => {
  ...
  it('validates invalid input port minItems', () => {
    ...
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error.some((e) => e.code === 'DAG_VALIDATION_INVALID_INPUT_MIN_ITEMS')).toBe(
      true,
    );
  });
  ...
  it('validates invalid input port maxItems', () => {
    ...
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error.some((e) => e.code === 'DAG_VALIDATION_INVALID_INPUT_MAX_ITEMS')).toBe(
      true,
    );
  });
  ...
  it('validates minItems exceeding maxItems', () => {
    ...
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error.some((e) => e.code === 'DAG_VALIDATION_INVALID_INPUT_ITEM_RANGE')).toBe(
      true,
    );
  });
  ...
  it('validates cost policy with invalid version', () => {
    ...
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error.some((e) => e.code === 'DAG_VALIDATION_INVALID_COST_POLICY_VERSION')).toBe(
      true,
    );
  });
