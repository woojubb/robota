...
describe('DagDefinitionValidator', () => {
  it('returns validation error for duplicate nodeId', () => {
    ...
    expect(validated.error.some((error) => error.code === 'DAG_VALIDATION_DUPLICATE_NODE_ID')).toBe(
      true,
    );
  });
  ...
  it('returns validation error for cycle', () => {
    ...
    definition.edges.push({
      ...
      bindings: [{ outputKey: 'status', inputKey: 'image' }],
    });
    ...
    expect(validated.error.some((error) => error.code === 'DAG_VALIDATION_CYCLE_DETECTED')).toBe(
      true,
    );
  });
});
...
describe('DagDefinitionService', () => {
  ...
  it('fails to update non-draft definition', async () => {
    ...
    expect(
      updateAttempt.error.some((error) => error.code === 'DAG_VALIDATION_UPDATE_ONLY_DRAFT'),
    ).toBe(true);
  });
});
