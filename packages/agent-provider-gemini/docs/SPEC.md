...
| `IGeminiProviderOptions`                    | interface                   | `src/types.ts`               | Configuration options for constructing `GeminiProvider`. Fields: `apiKey` (required), `responseMimeType`, `responseSchema`, `defaultResponseModalities`, `imageCapableModels`, `executor`, plus index signature.                                |
