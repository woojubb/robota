      ...
      if (port.required) {
        return {
          ...
          error: buildValidationError(
            ...
            mode === 'input'
              ? `Required input port "${port.key}" (${port.type}) is missing on node "${nodeId}"`
              : `Required output port "${port.key}" (${port.type}) was not set by node "${nodeId}"`,
      ...
      return {
        ...
        error: buildValidationError(
          mode === 'input'
            ...
            : 'DAG_VALIDATION_NODE_OUTPUT_TYPE_MISMATCH',
          mode === 'input'
            ? `Input port "${port.key}" on node "${nodeId}" expects type "${port.type}" but received incompatible value`
            : `Output port "${port.key}" on node "${nodeId}" emitted value incompatible with type "${port.type}"`,
          { nodeId, key: port.key, type: port.type },
          mode === 'input'
            ? {
