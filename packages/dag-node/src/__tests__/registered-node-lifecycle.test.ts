...
describe('RegisteredNodeLifecycle', () => {
  ...
  describe('validateInput', () => {
    it('validates required input ports', async () => {
      const inputs: IPortDefinition[] = [{ key: 'text', type: 'string', required: true }];
      const handler: INodeTaskHandler = {
        execute: vi.fn(),
      };
      const lifecycle = new RegisteredNodeLifecycle(handler);
    ...
    it('allows optional missing ports', async () => {
      const inputs: IPortDefinition[] = [{ key: 'text', type: 'string', required: false }];
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateInput({}, makeContext(inputs));
      expect(result.ok).toBe(true);
    ...
    it('validates list port type mismatch', async () => {
      ...
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateInput({ items: 'not-array' }, makeContext(inputs));
      expect(result.ok).toBe(false);
      if (result.ok) return;
      expect(result.error.code).toBe('DAG_VALIDATION_NODE_INPUT_TYPE_MISMATCH');
    });
    ...
    it('validates list minItems', async () => {
      ...
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateInput({ items: ['a'] }, makeContext(inputs));
      expect(result.ok).toBe(false);
      if (result.ok) return;
    ...
    it('validates list maxItems', async () => {
      ...
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateInput({ items: ['a', 'b'] }, makeContext(inputs));
      expect(result.ok).toBe(false);
      if (result.ok) return;
  ...
  describe('validateOutput', () => {
    it('validates required output ports', async () => {
      const outputs: IPortDefinition[] = [{ key: 'result', type: 'string', required: true }];
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
    ...
    it('passes valid output', async () => {
      ...
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateOutput({ result: 'done' }, makeContext([], outputs));
      expect(result.ok).toBe(true);
    });
    ...
    it('validates output list minItems', async () => {
      ...
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateOutput({ items: ['a'] }, makeContext([], outputs));
      expect(result.ok).toBe(false);
      if (result.ok) return;
  ...
  describe('binary port validation', () => {
    ...
    it('validates number port type', async () => {
      const inputs: IPortDefinition[] = [{ key: 'count', type: 'number', required: true }];
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
    ...
    it('validates boolean port type', async () => {
      const inputs: IPortDefinition[] = [{ key: 'flag', type: 'boolean', required: true }];
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
    ...
    it('validates object port type', async () => {
      const inputs: IPortDefinition[] = [{ key: 'data', type: 'object', required: true }];
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateInput({ data: { key: 'value' } }, makeContext(inputs));
      expect(result.ok).toBe(true);
    });

    it('validates array port type', async () => {
      const inputs: IPortDefinition[] = [{ key: 'items', type: 'array', required: true }];
      const handler: INodeTaskHandler = { execute: vi.fn() };
      const lifecycle = new RegisteredNodeLifecycle(handler);
      const result = await lifecycle.validateInput({ items: ['a', 'b'] }, makeContext(inputs));
      expect(result.ok).toBe(true);
