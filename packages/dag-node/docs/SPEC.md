...
| `normalizeProviderMediaOutput`     | Function       | #2168: provider-neutral projection of a provider media output onto `IPortBinaryValue` under a node policy                                                  |
| `IProviderMediaOutputPolicy`       | Interface      | #2168: node-specific kind / MIME family / default MIME / data-URI policy for `normalizeProviderMediaOutput`                                                |
| `TProviderMediaOutputRejection`    | Type           | #2168: rejection reason the leaf decorates into its own DAG error code                                                                                     |
...
| `StaticNodeLifecycleFactory`, `StaticNodeTaskHandlerRegistry` | the runtime server                  | Creates lifecycle instances for node execution                   |
