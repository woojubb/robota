...
import { resolvePartialExecutionTargets } from './partial-run-targets.js';

type TRunStatus = 'pending' | 'running' | 'success' | 'failed';

export interface IOrchestratorCreateRunOptions {
...
export class OrchestratorRunService {
  ...
  async createRun(
    definition: IDagDefinition,
    input: TPortPayload,
    options?: IOrchestratorCreateRunOptions,
    ...
    const promptRequest = translationResult.value;
    if (options?.partialRun) {
      const partialTargets = resolvePartialExecutionTargets(definition, options.partialRun);
      if (!partialTargets.ok) {
        return partialTargets;
      }
      promptRequest.partial_execution_targets = partialTargets.value;
    }
  ...
  async startRun(
    ...
    if (!run) {
      return {
        ok: false,
        error: buildValidationError('ORCHESTRATOR_RUN_NOT_FOUND', 'Run not found', {
          preparationId,
        }),
      };
    }
  ...
  async createAndStartRun(
    ...
    const createResult = await this.createRun(definition, input, options);
    if (!createResult.ok) {
      return createResult;
    }
  ...
  async getRunStatus(dagRunId: string): Promise<TResult<{ status: TRunStatus }, IDagError>> {
    ...
    if (!found) {
      return {
        ok: false,
        error: buildValidationError('ORCHESTRATOR_RUN_NOT_FOUND', 'Run not found', { dagRunId }),
      };
    }
  ...
  async getRunResult(dagRunId: string): Promise<TResult<IRunResult, IDagError>> {
    ...
    if (!entry) {
      return {
        ok: false,
        error: buildValidationError('ORCHESTRATOR_RUN_NOT_COMPLETED', 'Run has not completed yet', {
          dagRunId,
          promptId,
        }),
      };
  ...
  private buildFailedRunResult(run: IRunState, promptId: string): IRunResult {
    const nodeTypeMap = new Map(run.definition.nodes.map((node) => [node.nodeId, node.nodeType]));
    const nodeErrors: IRunNodeError[] = run.nodeEvents
      .filter(
        (evt): evt is Extract<TRunProgressEvent, { eventType: 'task.failed' }> =>
  ...
  private buildSuccessRunResult(run: IRunState, promptId: string): IRunResult {
    const completedEventsByNode = new Map<
      string,
      TRunProgressEvent & { eventType: 'task.completed' }
    >();
