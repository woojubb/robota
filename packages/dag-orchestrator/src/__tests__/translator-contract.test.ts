...
describe('translateDefinitionToPrompt', () => {
  it('returns error result for empty definition', () => {
    const result = translateDefinitionToPrompt(makeDefinition({ nodes: [] }), {});
  ...
  it('produces TPromptLink arrays from edge bindings', () => {
    const definition = makeDefinition({
      nodes: [makeNode('src', 'Source', {}), makeNode('dst', 'Sink', {})],
      edges: [
        {
          from: 'src',
          to: 'dst',
  ...
  it('does not require node port definitions to translate edge bindings', () => {
    const definition = makeDefinition({
      nodes: [makeNode('src', 'Source', {}, false), makeNode('dst', 'Sink', {}, false)],
      edges: [
        {
          from: 'src',
          to: 'dst',
    ...
    expect(result.value.prompt['dst'].inputs['input_image']).toEqual(['src', 0]);
  });

  it('assigns incremental slot indices for multiple output keys from the same node', () => {
    const definition = makeDefinition({
      nodes: [makeNode('src', 'MultiOutput', {}), makeNode('dst', 'Sink', {})],
      edges: [
        {
          from: 'src',
          to: 'dst',
