...
import type { IDagDefinition, TRunProgressEvent, IDagError } from '@robota-sdk/dag-core';
...
describe('OrchestratorRunService', () => {
  ...
  describe('createRun', () => {
    ...
    it('adds partial execution targets for start node and downstream nodes', async () => {
      const partialDefinition: IDagDefinition = {
        dagId: 'partial-dag',
        version: 1,
        status: 'draft',
        nodes: [
      ...
      expect(vi.mocked(mockClient.submitPrompt).mock.calls[0][0].partial_execution_targets).toEqual(
        ['source', 'middle'],
      );
    });

    it('rejects partial run when start node does not exist', async () => {
      const result = await service.createRun(
        makeDefinition(),
        {},
        {
          partialRun: { startNodeId: 'missing' },
        },
      );

      expect(result.ok).toBe(false);
      if (result.ok) return;
      expect(result.error.code).toBe('ORCHESTRATOR_PARTIAL_RUN_START_NODE_NOT_FOUND');
    });
  });
