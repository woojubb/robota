...
beforeEach(async () => {
  dir = await realpath(await mkdtemp(join(tmpdir(), 'wf-create-')));
});
afterEach(async () => {
...
describe.runIf(process.platform === 'linux')(
  'executeWorkflowsCreate (TC-02: author + save + run existing nodes)',
  () => {
...
// ARCH-047: project mutation is Linux-only (stable root-anchored host); refused elsewhere.
describe.runIf(process.platform === 'linux')(
  'executeWorkflowsRun (TC-03: the saved artifact is re-runnable)',
  () => {
...
// ARCH-047: project mutation is Linux-only (stable root-anchored host); refused elsewhere.
describe.runIf(process.platform === 'linux')(
  'executeWorkflowsCreate (TC-05: on-the-fly prompt node)',
  () => {
    ...
    it('saves the prompt node + workflow, surfaces the missing-key error, and reuses the node', async () => {
      const first = await executeWorkflowsCreate(
        '"rewrite as a pirate"',
        dir,
