...
**Re-derive the list rather than trusting it**, because a hand-written enumeration here has now been wrong twice — first claiming no public entry point existed, then naming three when there were six. The complete set is `grep -rn "hookTypeExecutors?:" packages/*/src` for the option declarations, cross-checked against the `runHooks(...)` call sites that forward a fourth argument. What holds across all of them is the property, not the count: **none calls `buildHookTypeExecutors`, so none seeds the built-ins.** Whether that is itself a defect is triage for the seam's own root item, not a claim to resolve here. Both statements are about reachability, not safety; see the seeding paragraph above for why this section refuses to treat "unexported" as a boundary.
