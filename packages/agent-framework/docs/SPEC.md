...
| `taskState` / `groupState` / `mainThreadState`          | function  | SCREEN-1992: the total mapping onto the five-word `state` (`execution-workspace-state.ts`) |
| `taskHeadline` / `groupHeadline` / `mainThreadHeadline` | function  | SCREEN-1992: the row-text SSOT — `question`, `result` or `activity` by state               |
