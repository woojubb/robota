...
| `consentScopeFor`   | function | Issue #2351: the permission pattern a "don't ask again" answer grants for this invocation — the framework's facade over `agent-session`'s scope rule, so every prompt surface (`agent-transport-tui` reaches it only through this package) prints the same scope |
