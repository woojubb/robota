...
- Provider-neutral output-style prompt composition and live host application. The framework owns the
  additive prompt section and runtime state seam; style discovery and persistence are injected.
...
- **Node-host workspace trust (SECURITY-2465)**: `createNodeWorkspaceIdentityResolver()` resolves the
  ...
  directory. `createNodeWorkspaceTrustStore()` persists only `{ repositoryKey, worktreeRoot, state,
...
- **Provider endpoint/credential ownership (SECURITY-2465)**: settings-layer merges preserve the most
  ...
  therefore cannot redirect a higher-trust secret to a lower-trust endpoint; the CLI diagnostic reports
  only the redacted provenance finding.
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `IInteractiveSession` + named `ISession*` roles                                                                                                      | `@robota-sdk/agent-interface-session` (SSOT; ARCH-012)                                               | Legacy full session interface plus capability-scoped consumer ports; framework supplies the full implementation                                                                                                                                                            |
...
| `IInteractiveSessionRecord`                                                                                                                          | `@robota-sdk/agent-interface-session` (SSOT; DATA-001 — framework only imports + locally re-exports) | Persisted session record shape                                                                                                                                                                                                                                             |
| `IInteractiveSessionStore`                                                                                                                           | `@robota-sdk/agent-interface-session` (SSOT; DATA-001 — framework only imports + locally re-exports) | Session persistence adapter interface                                                                                                                                                                                                                                      |
| `IResumableSessionSummary`                                                                                                                           | `@robota-sdk/agent-interface-session` (SSOT; DATA-001 — framework only imports + locally re-exports) | Lightweight session summary for session picker                                                                                                                                                                                                                             |
...
| `ICommandHostAdapters`                                                                                                                               | `src/command-api/host-adapters.ts`                                                                     | Host-provided adapter bag, including live model-effort resolution/application and the optional secret-free MCP activation lifecycle port                                                                                                                                   |
| `ICommandEffortAdapter`                                                                                                                              | `src/command-api/host-adapters.ts`                                                                     | Live effort selection/application port returning the provider-neutral resolution record                                                                                                                                                                                    |
| `ICommandMCPActivationAdapter`                                                                                                                       | `src/command-api/host-adapters.ts`                                                                     | Secret-free MCP definition status and typed approve/reject/revoke lifecycle port                                                                                                                                                                                           |
| `ICommandMCPActivationSummary`                                                                                                                       | `src/command-api/host-adapters.ts`                                                                     | Public MCP activation projection consumed by command modules                                                                                                                                                                                                               |
| `IOutputStylePrompt`                                                                                                                                 | `src/context/output-style-prompt.ts`                                                                   | Framework-owned prompt projection for an active response style                                                                                                                                                                                                             |
...
| `IModelEffortResolution`                                                                                                                             | `src/effort/effort-resolution.ts`                                                                      | FLOW-008 source-precedence record; its requested selection is separate from provider capability resolution                                                                                                                                                                 |
| `TEffortSelection`                                                                                                                                   | `src/effort/effort-resolution.ts`                                                                      | `auto` or one core-owned model-effort tier                                                                                                                                                                                                                                 |
...
| `IBundlePluginInspection` / `IBundlePluginSkip` / `IBundlePluginHookIssue` / `IBundlePluginMcpServer` / `IBundlePluginMcpFault`                      | `src/plugins/bundle-plugin-types.ts`                                                                   | OBSERVABILITY-1991: the loader's read-only inspection — loaded plugins, skipped ones with reason and manifest path, `HooksSchema` issue paths, typed `.mcp.json` servers (env KEY names only), structural faults                                                           |
| `ISettingsInspection` / `ISettingsLayerInspection` / `ISettingsLayerCause` / `ISettingsKeyProvenance` / `TSettingsLayerState` / `TSettingsMergeRule` | `src/config/settings-inspection.ts`, `src/config/config-merge.ts`                                      | OBSERVABILITY-1991: per-layer state (`absent \| ok \| empty \| unreadable \| invalid-json \| schema-invalid`) with a structured cause (errno, byte offset, issue paths — never file content), the merged view with `partial`, and per-key merge rule + contributing layers |
| `ISkillSourceInspection` / `ISkillRootInspection` / `ISkillSourceSkip` / `TSkillSkipReason`                                                          | `src/commands/skill-source.ts`                                                                         | OBSERVABILITY-1991: what skill/command discovery silently skips per root — missing `SKILL.md`, unreadable, frontmatter missing or unterminated                                                                                                                             |
| `IWorkspaceTrustCause`                                                                                                                               | `src/workspace-trust/types.ts`                                                                         | OBSERVABILITY-1991: the swallowed identity/store error's name and message on a restricted access (`cause?`), so a doctor can name it                                                                                                                                       |
...
| `readAppearanceSettings`                     | function  | SCREEN-2002: the three flat appearance keys, each falling back to its default INDEPENDENTLY                                                                                                                                                                                                                                                                                                                                                   |
| `applyAppearanceSettings`                    | function  | Merge an appearance patch into the settings document and persist it                                                                                                                                                                                                                                                                                                                                                                           |
| `isAppearanceSettingsPatch`                  | function  | Guard: a patch carrying only appearance fields of the right type; the host refuses anything else                                                                                                                                                                                                                                                                                                                                              |
| `DEFAULT_APPEARANCE_SETTINGS`                | const     | What a user who has set nothing gets                                                                                                                                                                                                                                                                                                                                                                                                          |
| `APPEARANCE_SETTINGS_KEYS`                   | const     | The settings-document key names this module owns                                                                                                                                                                                                                                                                                                                                                                                              |
| `IAppearanceSettings`                        | type      | Re-export: theme id, syntax highlighting and reduced motion                                                                                                                                                                                                                                                                                                                                                                                   |
| `TAppearanceSettingsPatch`                   | type      | Re-export: a sparse appearance patch                                                                                                                                                                                                                                                                                                                                                                                                          |
| `ICommandMCPActivationAdapter`               | interface | Host-provided secret-free MCP status and approve/reject/revoke lifecycle port                                                                                                                                                                                                                                                                                                                                                                 |
| `ICommandMCPActivationSummary`               | interface | Secret-free MCP activation status projection consumed by command modules                                                                                                                                                                                                                                                                                                                                                                      |
| `ICommandEffortAdapter`                      | interface | Host-provided live model-effort selection/application port                                                                                                                                                                                                                                                                                                                                                                                    |
| `IOutputStylePrompt`                         | interface | Framework-owned prompt projection for an active provider-neutral response style                                                                                                                                                                                                                                                                                                                                                               |
| `ICommandOutputStyleRegistryAdapter`         | interface | Host-provided list/get adapter for the resolved output-style catalog                                                                                                                                                                                                                                                                                                                                                                          |
| `ICommandOutputStyleSummary`                 | interface | Secret-free output-style discovery projection for command listings and pickers                                                                                                                                                                                                                                                                                                                                                                |
| `IModelEffortResolution`                     | interface | FLOW-008 source-precedence record projected to commands and headless output; it is not the provider application outcome                                                                                                                                                                                                                                                                                                                       |
| `IModelEffortInputs`                         | interface | Boundary inputs resolved by the single provider-neutral effort precedence authority                                                                                                                                                                                                                                                                                                                                                           |
| `TEffortDisposition`                         | type      | Result classification: applied, model-default, clamped, or not-applied                                                                                                                                                                                                                                                                                                                                                                        |
| `TEffortSelection`                           | type      | User selection union: `auto` or a Core-owned concrete `TModelEffort` tier (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`)                                                                                                                                                                                                                                                                                                        |
| `TEffortSource`                              | type      | Precedence source that supplied the selected effort                                                                                                                                                                                                                                                                                                                                                                                           |
| `parseModelEffort`                           | function  | Validates an untrusted CLI, environment, or settings effort value                                                                                                                                                                                                                                                                                                                                                                             |
| `resolveModelEffort`                         | function  | Resolves command, flag, environment, settings, preset, and model-default inputs while retaining `auto` for the provider boundary                                                                                                                                                                                                                                                                                                              |
...
| `createNodeWorkspaceIdentityResolver`        | function  | Node-host Git identity adapter; canonical worktree plus replacement-safe common-directory identity                                                                                                                                                                                                                                                                                                                                            |
...
| `EditCheckpointStore`                        | class     | Checkpoint capture/state bound to one authority; restore/delete additionally require its mutation capability. A manifest is untrusted bytes: before the first mutation, restore re-validates every entry — target inside the project, snapshot a strict descendant of its checkpoint directory — and one invalid entry aborts the whole restore with `EditCheckpointManifestEscapeError` (issue #2076)                                        |
...
| `resolveExecutionAttach`                     | function  | CLI-1994: whether a workspace entry can be ATTACHED to, and what attaching produces — a `switch-session` outcome carrying the forked session id, or a refusal naming the reason (not a fork, terminal task, or a record no longer in the store). Owned beside the projection that offers the `attach` control, so a surface cannot invent a second answer. Attaching is a **view switch, not a merge**.                                       |
| `restoreSessionRecordIntoSession`            | function  | CLI-1994: restore a persisted record — its messages and its assembled system message — into an ALREADY CONSTRUCTED session. The path a subagent runner takes for a fork job in either process: the runner receives only `resumeSessionId`, and this is where the id becomes the conversation. A record the store cannot hand back fails the job rather than starting it empty.                                                                |
...
| `userPaths`                                  | function  | Structured user-local paths under `~/.robota/` (`settings`, `sessions`, `onboarded`, and SCREEN-1993 `history`)                                                                                                                                                                                                                                                                                                                               |
...
| `updateModelInSettings`                      | function  | Update `providers[currentProvider].model` in a settings file; throws `NoCurrentProviderProfileError` (file untouched) when no active profile exists — never writes the loader-rejected legacy `provider` shape (CONFIG-002)                                                                                                                                                                                                                   |
...
| `TSessionResponseFormat`                     | type      | Issue #2056 (CLI-081): the session-level structured-output option — `agent-core`'s `IResponseFormatConfig` (including `json_schema`), routed as provider policy, never as prompt prose                                                                                                                                                                                                                                                        |
| `NoCurrentProviderProfileError`              | class     | CONFIG-002: thrown by `updateModelInSettings` when the settings file names no active provider profile to write the model into; carries `filePath`                                                                                                                                                                                                                                                                                             |
| `loadHostBundlePluginsFromScopes`            | function  | Issue #2487: load bundle plugins from several scope directories (project, then user), most specific first                                                                                                                                                                                                                                                                                                                                     |
| `loadHostBundlePluginInspectionFromScopes`   | function  | OBSERVABILITY-1991: one `IBundlePluginInspection` per scope directory, same order and enablement as `loadHostBundlePluginsFromScopes`; shadowed copies are kept so a diagnostic sees both                                                                                                                                                                                                                                                     |
| `BundlePluginLoader.inspectPluginsSync`      | method    | OBSERVABILITY-1991: discover and classify every plugin (loaded / skipped with reason); `loadPluginsSync()` is its `loaded` projection and logs the skips as before; the classification helpers live in `src/plugins/bundle-plugin-inspection.ts` and type what the loader already parsed (no second read of `.mcp.json`); `hooks/hooks.json` is validated through `HooksSchema` report-only                                                   |
| `inspectSettingsLayers`                      | function  | OBSERVABILITY-1991: the read-only view of every settings layer plus merged provenance; `loadConfig` derives its layers from the same reader and raises the same error at the same layer as before (read-phase errors across all layers first, then the first schema failure)                                                                                                                                                                  |
| `inspectSkillSources`                        | function  | OBSERVABILITY-1991: the read-only counterpart of `SkillCommandSource.getCommands()` over the same four roots, reporting silent skips                                                                                                                                                                                                                                                                                                          |
| `ISettingsInspection`                        | type      | OBSERVABILITY-1991: layers, merged view, `partial`, provenance                                                                                                                                                                                                                                                                                                                                                                                |
| `ISettingsLayerInspection`                   | type      | OBSERVABILITY-1991: one settings layer's state, structured cause and validated document                                                                                                                                                                                                                                                                                                                                                       |
| `ISettingsLayerCause`                        | type      | OBSERVABILITY-1991: errno / byte offset / issue paths — never file content                                                                                                                                                                                                                                                                                                                                                                    |
| `ISettingsSchemaIssue`                       | type      | OBSERVABILITY-1991: one zod issue as `{ path, code }`                                                                                                                                                                                                                                                                                                                                                                                         |
| `ISettingsKeyProvenance`                     | type      | OBSERVABILITY-1991: per-key merge rule and contributing layers                                                                                                                                                                                                                                                                                                                                                                                |
| `TSettingsLayerState`                        | type      | OBSERVABILITY-1991: `absent \| ok \| empty \| unreadable \| invalid-json \| schema-invalid`                                                                                                                                                                                                                                                                                                                                                   |
| `TSettingsMergeRule`                         | type      | OBSERVABILITY-1991: the merge rule a top-level key follows (`SETTINGS_MERGE_RULES` owner: `config-merge.ts`)                                                                                                                                                                                                                                                                                                                                  |
| `TSettings`                                  | type      | The schema-validated settings document before `$ENV:` resolution (`SettingsSchema` inference)                                                                                                                                                                                                                                                                                                                                                 |
| `IBundlePluginInspection`                    | type      | OBSERVABILITY-1991: loaded plugins plus skipped, hook-issue, MCP-server and MCP-fault records                                                                                                                                                                                                                                                                                                                                                 |
| `IBundlePluginSkip`                          | type      | OBSERVABILITY-1991: a discovered plugin that did not load — id, manifest path, reason, sanitized detail                                                                                                                                                                                                                                                                                                                                       |
| `IBundlePluginHookIssue`                     | type      | OBSERVABILITY-1991: a plugin hooks.json the settings `HooksSchema` refuses (issue paths only)                                                                                                                                                                                                                                                                                                                                                 |
| `IBundlePluginMcpServer`                     | type      | OBSERVABILITY-1991: a plugin-declared MCP server — name, transport, command/url, env KEY names                                                                                                                                                                                                                                                                                                                                                |
| `IBundlePluginMcpFault`                      | type      | OBSERVABILITY-1991: a structurally unusable plugin `.mcp.json` (`not-an-object` / `no-servers`; an unparseable file is a `load-failed` skip, not a fault)                                                                                                                                                                                                                                                                                     |
| `TBundlePluginSkipReason`                    | type      | OBSERVABILITY-1991: `manifest-unreadable \| manifest-invalid \| disabled \| load-failed`                                                                                                                                                                                                                                                                                                                                                      |
| `ISkillSourceInspection`                     | type      | OBSERVABILITY-1991: every discovery root's inspection                                                                                                                                                                                                                                                                                                                                                                                         |
| `ISkillRootInspection`                       | type      | OBSERVABILITY-1991: one root — presence, discovered names, silent skips                                                                                                                                                                                                                                                                                                                                                                       |
| `ISkillSourceSkip`                           | type      | OBSERVABILITY-1991: a skipped skill/command path and its reason                                                                                                                                                                                                                                                                                                                                                                               |
| `TSkillSkipReason`                           | type      | OBSERVABILITY-1991: `missing-skill-file \| unreadable \| frontmatter-missing \| frontmatter-unterminated \| frontmatter-invalid` (the last is the case discovery throws on)                                                                                                                                                                                                                                                                   |
| `IWorkspaceTrustCause`                       | type      | OBSERVABILITY-1991: name and message of the swallowed identity/store error on a restricted access                                                                                                                                                                                                                                                                                                                                             |
| `IWorkspaceTrustGrant`                       | type      | FLOW-2006: one recorded trust decision — path, state and generation — read back through the store's optional `listGrants()`                                                                                                                                                                                                                                                                                                                   |
| `ICommandCostBudget`                         | type      | CMD-007 (issue #2058): the persisted `/cost budget` document — `{ monthly }`                                                                                                                                                                                                                                                                                                                                                                  |
...
This table covers TYPE exports as well as runtime ones. The `spec-public-surface` ratchet (`check-spec-public-surface.mjs`) counts RUNTIME exports only (issue #2331), so a type-only export such as `ICreateSessionOptions` is claimed by this table and by nothing mechanical — its row above (issue #2332) is the only thing that records why it survived the removal of its factory.
...
`sessionRequirements` has demand-switch semantics (CMD-008): a composed module that declares a requirement causes `buildCreateSessionOptions()` to enable that facility (`enableAgentRuntime: true`); a declaration never gates registration on the facility being pre-enabled, and a composition with no such module leaves the key absent.
...
`agent-ui-terminal` — and it is agent-framework's OWN type, so exporting it is ownership rather than
...
### Prompt history append (SCREEN-1993)

`InteractiveSession` accepts an optional `promptHistory: IPromptHistoryOptions` (`{ writer, project }`,
the writer an `IPromptHistoryWriter` from `agent-interface-session`; `userPaths().history` names the
user-level file `~/.robota/history.jsonl` the surface composes it over). When present, the execution
controller calls the recorder (`interactive-session-prompt-history.ts`) once per prompt turn, right
after `user_message` is emitted — an injected port at the turn site, not a `user_message` subscriber,
because that event is also emitted for `[remote-control]` notices with no preceding `turn_source`, so a
subscriber could not tell a prompt from a notice. The recorder appends `{ at, sessionId, project, text }`
only when the turn source is `user` and the driver is absent or `OWNER_DRIVER_ID` (a wake, a peer turn
or a remote co-driver's prompt is not the owner's history); `text` is `rawInput ?? input` trimmed — what
was typed, never a skill's synthesized display text — and an entry equal to the last one this session
wrote is suppressed. A failed append never aborts the turn: it is reported once per session as a
system history entry (`Prompt history could not be written: <message>`) and later appends are still
attempted. Absent ⇒ nothing is written. Enablement, the project key and which surfaces write are
surface-owned (agent-cli: interactive TUI only).

`IRestrictedWorkspaceProjectAccess.identity` carries the workspace identity the trust service resolved
before deciding a non-trusted state (every state except `identity-unavailable`), so a consumer that
needs the worktree root reads the one resolution already made instead of resolving again.
...
**Parked prompt → workspace (SCREEN-1992).** The registry retains each parked request's text
(`Allow <tool>?` for a permission, the request title for an ask) and exposes the oldest as
`pending(): IExecutionPendingRequest | undefined`. `InteractiveSession` passes it as the main
thread's `pendingRequest` on every workspace snapshot and emits `execution_workspace_updated` when a
prompt parks or settles, so the main-thread entry reads `needs-input` with a `question` headline
carrying the request text while a prompt waits, and the main-thread detail page lists that question
as its first record. `hasPendingPrompt` is NOT that signal: it is the co-drive input queue ("a turn
is waiting to run"), the opposite meaning.
...
**Output styles (CLI-1988):** `ICommandHostAdapters.outputStyleRegistry` is the injected discovery
port (`listOutputStyles()` / `getOutputStyle(id)`). The `/output-style` command emits only the
`output-style-change` host action; the session validates the resolved style, persists its id through
the settings adapter, and rebuilds the additive `Output Style: <name>` prompt section. The style
section cannot remove framework, project, permission, security, tool, or capability instructions.
...
- `robota doctor` remediation in the message). Corrupt is never treated as missing — both
...
| `IHeadlessRunnerOptions`             | `src/transport-host/headless/headless-runner.ts`             | Options for `createHeadlessRunner`, including optional effort-result projection     |
...
| `createHeadlessRunner`               | function   | Creates a runner with `run(prompt): Promise<number>`; supports text/json/stream-json modes and optional effort-result projection   |
...
- **TUI permission prompt**: `PermissionPrompt.tsx` in `agent-ui-terminal` presents 4 options: **Allow** (once), **Allow always (this session)** (`a` shortcut), **Allow always (this project)** (`p` shortcut), **Deny** (`n`/`d` shortcut). The TUI subscribes to `permission_request`, renders its async queue, and answers through `resolvePermission`.
...
- **Forked system prompt (CLI-1994)**: every persist writes `record.systemPrompt` — the session's ASSEMBLED system message — and until CLI-1994 nothing read it back. `loadSessionRecord` now returns it as `restoredSystemPrompt`, and a session initialised as a FORK applies it: **a fork inherits the parent's assembled system message**, verbatim, rather than rebuilding one from the current files. That is what "a copy of the conversation" means for the prompt half, and it is strictly stronger than inheriting any single prompt section — an output style, when one exists, is a section inside the same string and is inherited for free. A plain resume is not a copy and still rebuilds, so edits to AGENTS.md between sessions reach it. A record written without a `systemPrompt` rebuilds exactly as before; an explicit `systemPrompt` option is the operator replacing the prompt on purpose and still wins.
- **forkSession() — the in-session fork (CLI-1994)**: `InteractiveSession.forkSession({ name? })` writes a **copy** of the live conversation as a NEW session record — a fresh `session_<uuid>`, the messages, the assembled system message, the tool schemas and the full history, under a name that is deliberately NOT the source's (`<source> (fork)` by default) so `resolveSessionIdByIdOrName` cannot match two records. It drops what the startup fork drops: `sandboxSnapshotId`, `goal`, `plan`, `activeBranch`. **A fork is a copy**: from the moment the record is written the two conversations are separate records that never rejoin, and writing the copy leaves the source record byte-identical. The copy is not started here — `/fork` spawns the background job that resumes it, carrying only the returned id, so the conversation never crosses the child-process wire (ARCH-044). `resolveExecutionAttach` is the counterpart: **attaching to a fork is a view switch, not a merge** — the surface points its view at the fork's record, and neither record is read into or written from the other. It refuses, naming the reason, when the entry is not a fork, when the task reached a terminal status, or when the record is gone from the store.
...
- **Legacy compatibility**: legacy flat `provider` settings are rejected by `loadConfig()` (fail-closed, CONFIG-002); `updateModelInSettings` likewise refuses to write them.
...
| `createExecutionWorkspaceSnapshot`      | function  | SDK-owned main-thread/task/group workspace projection; SCREEN-1992: every entry carries `state`, `headline` and (sleeping schedule) `nextFireAt`, derived by the internal `execution-workspace-state.ts` (total status → state table; headline `question` / `result` / `activity`, one bounded line by the preview rule) |
...
`ICommandHostAdapters` is the host-provided adapter bag exposed through `ICommandHostContext.getCommandHostAdapters()`. It includes settings persistence, live model-effort resolution/application, process lifecycle, permission-mode access, plugin-management adapters, and the optional `mcpActivation` port (`ICommandMCPActivationAdapter`). The effort adapter applies a command selection to the live session and returns the typed requested/effective/source/disposition record; an `auto` selection remains `auto` on that session path, and the record does not perform provider-specific capability mapping or claim a provider result. That MCP port exposes only secret-free definition summaries plus approve/reject/revoke operations; it does not expose a client or connection primitive. Command modules may request only the adapter they need; they must not import a concrete CLI/TUI implementation. Status inspection is read-only and must not activate a server.
...
| `consentScopeFor`   | function | Issue #2351: the permission pattern a "don't ask again" answer grants for this invocation — the framework's facade over `agent-session`'s scope rule, so every prompt surface (`agent-ui-terminal` reaches it only through this package) prints the same scope |
...
`IAgentJobSchedules.spawnScheduledWake` accepts an optional `sessionLoop` marker and `sessionLoopId`
...
order `explicit request > selected skill/agent definition > parent selection > core auto`; an inherited
`auto` is omitted from the child override so the child's provider boundary preserves the same default
selection.
