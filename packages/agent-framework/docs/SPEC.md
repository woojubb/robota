...
- **An external event's sender is the grant its verified access token matched.** Opening a grant does
  ...
  builds that verifier from the grant itself, so no caller can pair a grant with a verifier that
  checks someone else. Revoking a grant stops its pending turns and refuses its later events for the
