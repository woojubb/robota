...
- Evals-as-code (SELFHOST-011): the neutral eval-definition/runner surface — `defineEval()`, `runEval(def, runFn)`, and the default `createSessionRunFn(runtime)` (captures a session's `complete`-event `IExecutionResult`). A metric is a pure function over the SSOT `IExecutionResult` (`IMetric`); concrete metrics/datasets are consumer-supplied — NO eval content ships here.
