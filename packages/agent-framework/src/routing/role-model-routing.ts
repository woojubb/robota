...
export function resolveRoleModel(map: TRoleModelMap, roleKey: string): IModelRef | undefined {
  return map[roleKey]?.[0];
}

/** The full ordered fallback chain for a role (primary first, then fallbacks); empty if unmapped. */
export function resolveRoleFallbackChain(map: TRoleModelMap, roleKey: string): IModelRef[] {
  return map[roleKey] ?? [];
}

/**
 * Walk a role's fallback chain, trying each `IModelRef` in order until one succeeds. `run(ref)` performs
 * the actual execution against the given provider+model over the DIP; on a thrown error it advances to
 * the next fallback (an alternate provider AND model). Throws the LAST error if the whole chain is
 * exhausted, or immediately if the chain is empty.
 *
 * `shouldRetry(err)` gates advancing to the next fallback (default: retry ALL errors). A caller that
 * only wants provider/transient failures to fall back — not auth/4xx/config errors that will fail
 * identically on every provider — should pass a predicate; a non-retryable error then rethrows
 * immediately instead of burning the chain and masking the root cause.
 */
export async function runWithRoleFallback<T>(
  chain: IModelRef[],
  run: (ref: IModelRef) => Promise<T>,
