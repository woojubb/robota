...
describe('TC-01: the replay log is reached by missing and by nothing else', () => {
  ...
  it('reports corrupt — NOT a replayed reconstruction — when the snapshot is damaged', async () => {
  ...
  it('reports unsupported for a pre-envelope snapshot rather than replaying around it', async () => {
    const { root, sessionsDir } = await projectFixture();
    const store = await createTrustedProjectSessionStoreFixture(root);
    store.save(record());
    await writeFile(
...
describe('TC-08 / TC-09: unreadable sessions are reported, not dropped', () => {
...
describe('TC-06: resume says why a session came back empty', () => {
  it('reports the outcome instead of a session indistinguishable from a new one', async () => {
    const { root, sessionsDir } = await projectFixture();
    const store = await createTrustedProjectSessionStoreFixture(root);
    store.save(record());
    await writeFile(
  ...
  it('restores a readable session and says so', async () => {
    const { root } = await projectFixture();
    const store = await createTrustedProjectSessionStoreFixture(root);
    store.save(record());
    const restored = loadSessionRecord(store, SESSION_ID, null);
...
describe('TC-10: the project store writes the envelope too', () => {
...
describe('TC-05: the framework write path refuses mid read-modify-write', () => {
  /**
...
describe('TC-01b: a replayed reconstruction is decoded like everything else', () => {
  /**
...
describe('TC-07: a rename that cannot be written down says so', () => {
   ...
   */
  it('throws rather than silently failing when the stored record cannot be read', async () => {
    const { root, sessionsDir } = await projectFixture();
    const store = await createTrustedProjectSessionStoreFixture(root);
    store.save(record());
    await writeFile(
  ...
  it('writes the name when the record is readable', async () => {
    const { root } = await projectFixture();
    const store = await createTrustedProjectSessionStoreFixture(root);
    store.save(record());
