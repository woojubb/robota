...
export const BlockTree: React.FC<IBlockTreeProps> = ({
  ...
  const [stats, setStats] = useState(() =>
    blockCollector
      ? blockCollector.getStats()
      : { total: 0, byType: {}, byState: {}, rootBlocks: 0 },
  );
  ...
  const renderTreeNode = useCallback(
    (treeNode: IBlockTreeNode, level: number = 0): React.ReactNode => {
      const { block, children } = treeNode;
      const isExpanded =
        expandedBlocks.has(block.blockMetadata.id) ?? block.blockMetadata.isExpanded;
          ...
          <div className="flex items-center gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={handleExpandAll}>Expand All</DropdownMenuItem>
                <DropdownMenuItem onClick={handleCollapseAll}>Collapse All</DropdownMenuItem>
                <DropdownMenuSeparator />
      ...
      <ScrollArea className={`flex-1 ${treeHeightClassName}`}>
        <div className="p-2">
          {treeNodes.length === 0 ? (
            ...
            <div className="space-y-1">{treeNodes.map((treeNode) => renderTreeNode(treeNode))}</div>
          )}
        </div>
      </ScrollArea>
    </div>
