...
import type { IToolSchema, TUniversalValue } from '@robota-sdk/agent-core';
import type { IPlaygroundTool } from '../../lib/playground/robota-executor';
import {
  type IToolBlock,
  type IToolContainerBlockProps,
...
function IndividualToolBlock({
  ...
  return (
    <Card
      className={`transition-all duration-200 border ${toolBlock.isActive ? 'border-blue-500 bg-blue-50' : 'border-gray-200'} ${hasErrors ? 'border-red-300 bg-red-50' : ''} ${!toolBlock.isEnabled ? 'opacity-60' : ''}`}
    >
          ...
          <CardContent className="pt-0">
            <div className="space-y-3">
              <div className="space-y-2">
                ...
                {Object.entries(getToolSchema(toolBlock.tool)?.parameters?.properties || {}).map(
                  ([key, paramConfig]) => (
                    <div key={key} className="space-y-1">
                      ...
                      <p className="text-xs text-gray-400">
                        {(paramConfig as TToolSchemaParameter).description}
                      </p>
                    </div>
                  ),
                )}
