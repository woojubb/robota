...
import { GitBranch, Code, RotateCcw, RefreshCw, Play, Zap } from 'lucide-react';
...
export const ExecutionTreeDebug: React.FC<IExecutionTreeDebugProps> = ({
  ...
  const { debugTree, rawBlocks, stats } = useMemo(() => {
    ...
    const realTimeBlocks = allBlocks.filter(
      (block) => 'startTime' in block.blockMetadata || 'executionHierarchy' in block.blockMetadata,
    ...
    const stats = {
      ...
      inProgress: realTimeBlocks.filter((b) => b.blockMetadata.visualState === 'in_progress')
        .length,
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2 text-sm">
                ...
                {stats.inProgress > 0 && (
                  <Badge className="bg-blue-500">{stats.inProgress} active</Badge>
                )}
                          ...
                          {JSON.stringify(
                            {
                              ...
                              content:
                                typeof block.content === 'string'
                                  ? block.content.substring(0, CONTENT_PREVIEW_LENGTH) +
                                    (block.content.length > CONTENT_PREVIEW_LENGTH ? '...' : '')
                                  : block.content
                                    ? JSON.stringify(block.content).substring(
                                        0,
