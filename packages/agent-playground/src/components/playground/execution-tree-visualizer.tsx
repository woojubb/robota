...
import { GitBranch, Activity, RotateCcw } from 'lucide-react';
...
export const ExecutionTreeVisualizer: React.FC<IExecutionTreeVisualizerProps> = ({
  ...
  const executionStats = useMemo(() => {
    const allBlocks = blockCollector.getBlocks();
    const realTimeBlocks = allBlocks.filter(
      (block) => 'startTime' in block.blockMetadata,
    ) as IRealTimeBlockMessage[];
  ...
  const renderTreeNode = useCallback(
    (node: IExecutionTreeNode): React.ReactNode => {
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2 text-sm">
                ...
                {executionStats.error > 0 && (
                  <Badge variant="destructive">{executionStats.error} failed</Badge>
                )}
              </div>

              {/* Controls */}
              <Button variant="outline" size="sm" onClick={handleClearBlocks} className="text-xs">
        ...
        {executionStats.completed > 0 && (
          <CardContent className="pt-0">
            <div className="flex items-center space-x-4 text-xs text-gray-600">
              ...
              <span>Avg Duration: {(executionStats.avgDuration / MS_PER_SECOND).toFixed(1)}s</span>
            </div>
          </CardContent>
        )}
      </Card>
