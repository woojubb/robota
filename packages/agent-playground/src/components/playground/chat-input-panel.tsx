...
export function ChatInputPanel({ onClose }: IChatInputPanelProps) {
  ...
  const { inputState, setValue, sendMessage, sendStreamingMessage, inputRef, canSend } =
    useChatInput();
  ...
  useEffect(() => {
    try {
      ...
      WebLogger.error('Failed to load recent prompts', {
  ...
  const addToRecentPrompts = useCallback((prompt: string) => {
    ...
    setRecentPrompts((current) => {
      ...
      try {
        ...
        WebLogger.error('Failed to save recent prompts', {
          error: error instanceof Error ? error.message : String(error),
        });
      }
  ...
  const handleRecentPromptClick = useCallback(
    (prompt: string) => {
      setValue(prompt);
      if (inputRef.current) {
