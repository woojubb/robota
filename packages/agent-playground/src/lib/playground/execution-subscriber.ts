 ...
 */
function isHierarchicalEventData(
  data: IEventEmitterEventData,
): data is IEventEmitterHierarchicalEventData {
  return (
    'executionLevel' in data &&
    typeof (data as Record<string, unknown>).executionLevel === 'number' &&
    'executionPath' in data &&
    Array.isArray((data as Record<string, unknown>).executionPath)
  );
}
...
export class ExecutionSubscriber {
  ...
  private subscribeToEvents(): void {
    ...
    this.eventEmitter.on(
      EVENT_EMITTER_EVENTS.EXECUTION_COMPLETE,
  ...
  private onExecutionStart(eventData: IEventEmitterEventData): void {
    ...
    if (hierarchicalData.executionLevel <= 1) {
      ...
      const blockMessage: IRealTimeBlockMessage = {
        ...
        content:
          hierarchicalData.executionLevel === 0 ? 'Team execution started' : 'Agent processing...',
        blockMetadata,
      };
