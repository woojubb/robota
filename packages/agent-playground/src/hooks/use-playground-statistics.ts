...
export interface IPlaygroundStatisticsHookResult {
  ...
  // Actions
  recordAction: (
    actionName: TPlaygroundActionName,
...
export function usePlaygroundStatistics(): IPlaygroundStatisticsHookResult {
  ...
  useEffect(() => {
    ...
    const updateStats = () => {
      try {
        ...
        WebLogger.error('usePlaygroundStatistics: Failed to get statistics', {
  ...
  }, [isExecutorReady, executor, state.lastExecutionResult, state.mode, state.isExecuting]);

  /**
   * UI action methods (memoized with useCallback)
   */
  const recordAction = useCallback(
    async (actionName: TPlaygroundActionName, metadata?: Record<string, TUniversalValue>) => {
      if (!isExecutorReady) return;

      try {
        await executor.recordPlaygroundAction(actionName, metadata);
      } catch (error) {
        WebLogger.warn('Failed to record playground action', {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    },
  ...
  const recordBlockCreation = useCallback(
    async (blockType: string, metadata?: Record<string, TUniversalValue>) => {
      if (!isExecutorReady) return;

      try {
        await executor.recordBlockCreation(blockType, metadata);
      } catch (error) {
        WebLogger.warn('Failed to record block creation', {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    },
  ...
  const resetStatistics = useCallback(() => {
    ...
    try {
      ...
      WebLogger.warn('Failed to reset statistics', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }, [isExecutorReady, executor]);
  ...
  const processedStatistics = useMemo((): IPlaygroundStatisticsHookResult => {
    ...
    const isRecentlyActive =
      Date.now() - rawStatistics.lastUpdated.getTime() < RECENTLY_ACTIVE_THRESHOLD_MS;
