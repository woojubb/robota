...
export function useRobotaExecution(): IRobotaExecutionHookReturn {
  ...
  const successRate =
    totalExecutions > 0
      ? (executionHistory.filter((result) => result.success).length / totalExecutions) *
  ...
  const createAgent = useCallback(
    async (config: IPlaygroundAgentConfig) => {
      try {
        ...
        WebLogger.error('createAgent error', {
  ...
  const validateConfiguration = useCallback(
    (config: IPlaygroundAgentConfig): { isValid: boolean; errors: string[] } => {
      const errors: string[] = [];
