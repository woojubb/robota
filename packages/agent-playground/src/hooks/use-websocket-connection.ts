...
export function useWebSocketConnection(): IWebSocketConnectionHookReturn {
  ...
  const disconnect = useCallback(() => {
    ...
    setStatistics((prev) => ({
      ...
      averageConnectionTime:
        prev.totalConnections > 0
          ? (prev.averageConnectionTime * (prev.totalConnections - 1) + connectionDuration) /
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    const delay = Math.min(
      reconnectInterval * Math.pow(2, connectionInfo.connectionAttempts),
