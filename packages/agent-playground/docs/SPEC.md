...
- Allowed: `@robota-sdk/agent-core` (type contracts), `@robota-sdk/agent-remote-client`
  (remote execution), and `@robota-sdk/agent-tools` (`FunctionTool` for built-in tool factories such
  as `current-time`). The code-editor templates and the remote-injection import rewriter do not
  depend on any provider package — they only emit and rewrite provider import-path strings as
  generated code text (the literal paths live in `src/lib/code-generator/provider-templates.ts` and
  the `remote-injection` rewriter), never linking a provider package.
...
- `components/playground/__tests__/project-browser.test.tsx` characterizes project loading,
  sorting/searching, current project display, empty-state creation, create dialog submission, and
  import dialog submission.
- `components/playground/__tests__/agent-configuration-block.test.tsx` characterizes editable
  model settings, invalid-config execution blocking, running-state editing lockout, header
  callbacks, and tab rendering.
...
- `contexts/__tests__/playground-context.test.tsx` characterizes context provider initialization,
  split context hook errors, constructor failures, agent creation, prompt and streaming execution,
  auth/tool/history actions, and the deprecated combined `usePlayground()` hook.
...
- `hooks/__tests__/use-robota-execution.test.tsx` characterizes derived execution state,
  execution readiness blocking, agent creation failure handling, prompt execution, retry,
  streaming accumulation, context result collection, metrics, and clear behavior for the Robota
  execution hook.
