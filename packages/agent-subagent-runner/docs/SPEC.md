...
| `ISubagentWorkerStartPayload`             | interface (local) | this pkg                  | IPC payload for `start` message: taskId, request (including optional typed effort), worktree?, agentDefinition, parentConfig, parentContext, providerProfile, permissionMode, logsDir |
...
| `TResumeSessionStore`                     | type alias | CLI-1994: the session-record store `ISubagentWorkerComposition.openSessionStore` returns, typed FROM the framework function that reads it so it cannot drift from what the consumer accepts. A fork job carries only a `resumeSessionId`; the child opens this store for the PARENT's cwd, restores the record there, and persists new child turns back under the same ID, so the forked conversation never crosses the wire (ARCH-044). A composition that registers no store FAILS such a job rather than starting it empty. |
...
| `ISubagentWorkerAgentDefinitionDto`       | interface  | The wire shape of an `IAgentDefinition` in the worker start message — declared fields only, including enum-validated effort, with no runtime aliasing.                                                                                                                                                                                                                                                                                                                                                                         |
...
### Fork record persistence (CLI-1994 / issue #2675)

For a request with `resumeSessionId`, the worker calls `openSessionStore({ cwd: request.cwd })`
once, constructs the child `Session` with that store and the resumed ID, restores the copied record,
and runs the prompt. The normal `Session.run()` persistence hook writes the new turn back to the
copied record. A request without `resumeSessionId` does not open a store and keeps the existing
transient behavior. The parent runner's IPC payload contains the ID only; messages never cross the
process boundary.
