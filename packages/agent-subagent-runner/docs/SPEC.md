...
| `TResumeSessionStore`                     | type alias | CLI-1994: the session-record store `ISubagentWorkerComposition.openSessionStore` returns, typed FROM the framework function that reads it so it cannot drift from what the consumer accepts. A fork job carries only a `resumeSessionId`; the child opens this store for the PARENT's cwd and restores the record there, so the forked conversation never crosses the wire (ARCH-044). A composition that registers no store FAILS such a job rather than starting it empty. |
