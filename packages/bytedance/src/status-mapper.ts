import type { TProviderMediaResult, IVideoJobSnapshot } from '@robota-sdk/agents';
...
export function mapVideoJobSnapshot(response: IBytedanceVideoTaskResponse): TProviderMediaResult<IVideoJobSnapshot> {
    if (response.id.trim().length === 0) {
        return { ok: false, error: { code: 'PROVIDER_UPSTREAM_ERROR', message: 'Bytedance video job response is missing task id.' } };
    }
...
export function mapVideoStatus(status: string): TProviderMediaResult<IVideoJobSnapshot['status']> {
    ...
    return { ok: false, error: { code: 'PROVIDER_UPSTREAM_ERROR', message: `Unexpected video job status from Bytedance: ${status}` } };
}

/** Maps the initial createVideo status to queued or running. */
export function mapInitialStatus(statusValue: string | undefined): TProviderMediaResult<'queued' | 'running'> {
    ...
    return { ok: false, error: { code: 'PROVIDER_UPSTREAM_ERROR', message: `Unexpected createVideo status from Bytedance: ${statusValue}` } };
}
