...
export class BytedanceProvider implements IVideoGenerationProvider {
    ...
    public async createVideo(request: IVideoGenerationRequest): Promise<TProviderMediaResult<IVideoJobAccepted>> {
        ...
        if (typeof request.seed === 'number') {
            return buildInvalidRequestError('ModelArk Seedance provider does not support seed field in current contract.');
        }
...
function toContentImageUrl(image: IInlineImageInputSource | IUriImageInputSource): TProviderMediaResult<string> {
    if (image.kind === 'uri') {
