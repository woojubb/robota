 ...
 * restart does; asserting through the same instance would pass against the in-memory Maps. A crash
 * does not release the storage root's owner lock cleanly, so each "restarted" instance below opens
 * over a directory whose previous owner already called `close()` — modelling an orderly shutdown
 * before restart. Owner-lock takeover of a genuinely crashed (never-closed) owner is covered
 * separately in `file-store-owner-lock.test.ts`.
 */
const dirs: string[] = [];
...
/** Open, use, and close a `FileStoragePort` for one "process lifetime" against `root`. */
async function withStorage<T>(
  root: string,
  use: (storage: FileStoragePort) => Promise<T>,
): Promise<T> {
...
describe('FileStoragePort survives a restart (DAG-003)', () => {
  it('a DAG run written by one instance is readable by the next', async () => {
    ...
    await withStorage(root, (storage) => storage.createDagRun(dagRun()));

    // Against the defect this is `undefined`: the second instance starts with an empty Map.
    await withStorage(root, async (storage) => {
      expect(await storage.getDagRun('run-1')).toMatchObject({
        dagRunId: 'run-1',
        status: 'running',
      });
    });
  ...
  it('preserves child ancestry across adapter reconstruction', async () => {
    ...
    await withStorage(root, (storage) => storage.createDagRun(dagRun({ lineage })));
    await withStorage(root, async (storage) => {
      expect((await storage.getDagRun('run-1'))?.lineage).toEqual(lineage);
    });
    await withStorage(root, (storage) => storage.createDagRun(dagRun({ dagRunId: 'legacy' })));
    await withStorage(root, async (storage) => {
      expect((await storage.getDagRun('legacy'))?.lineage).toBeUndefined();
    });
  });

  it('a status update survives too, not just the creation', async () => {
    const root = storageRoot();
    await withStorage(root, async (first) => {
      await first.createDagRun(dagRun());
      await first.updateDagRunStatus('run-1', 'success', '2026-08-03T00:01:00.000Z');
    });

    await withStorage(root, async (storage) => {
      const restarted = await storage.getDagRun('run-1');
  ...
  it('a task run survives, with the two fields the sweeper reads', async () => {
    ...
    const root = storageRoot();
    await withStorage(root, (storage) =>
      storage.createTaskRun(taskRun({ leaseUntil: '2026-08-03T00:00:30.000Z' } as Partial<ITaskRun>)),
    );

    await withStorage(root, async (storage) => {
      const restarted = await storage.getTaskRun('task-1');
  ...
  it('listing sees runs written before the restart', async () => {
    ...
    await withStorage(root, async (storage) => {
      const listed = await storage.listDagRuns();
  ...
  it('a task run is listable by its dag run after a restart', async () => {
    ...
    await withStorage(root, async (storage) => {
      const listed = await storage.listTaskRunsByDagRunId('run-1');
  ...
  it('a fresh directory reads as empty rather than throwing', async () => {
    await withStorage(storageRoot(), async (storage) => {
      expect(await storage.listDagRuns()).toEqual([]);
    });
  });
});
...
describe("DAG-001's sweep has something to recover after a restart (DAG-003)", () => {
  it('finds a task abandoned by a process that died', async () => {
    const root = storageRoot();
    await withStorage(root, async (beforeCrash) => {
    ...
    await withStorage(root, async (storage) => {
      const stale = await storage.listStaleRunningTaskRuns('2026-08-03T00:05:00.000Z');
  ...
  it('does not reclaim a task whose lease is still live', async () => {
    ...
    const root = storageRoot();
    await withStorage(root, async (before) => {
...
describe('every task-run mutator reaches the disk (DAG-003)', () => {
  it('the retry attempt count survives — a reset lets a task exceed its limit', async () => {
    ...
    await withStorage(root, async (storage) => {
      expect((await storage.getTaskRun('retried'))?.attempt).toBe(3);
    });
  });
  ...
  it('snapshots and credits survive', async () => {
    ...
    await withStorage(root, async (storage) => {
      const restarted = await storage.getTaskRun('snapshotted');
...
describe('concurrent first access does not revert a write (DAG-003)', () => {
  it('a value written during hydration survives', async () => {
    ...
    await withStorage(root, (storage) => storage.createTaskRun(taskRun({ taskRunId: 't1', attempt: 1 })));

    // A fresh instance, driven concurrently from its very first call.
    await withStorage(root, async (restarted) => {
      await Promise.all([
    ...
    await withStorage(root, async (storage) => {
      expect((await storage.getTaskRun('t1'))?.attempt).toBe(3);
    });
  });
