 ...
 */
import type {
    IAssistantMessage, TUniversalMessageMetadata, ISystemMessage,
    IToolCall, IToolMessage, TUniversalMessage, IUserMessage,
...
export function createAssistantMessage(
    content: string | null,
    options?: { toolCalls?: IToolCall[]; metadata?: TUniversalMessageMetadata; parts?: TUniversalMessagePart[] }
): IAssistantMessage {
...
export function createToolMessage(
    content: string,
    options: { toolCallId: string; name?: string; metadata?: TUniversalMessageMetadata; parts?: TUniversalMessagePart[] }
): IToolMessage {
    const message: IToolMessage = { role: 'tool', content, toolCallId: options.toolCallId, timestamp: new Date() };
    if (options.name) message.name = options.name;
    if (options.metadata) message.metadata = options.metadata;
    if (options.parts) message.parts = options.parts;
    return message;
