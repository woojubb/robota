 ...
 */
import type {
    TUniversalMessageMetadata, TUniversalMessageRole,
    IToolCall, TUniversalMessage, TUniversalMessagePart
} from '../interfaces/messages';
import {
    isSystemMessage, isAssistantMessage, isToolMessage,
    createUserMessage, createAssistantMessage, createSystemMessage, createToolMessage
...
export class SimpleConversationHistory implements IConversationHistory {
    ...
    addUserMessage(content: string, metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void {
        this.addMessage(createUserMessage(content, { ...(metadata && { metadata }), ...(parts && { parts }) }));
    }
    ...
    addAssistantMessage(content: string | null, toolCalls?: IToolCall[], metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void {
        this.addMessage(createAssistantMessage(content, { ...(toolCalls && { toolCalls }), ...(metadata && { metadata }), ...(parts && { parts }) }));
    }

    addSystemMessage(content: string, metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void {
        this.addMessage(createSystemMessage(content, { ...(metadata && { metadata }), ...(parts && { parts }) }));
    }
    ...
    addToolMessageWithId(content: string, toolCallId: string, toolName: string, metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void {
        this.addMessage(createToolMessage(content, { toolCallId, name: toolName, ...(metadata && { metadata }), ...(parts && { parts }) }));
    }

    getMessages(): TUniversalMessage[] { return [...this.messages]; }
    getMessagesByRole(role: TUniversalMessageRole): TUniversalMessage[] { return this.messages.filter(m => m.role === role); }
    getRecentMessages(count: number): TUniversalMessage[] { return this.messages.slice(-count); }
...
export class PersistentSystemConversationHistory implements IConversationHistory {
    ...
    addMessage(message: TUniversalMessage): void { this.history.addMessage(message); }
    addUserMessage(content: string, metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void { this.history.addUserMessage(content, metadata, parts); }
    addAssistantMessage(content: string | null, toolCalls?: IToolCall[], metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void { this.history.addAssistantMessage(content, toolCalls, metadata, parts); }
    addSystemMessage(content: string, metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void { this.history.addSystemMessage(content, metadata, parts); }
    addToolMessageWithId(content: string, toolCallId: string, toolName: string, metadata?: TUniversalMessageMetadata, parts?: TUniversalMessagePart[]): void { this.history.addToolMessageWithId(content, toolCallId, toolName, metadata, parts); }
    getMessages(): TUniversalMessage[] { return this.history.getMessages(); }
    getMessagesByRole(role: TUniversalMessageRole): TUniversalMessage[] { return this.history.getMessagesByRole(role); }
    getRecentMessages(count: number): TUniversalMessage[] { return this.history.getRecentMessages(count); }
    getMessageCount(): number { return this.history.getMessageCount(); }

    clear(): void { this.history.clear(); this.history.addSystemMessage(this.systemPrompt); }
...
export class ConversationSession implements IConversationHistory {
    ...
    getMessagesForAPI(): IProviderApiMessage[] {
        return this.history.getMessages().map(msg => {
            ...
            if (msg.role === 'assistant' && isAssistantMessage(msg) && msg.toolCalls) apiMsg.tool_calls = msg.toolCalls;
            if (msg.role === 'tool' && isToolMessage(msg)) apiMsg.tool_call_id = msg.toolCallId;
            return apiMsg;
        });
    }

    clear(): void { this.history.clear(); this.toolCallIds.clear(); }
}
