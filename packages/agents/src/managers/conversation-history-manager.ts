...
export class ConversationHistory {
    ...
    getConversationSession(conversationId: string): ConversationSession {
        if (!this.conversations.has(conversationId)) {
            ...
            this.conversations.set(conversationId, new ConversationSession(this.maxMessagesPerConversation));
        }
        return this.conversations.get(conversationId)!;
    }

    hasConversation(conversationId: string): boolean { return this.conversations.has(conversationId); }
    ...
    getStats(): { totalConversations: number; conversationIds: string[]; totalMessages: number } {
        ...
        const totalMessages = Array.from(this.conversations.values()).reduce((sum, s) => sum + s.getMessageCount(), 0);
