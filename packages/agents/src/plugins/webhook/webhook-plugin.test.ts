...
import type { IPluginExecutionContext, IPluginExecutionResult, IPluginErrorContext } from '../../abstracts/abstract-plugin';
...
describe('WebhookPlugin', () => {
    ...
    describe('sendCustomWebhook', () => {
        it('should send a custom event webhook', async () => {
            ...
            const body = JSON.parse((fetch as ReturnType<typeof vi.fn>).mock.calls[0][1].body as string) as Record<string, unknown>;
    ...
    describe('payload transformer', () => {
        it('should apply custom payload transformer', async () => {
            ...
            expect(transformer).toHaveBeenCalledWith('custom', expect.objectContaining({ executionId: 'original' }));
