...
describe('ConversationHistoryPlugin', () => {
    ...
    describe('constructor', () => {
        ...
        it('throws on missing storage strategy', () => {
            expect(() => new ConversationHistoryPlugin({ storage: '' as any }))
                .toThrow(ConfigurationError);
        });

        it('throws on invalid storage strategy', () => {
            expect(() => new ConversationHistoryPlugin({ storage: 'redis' as any }))
                .toThrow(ConfigurationError);
        });

        it('throws on non-positive maxConversations', () => {
            expect(() => new ConversationHistoryPlugin({ storage: 'memory', maxConversations: 0 }))
                .toThrow(ConfigurationError);
        });

        it('throws on non-positive maxMessagesPerConversation', () => {
            expect(() => new ConversationHistoryPlugin({ storage: 'memory', maxMessagesPerConversation: -1 }))
                .toThrow(ConfigurationError);
        });

        it('throws when file storage is missing filePath', () => {
            expect(() => new ConversationHistoryPlugin({ storage: 'file', filePath: '' }))
                .toThrow(ConfigurationError);
        });

        it('throws when database storage is missing connectionString', () => {
            expect(() => new ConversationHistoryPlugin({ storage: 'database', connectionString: '' }))
                .toThrow(ConfigurationError);
        });
    });
    ...
    describe('conversation CRUD', () => {
        ...
        it('throws when adding message without active conversation', async () => {
            ...
            await expect(plugin.addMessage(createUserMessage('hello')))
