...
import type { ILimitsPluginOptions } from './limits/types';
...
describe('LimitsPlugin', () => {
    ...
    describe('construction validation', () => {
        ...
        it('should throw when bucketSize is negative for token-bucket', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'token-bucket', bucketSize: -1 })
            ).toThrow('Bucket size must be positive');
        });

        it('should throw when bucketSize is zero for token-bucket', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'token-bucket', bucketSize: 0 })
            ).toThrow('Bucket size must be positive');
        });

        it('should throw when refillRate is negative for token-bucket', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'token-bucket', refillRate: -1 })
            ).toThrow('Refill rate must be non-negative');
        });

        it('should throw when maxRequests is negative', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'fixed-window', maxRequests: -1 })
            ).toThrow('Max requests must be non-negative');
        });

        it('should throw when maxTokens is negative', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'fixed-window', maxTokens: -1 })
            ).toThrow('Max tokens must be non-negative');
        });

        it('should throw when maxCost is negative', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'fixed-window', maxCost: -1 })
            ).toThrow('Max cost must be non-negative');
        });

        it('should throw when tokenCostPer1000 is negative', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'fixed-window', tokenCostPer1000: -1 })
            ).toThrow('Token cost per 1000 must be non-negative');
        });

        it('should throw when timeWindow is non-positive for sliding-window', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'sliding-window', timeWindow: 0 })
            ).toThrow('Time window must be positive');
        });

        it('should throw when timeWindow is non-positive for fixed-window', () => {
            expect(
                () => new LimitsPlugin({ strategy: 'fixed-window', timeWindow: -100 })
            ).toThrow('Time window must be positive');
        });
    });
    ...
    describe('"fixed-window" strategy', () => {
        ...
        it('should throw PluginError when request limit is exceeded', async () => {
            ...
            await expect(plugin.beforeExecution(createContext())).rejects.toThrow(PluginError);
            await expect(plugin.beforeExecution(createContext())).rejects.toThrow('Request limit exceeded');
        });
    ...
    describe('"token-bucket" strategy', () => {
        ...
        it('should throw when bucket is depleted', async () => {
            ...
            await expect(plugin.beforeExecution(createContext())).rejects.toThrow(PluginError);
            await expect(plugin.beforeExecution(createContext())).rejects.toThrow('Token bucket depleted');
        });
    ...
    describe('key derivation', () => {
        ...
        it('should fall back to executionId when userId and sessionId are absent', async () => {
            ...
            await plugin.beforeExecution(createContext({ userId: undefined, sessionId: undefined, executionId: 'eid_1' }));
        ...
        it('should use "default" when no identifiers are present', async () => {
            ...
            await plugin.beforeExecution(createContext({ userId: undefined, sessionId: undefined, executionId: undefined }));
