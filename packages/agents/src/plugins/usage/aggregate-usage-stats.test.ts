...
describe('aggregateUsageStats', () => {
    ...
    it('aggregates multiple entries', () => {
        const entries = [
            ...
            makeEntry({ provider: 'anthropic', model: 'claude-3', success: false, tokensUsed: { input: 200, output: 100, total: 300 } })
        ];
    ...
    it('aggregates by provider', () => {
        const entries = [
            makeEntry({ provider: 'openai', requestCount: 1, tokensUsed: { input: 100, output: 50, total: 150 } }),
            makeEntry({ provider: 'openai', requestCount: 2, tokensUsed: { input: 200, output: 100, total: 300 } }),
            makeEntry({ provider: 'anthropic', requestCount: 1, tokensUsed: { input: 50, output: 25, total: 75 } })
        ];
        const result = aggregateUsageStats(entries);
