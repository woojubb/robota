...
export enum PluginCategory {
    MONITORING = 'monitoring', LOGGING = 'logging', STORAGE = 'storage', NOTIFICATION = 'notification',
    SECURITY = 'security', PERFORMANCE = 'performance', ERROR_HANDLING = 'error_handling',
    LIMITS = 'limits', EVENT_PROCESSING = 'event_processing', CUSTOM = 'custom'
}

const PRIORITY_CRITICAL = 1000; const PRIORITY_HIGH = 800; const PRIORITY_NORMAL = 500; const PRIORITY_LOW = 200; const PRIORITY_MINIMAL = 100;

/** Plugin priority levels */
export enum PluginPriority {
    CRITICAL = PRIORITY_CRITICAL, HIGH = PRIORITY_HIGH, NORMAL = PRIORITY_NORMAL, LOW = PRIORITY_LOW, MINIMAL = PRIORITY_MINIMAL
}

/** Plugin execution context for all plugins */
export interface IPluginExecutionContext {
    executionId?: string; sessionId?: string; userId?: string;
...
export interface IPluginExecutionResult {
    response?: string; content?: string; duration?: number; tokensUsed?: number; toolsExecuted?: number; success?: boolean;
    usage?: { totalTokens?: number; promptTokens?: number; completionTokens?: number };
    toolCalls?: Array<{ id?: string; name?: string; arguments?: Record<string, string | number | boolean>; result?: string | number | boolean | null }>;
    results?: Array<{ id?: string; type?: string; data?: string | number | boolean | null; success?: boolean }>;
    error?: Error;
    metadata?: Record<string, string | number | boolean | Date>;
}
...
export interface IPluginErrorContext {
    action: string; tool?: string; parameters?: TToolParameters; result?: IToolExecutionResult; error?: Error;
    executionId?: string; sessionId?: string; userId?: string; timestamp?: Date; attempt?: number; stack?: string;
    metadata?: Record<string, string | number | boolean>;
}

/** Plugin configuration interface */
export interface IPluginConfig extends IPluginOptions { options?: Record<string, string | number | boolean> }

/** Plugin options that all plugin options should extend */
export interface IPluginOptions {
    enabled?: boolean; category?: PluginCategory; priority?: PluginPriority | number;
    moduleEvents?: TEventName[]; subscribeToAllModuleEvents?: boolean;
}

/** Plugin data interface */
export interface IPluginData {
    name: string; version: string; enabled: boolean; category: PluginCategory;
    priority: number; subscribedEvents: TEventName[]; metadata?: Record<string, string | number | boolean>;
}
...
export interface IPluginContract<TOptions extends IPluginOptions = IPluginOptions, TStats = IPluginStats> {
    name: string; version: string; enabled: boolean; category: PluginCategory; priority: number;
...
export interface IPluginStats {
    enabled: boolean; calls: number; errors: number; lastActivity?: Date; moduleEventsReceived?: number;
