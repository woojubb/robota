...
import {
    IEventService, DEFAULT_ABSTRACT_EVENT_SERVICE, isDefaultEventService,
    IOwnerPathSegment, IAgentEventData, bindWithOwnerPath
...
export class Robota extends AbstractAgent<IAgentConfig, IRunOptions, TUniversalMessage> implements IAgent<IAgentConfig, IRunOptions, TUniversalMessage> {
    ...
    constructor(config: IAgentConfig) {
        ...
        this.eventEmitter = new EventEmitterPlugin({
            enabled: true,
            events: [
                EVENT_EMITTER_EVENTS.MODULE_INITIALIZE_START, EVENT_EMITTER_EVENTS.MODULE_INITIALIZE_COMPLETE,
                EVENT_EMITTER_EVENTS.MODULE_INITIALIZE_ERROR, EVENT_EMITTER_EVENTS.MODULE_EXECUTION_START,
                EVENT_EMITTER_EVENTS.MODULE_EXECUTION_COMPLETE, EVENT_EMITTER_EVENTS.MODULE_EXECUTION_ERROR,
                EVENT_EMITTER_EVENTS.MODULE_DISPOSE_START, EVENT_EMITTER_EVENTS.MODULE_DISPOSE_COMPLETE,
        ...
        this.agentEventService = bindWithOwnerPath(this.eventService, {
            ownerType: 'agent', ownerId: this.conversationId,
    ...
    private initDelegates(): void {
        this.moduleManager = new RobotaModuleManager(
            this.name, this.moduleRegistry, this.logger,
            () => this.isFullyInitialized, () => this.ensureFullyInitialized()
        );
        this.pluginManager = new RobotaPluginManager(
            this.logger, () => this.isFullyInitialized, () => this.executionService
        );
        this.configManager = new RobotaConfigManager(
            this.logger, () => this.aiProviders, () => this.tools,
            () => this.eventService, () => this.isFullyInitialized,
            () => this.ensureFullyInitialized(), () => this.config,
            (c: IAgentConfig) => { this.config = c; },
            () => this.configVersion, () => ++this.configVersion,
            () => this.configUpdatedAt, (t: number) => { this.configUpdatedAt = t; },
    ...
    private emitCreatedEvent(): void {
        ...
        this.emitAgentEvent(AGENT_EVENTS.CREATED, {
            parameters: {
                tools: toolNames, systemMessage: this.config.defaultModel.systemMessage,
                provider: this.config.defaultModel.provider, model: this.config.defaultModel.model,
                temperature: this.config.defaultModel.temperature, maxTokens: this.config.defaultModel.maxTokens
            }
        });
    }
    ...
    private executionDeps(): IRobotaExecutionDeps {
        return {
            conversationId: this.conversationId, config: this.config, logger: this.logger,
            getHistory: () => this.getHistory(), getExecutionService: () => this.executionService,
    ...
    async updateTools(next: Array<IToolWithEventService>): Promise<{ version: number }> { return this.configManager.updateTools(next); }
    async updateConfiguration(patch: Partial<IAgentConfig>): Promise<{ version: number }> { return this.configManager.updateConfiguration(patch); }
    async getConfiguration(): Promise<{ version: number; tools: Array<{ name: string; parameters?: string[] }>; updatedAt: number }> { return this.configManager.getConfiguration(); }
    setModel(mc: { provider: string; model: string; temperature?: number; maxTokens?: number; topP?: number; systemMessage?: string }): void { this.configManager.setModel(mc); }
    getModel(): { provider: string; model: string; temperature?: number; maxTokens?: number; topP?: number; systemMessage?: string } { return this.configManager.getModel(); }
    registerTool(tool: AbstractTool): void { this.configManager.registerTool(tool, this.tools); }
    unregisterTool(toolName: string): void { this.tools.removeTool(toolName); }
    getConfig(): IAgentConfig { return { ...this.config }; }

    // --- Plugins (delegated) ---

    addPlugin(plugin: IPluginContract<IPluginOptions, IPluginStats> & IPluginHooks): void { this.pluginManager.addPlugin(plugin); }
    removePlugin(pluginName: string): boolean { return this.pluginManager.removePlugin(pluginName); }
    getPlugin(pluginName: string): (IPluginContract<IPluginOptions, IPluginStats> & IPluginHooks) | undefined { return this.pluginManager.getPlugin(pluginName); }
    getPlugins(): Array<IPluginContract<IPluginOptions, IPluginStats> & IPluginHooks> { return this.pluginManager.getPlugins(); }
    getPluginNames(): string[] { return this.pluginManager.getPluginNames(); }

    // --- Modules (delegated) ---

    async registerModule(module: IModule, options?: { autoInitialize?: boolean; validateDependencies?: boolean }): Promise<void> { return this.moduleManager.registerModule(module, options); }
    async unregisterModule(moduleName: string): Promise<boolean> { return this.moduleManager.unregisterModule(moduleName); }
    getModule(moduleName: string): IModule | undefined { return this.moduleManager.getModule(moduleName); }
    getModulesByType(moduleType: string): IModule[] { return this.moduleManager.getModulesByType(moduleType); }
    getModules(): IModule[] { return this.moduleManager.getModules(); }
    getModuleNames(): string[] { return this.moduleManager.getModuleNames(); }
    hasModule(moduleName: string): boolean { return this.moduleManager.hasModule(moduleName); }
    async executeModule(moduleName: string, context: { executionId?: string; sessionId?: string; userId?: string; metadata?: Record<string, string | number | boolean | Date> }): Promise<{ success: boolean; data?: IModuleResultData; error?: Error; duration?: number }> { return this.moduleManager.executeModule(moduleName, context); }
    getModuleStats(moduleName: string): { totalExecutions: number; successfulExecutions: number; failedExecutions: number; averageExecutionTime: number; lastExecutionTime?: Date } | undefined { return this.moduleManager.getModuleStats(moduleName); }

    // --- Stats & Lifecycle ---

    getStats() {
        return buildAgentStats({
            name: this.name, version: this.version, conversationId: this.conversationId,
            startTime: this.startTime, isFullyInitialized: this.isFullyInitialized,
            aiProviders: this.aiProviders, tools: this.tools,
            getPluginNames: () => this.getPluginNames(), getModuleNames: () => this.getModuleNames(),
    ...
    async destroy(): Promise<void> {
        await destroyAgent({
            name: this.name, isFullyInitialized: this.isFullyInitialized,
            moduleRegistry: this.moduleRegistry, eventEmitter: this.eventEmitter,
            executionService: this.executionService, logger: this.logger,
            resetState: () => { this.isFullyInitialized = false; this.initializationPromise = undefined as Promise<void> | undefined; }
        });
    }

    // --- Initialization ---

    protected override async initialize(): Promise<void> { await this.ensureFullyInitialized(); }
    ...
    private async doAsyncInit(): Promise<void> {
        try {
            this.executionService = await performAsyncInitialization({
                config: this.config, aiProviders: this.aiProviders, tools: this.tools,
                agentFactory: this.agentFactory, conversationHistory: this.conversationHistory,
                moduleRegistry: this.moduleRegistry, eventEmitter: this.eventEmitter,
                eventService: this.eventService, logger: this.logger
            });
    ...
    private emitAgentEvent(eventType: string, data: Omit<IAgentEventData, 'timestamp'>): void {
        ...
        this.agentEventService.emit(eventType, { timestamp: new Date(), ...data }, {
            ownerType: AGENT_EVENT_PREFIX, ownerId: this.conversationId,
            ownerPath: this.buildOwnerPath(this.config.executionContext)
        });
    }
