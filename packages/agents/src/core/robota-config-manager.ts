...
export function validateAgentConfig(config: IAgentConfig): void {
    ...
    if (!providerNames.includes(config.defaultModel.provider)) {
        throw new ConfigurationError(
            ...
            { component: 'Robota', defaultProvider: config.defaultModel.provider, availableProviders: providerNames }
        );
    }
}
...
export class RobotaConfigManager {
    ...
    async getConfiguration(): Promise<{ version: number; tools: Array<{ name: string; parameters?: string[] }>; updatedAt: number; metadata?: TAgentStatsMetadata }> {
        await this.ensureReady();
        ...
        return { version: this.getConfigVersion(), tools, updatedAt: this.getConfigUpdatedAt(), metadata: undefined };
    }
    ...
    getModel(): {
        ...
        return {
            ...
            ...(config.defaultModel.systemMessage !== undefined && { systemMessage: config.defaultModel.systemMessage })
        };
    }
