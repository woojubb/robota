...
import type { IAgentEventData } from '../services/event-service';
...
export async function robotaRun(
    ...
    try {
        ...
        const result = await deps.getExecutionService().execute(
            input,
            messages,
            executionConfig,
            {
...
export async function* robotaRunStream(
    ...
    try {
        ...
        const stream = deps.getExecutionService().executeStream(
            input,
            messages,
            executionConfig,
            {
