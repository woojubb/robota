...
import type { IEventService, IBaseEventData } from '../services/event-service';
...
describe('ExecutionProxy', () => {
    ...
    describe('wrap', () => {
        it('wraps target and emits start/complete events', async () => {
            const target = {
                async myMethod(arg: string) { return `result-${arg}`; }
            };
        ...
        it('emits error event on method failure', async () => {
            const target = {
                async myMethod() { throw new Error('boom'); }
            };
            const proxy = new ExecutionProxy<typeof target>({
                eventService,
                sourceType: 'agent',
        ...
        it('uses extractMetadata and extractResult when configured', async () => {
            const target = { async compute(x: number) { return { value: x * 2 }; } };
            const proxy = new ExecutionProxy<typeof target>({
                eventService,
                sourceType: 'agent',
                sourceId: 'test'
...
describe('createExecutionProxy', () => {
    it('creates a proxy with standard methods', async () => {
        ...
        const target = {
            async run(input: string) { return `response: ${input}`; }
        };
