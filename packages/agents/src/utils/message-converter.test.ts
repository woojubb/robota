...
import type { TUniversalMessage, IUserMessage, IAssistantMessage, ISystemMessage, IToolMessage } from '../interfaces/messages';

function makeUserMessage(content: string): IUserMessage {
    return { role: 'user', content, timestamp: new Date() };
}

function makeAssistantMessage(content: string | null, toolCalls?: IAssistantMessage['toolCalls']): IAssistantMessage {
...
describe('MessageConverter', () => {
    describe('toProviderFormat - OpenAI', () => {
        it('should convert basic messages to OpenAI format', () => {
            ...
            expect(result[0]).toEqual(expect.objectContaining({ role: 'system', content: 'You are helpful.' }));
            expect(result[1]).toEqual(expect.objectContaining({ role: 'user', content: 'Hello' }));
            expect(result[2]).toEqual(expect.objectContaining({ role: 'assistant', content: 'Hi there!' }));
        });
        ...
        it('should include tool_calls for assistant messages with toolCalls', () => {
            ...
            const messages: TUniversalMessage[] = [
                makeAssistantMessage(null, toolCalls),
            ];
        ...
        it('should include tool_call_id for tool messages', () => {
            const messages: TUniversalMessage[] = [
                makeToolMessage('result data', 'call_1'),
            ];
    ...
    describe('toProviderFormat - Anthropic', () => {
        ...
        it('should map tool role to user', () => {
            const messages: TUniversalMessage[] = [
                makeToolMessage('tool result', 'call_1'),
            ];
    ...
    describe('toProviderFormat - Google', () => {
        it('should map assistant role to model', () => {
            const messages: TUniversalMessage[] = [
                makeAssistantMessage('model response'),
            ];
