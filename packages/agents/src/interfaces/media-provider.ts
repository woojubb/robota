...
export function isImageGenerationProvider(provider: object): provider is IImageGenerationProvider {
    return (
        ...
        && typeof (provider as { generateImage?: (...args: never[]) => Promise<TProviderMediaResult<IImageGenerationResult>> }).generateImage === 'function'
    );
}
...
export function isVideoGenerationProvider(provider: object): provider is IVideoGenerationProvider {
    return (
        ...
        && typeof (provider as { createVideo?: (...args: never[]) => Promise<TProviderMediaResult<IVideoJobAccepted>> }).createVideo === 'function'
        && typeof (provider as { getVideoJob?: (...args: never[]) => Promise<TProviderMediaResult<IVideoJobSnapshot>> }).getVideoJob === 'function'
        && typeof (provider as { cancelVideoJob?: (...args: never[]) => Promise<TProviderMediaResult<IVideoJobSnapshot>> }).cancelVideoJob === 'function'
    );
}
