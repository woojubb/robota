...
export async function* executeStream(
    ...
    const { aiProviders, tools, conversationHistory, toolExecutionService, plugins, logger, eventEmitter } = deps;
    ...
    try {
        const conversationSession =
            conversationHistory.getConversationSession(
        ...
        await callPluginHook(plugins, 'beforeRun', {
            input,
            ...(context?.metadata
        ...
        if (!provider) {
            throw new Error(
                `AI provider '${currentInfo.provider}' not found`
            );
        }
        ...
        const configToolsLength = Array.isArray(config.tools)
            ? config.tools.length
            : undefined;
        ...
        const toolSchemasLength = Array.isArray(toolSchemas)
            ? toolSchemas.length
            : undefined;
        ...
        const chatOptions: IChatOptions = {
            ...
            ...(config.tools &&
                config.tools.length > 0 && { tools: tools.getTools() }),
        };

        logger.debug(
            '[EXECUTION-SERVICE] Final chatOptions has tools:',
        ...
        const chatOptionsToolsLength = Array.isArray(chatOptions.tools)
            ...
            : undefined;
        logger.debug(
            '[EXECUTION-SERVICE] Final chatOptions.tools length:',
        ...
        for await (const chunk of stream) {
            ...
            if (chunk.role === 'assistant') {
                ...
                if (
                    Array.isArray(assistantChunk.toolCalls) &&
                    assistantChunk.toolCalls.length > 0
                ) {
                    for (const chunkToolCall of assistantChunk.toolCalls) {
                        if (chunkToolCall.id && chunkToolCall.id !== '') {
                            if (
                                !chunkToolCall.type ||
                                chunkToolCall.type.length === 0
                            ) {
                                throw new Error(
                            ...
                            if (
                                typeof chunkToolCall.function.arguments !==
                                'string'
                            ) {
                                throw new Error(
                            ...
                            const hasNameFragment =
                                typeof chunkToolCall.function?.name ===
                                    'string' &&
                                chunkToolCall.function.name.length > 0;
        ...
        logger.debug(
            '[EXECUTION-SERVICE-STREAM] Stream completed, toolCalls detected:',
            { count: toolCalls.length }
        );

        if (typeof fullResponse !== 'string') {
            throw new Error(
                '[EXECUTION] Streaming response content is required'
            );
        }
        ...
        await callPluginHook(plugins, 'afterRun', {
            input,
        ...
        await callPluginHook(plugins, 'onError', {
            input,
            error:
...
async function* executeStreamToolCalls(
    toolCalls: IToolCall[],
    conversationSession: { addToolMessageWithId: (content: string, toolCallId: string, toolName: string, metadata?: Record<string, string | number | boolean>) => void },
    ...
    logger.debug(
        '[EXECUTION-SERVICE-STREAM] Executing tools:',
    ...
    const streamingOwnerPathBase = [
        ...eventEmitter.buildExecutionOwnerContext(
            streamingRootId,
            executionId
        ).ownerPath,
    ...
    const toolRequests =
        toolExecutionService.createExecutionRequestsWithContext(
            toolCalls,
    ...
    const toolSummary =
        await toolExecutionService.executeTools(toolContext);

    for (const toolCall of toolCalls) {
        if (!toolCall.id) {
            throw new Error(
                '[EXECUTION] Tool call missing id in streaming mode'
            );
        }
        if (
        ...
        const result = toolSummary.results.find(
            (r) => r.executionId === toolCall.id
        );
        ...
        if (result && result.success) {
            if (typeof result.result === 'undefined') {
                throw new Error(
                    '[EXECUTION] Tool result missing result payload in streaming mode'
                );
            }
            content =
            ...
            if (!execMessage || execMessage.length === 0) {
                throw new Error(
                    '[EXECUTION] Tool execution error missing message in streaming mode'
                );
            }
    ...
    const streamingToolCallIds = toolCalls.map((toolCall) => {
        if (!toolCall.id || toolCall.id.length === 0) {
            throw new Error(
                '[EXECUTION] Tool call missing id for streaming tool results ready payload'
            );
        }
        return toolCall.id;
    });
    if (streamingToolCallIds.length === 0) {
        throw new Error(
            '[EXECUTION] Tool results ready requires toolCallIds in streaming mode'
        );
    }
