...
function validateReadmeFiles() {
  ...
  console.log(
    `${colors.blue}📂 Found ${packages.length} package(s): ${packages.join(', ')}${colors.reset}`,
  );
  ...
  for (const packageName of packages) {
    ...
    if (fs.existsSync(readmePath)) {
      ...
      if (stats.size === 0) {
        console.warn(
          `${colors.yellow}⚠️  README.md exists but is empty in packages/${packageName}${colors.reset}`,
        );
  ...
  if (missingReadmes.length > 0) {
    ...
    console.error(
      `${colors.red}The following packages are missing or have invalid README.md files:${colors.reset}`,
    );
    ...
    console.error(
      `${colors.yellow}1. Ensure each package has a README.md file in its root directory${colors.reset}`,
    );
    console.error(
      `${colors.yellow}2. If the package has docs/README.md, copy it to the package root${colors.reset}`,
    );
    console.error(
      `${colors.yellow}3. For deprecated packages, create a simple README.md with migration instructions${colors.reset}`,
    );
