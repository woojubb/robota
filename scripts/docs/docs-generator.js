...
// IMPORTANT: Keep this as the single source of truth for generated docs headers.
const DOCS_GENERATION_COMMAND = 'pnpm typedoc:convert';
const AUTO_GENERATED_WARNING = `<!--
⚠️ AUTO-GENERATED FILE - DO NOT EDIT MANUALLY

Generated by: scripts/docs/docs-generator.js
Command: ${DOCS_GENERATION_COMMAND}
...
// Dynamically discover API categories from packages directory
function getApiCategories() {
    ...
    for (const packageName of packageDirs) {
        ...
        const entryPoint = path.join(srcPath, 'index.ts');

        // Check if package has src/index.ts entry point
        if (fs.existsSync(entryPoint)) {
            categories.push({
                name: packageName,
                pattern: `packages/${packageName}/src/**/*.ts`,
                entryPoint: entryPoint
            });
            console.log(`📦 Found package: ${packageName}`);
        } else {
            console.log(`⚠️ Skipping ${packageName}: no src/index.ts found`);
        }
    }
...
function generateApiIndexPage(apiCategories) {
...
${apiCategories.map(category => `- [${category.name}](${category.name.toLowerCase()}/)`).join('\n')}
`;
...
function fixFileStructure(categoryDir, categoryName) {
    ...
    // For first package (agents), move project README.md to parent folder
    if (categoryName === 'agents' && fs.existsSync(readmePath)) {
...
// Fix TypeScript generic syntax that Vue/VitePress might interpret as HTML tags
function fixTypeScriptGenerics(content) {
    // Replace Promise<Type> with Promise\<Type\> to escape angle brackets
    content = content.replace(/Promise<([^>]+)>/g, 'Promise\\<$1\\>');

    // Replace other common generic patterns
    content = content.replace(/Array<([^>]+)>/g, 'Array\\<$1\\>');
    content = content.replace(/Record<([^>]+)>/g, 'Record\\<$1\\>');
    content = content.replace(/Partial<([^>]+)>/g, 'Partial\\<$1\\>');
    content = content.replace(/Required<([^>]+)>/g, 'Required\\<$1\\>');
    content = content.replace(/Readonly<([^>]+)>/g, 'Readonly\\<$1\\>');

    // Fix generic function signatures like Function<T>
    ...
    for (const mdFile of mdFiles) {
        try {
            ...
            if (!content.includes('AUTO-GENERATED FILE')) {
                // Fix TypeScript generics that might be interpreted as HTML
                content = fixTypeScriptGenerics(content);
...
async function main() {
    ...
    // Dynamically discover API categories
    const apiCategories = getApiCategories();
    console.log(`📋 Found ${apiCategories.length} packages with API documentation`);

    // Generate API index page with discovered packages
    generateApiIndexPage(apiCategories);
