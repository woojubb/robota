...
function copyReadmeFiles() {
  ...
  const docsPackages = fs
    .readdirSync(DOCS_PACKAGES_DIR, { withFileTypes: true })
...
if (command === 'copy') {
  ...
  console.log('Usage:');
  console.log(
    '  node scripts/copy-readme.cjs copy    # Copy README files from docs/packages to packages',
  );
