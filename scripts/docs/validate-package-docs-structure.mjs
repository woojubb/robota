...
async function validatePackageDocumentation(packagePath) {
  ...
  const hasDocsIndex = docsEntries.some(
    (entry) => entry.isFile() && entry.name === DOCS_INDEX_BASENAME,
  );
  if (!hasDocsIndex) {
    violations.push(
      `[docs-index] ${path.join(relativePackagePath, 'docs', DOCS_INDEX_BASENAME)} is required.`,
    );
  }
...
async function main() {
  const packageDirectories = (await listWorkspaceScopes()).map((scope) =>
    path.join(WORKSPACE_ROOT, scope.relativeDir),
  );
  const violations = [];
