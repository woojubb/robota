...
import { parseCatalogue, parsePriorGateMap } from './gate-catalogue.mjs';
...
// Document parsing and evidence rendering live in gate-document.mjs.
// Catalogue parsing lives in gate-catalogue.mjs.
  ...
  return [
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        // `AUTO_GENERATED_CHURN` is regenerated by any scan run — this judgement included — and the
        // pre-commit hook refuses to stage it, so it can never be absent from a committable tree.
        // Its owner already exempts it for receipts and pre-push; this criterion does too (#2376).
        const outside = paths.filter(
          (file) =>
            !allowed.has(file) &&
            !file.startsWith(PLAN_LEDGER_DIR) &&
            !AUTO_GENERATED_CHURN.has(file),
        );
