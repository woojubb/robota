...
export function extractClaims(source, file = '(fixture)') {
  ...
  for (const [index, line] of String(source).split('\n').entries()) {
    ...
    if (!match) continue;
    const subject = normalize(line.slice(0, match.index).replace(/^\s*(?:[-*+]\s+|\d+[.)]\s+)/, ''));
    const predicate = normalize(
      line.slice(match.index + match[0].length).replace(/\s+allow-rule-contradiction:\s*\S.*$/i, ''),
    );
...
function ruleFiles(root) {
  ...
  const files = readdirSync(directory, { withFileTypes: true })
    ...
    .sort();
  if (files.length === 0) throw new Error(`rule-contradictions: ${RULES_DIR} has no rule documents.`);
  return files;
}
...
function main() {
  ...
  console.log(`rule-contradictions scan passed (${result.claims} claims; ${result.suppressed.length} suppressed pair(s)).`);
}
