...
export function findReusableVerification({ baseRef, stages, updates, root }) {
  try {
    ...
    const expected = computeVerificationIdentity({ baseRef, stages, root, headRef: currentHead });
    if (receiptMatches(receipt, expected)) {
      return { reusable: true, headCommit: currentHead, verifiedHeadCommit: currentHead };
    }
