...
// `HARNESS_BASE_REF` is projected into a shell assignment solely so the existing Bash guard can
// apply the trusted-base policy it owns. Keep that projection to characters that are inert in an
// unquoted assignment word. Git permits ref names such as `agreement-2664;#`; interpolating one
// would turn the suffix into shell syntax and could hide the synthetic `git push` from the guard.
const SHELL_ASSIGNMENT_VALUE_RE = /^[A-Za-z0-9_./@%+=,:-]+$/;
...
export function runPostVerdictGuard({
  ...
  if (
    ...
    declaredBase.length > 0 &&
    !SHELL_ASSIGNMENT_VALUE_RE.test(declaredBase)
  ) {
    process.stderr.write(
      '[pre-push] Blocked: HARNESS_BASE_REF cannot be projected safely into the guard command.\n',
    );
    return false;
  }
...
export function assertTreePrerequisitesFor() {
  const required = prerequisitesFor();
  const result = checkTreePrerequisites('the pre-push gate', WORKSPACE_ROOT, required);
  if (result.ok) {
    process.stdout.write(
      '▶ build output not required: pre-push runs local planning and formatting only\n',
    );
    return;
  }
