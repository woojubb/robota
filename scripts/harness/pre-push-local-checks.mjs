...
import { isAutoGeneratedChurn, realDirtyLines } from './verification-receipt-storage.mjs';

const TRUSTED_INTEGRATION_BASE_REF = /^origin\/integration\/agreement-[0-9]+$/;
...
export function runPostVerdictGuard({
  ...
  const baseRef = env.HARNESS_BASE_REF?.trim();
  if (baseRef && !TRUSTED_INTEGRATION_BASE_REF.test(baseRef)) {
  ...
  const payload = JSON.stringify({
    tool_name: 'Bash',
    cwd,
    tool_input: { command: baseRef ? `HARNESS_BASE_REF=${baseRef} git push` : 'git push' },
  });
