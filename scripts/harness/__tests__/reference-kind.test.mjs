...
describe('the exemptions', () => {
  ...
  it('leaves a verbatim-quoted instruction alone', () => {
    ...
    expect(
      unqualifiedReferenceCount(
        '**Instruction (verbatim):** "#2004, #1990, #1994, #2054 모두 승인"',
      ),
    ).toBe(0);
  });

  it('still reports a bare reference this document makes outside a verbatim quote', () => {
    expect(
      unqualifiedReferences('**Instruction (verbatim):** "approve #2004"\nsee also #1884').map(
        (f) => f.number,
      ),
    ).toEqual([1884]);
  });
