...
describe('rule-to-rule normative contradiction detection', () => {
  ...
  it('requires a non-empty, same-line suppression reason', () => {
    const claims = [
      ...extractClaims('- **Workspace access** MUST use the policy. allow-rule-contradiction: legacy wording\n', 'a.md'),
      ...extractClaims('- **Workspace access** MAY use the policy.\n', 'b.md'),
    ];
    expect(findContradictions(claims)[0].suppressed).toBe(true);
    expect(extractClaims('- **Workspace access** MUST use the policy. allow-rule-contradiction:\n', 'a.md')[0].suppressed).toBe(false);
  });
