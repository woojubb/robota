...
describe('canonical recommendation decision projection', () => {
  ...
  it('rejects unknown visible H2 owners and content before the title', () => {
    expect(() =>
      decisionProjection(
    ...
    ).toThrow(/before the title/i);
    expect(
      decisionProjectionDigest(
        spec().replace('## Problem\n', 'Non-planning linkage after title.\n\n## Problem\n'),
      ),
    ).toBe(decisionProjectionDigest(spec()));
    expect(() =>
      decisionProjection(
        spec().replace(
          '# INFRA-999: recommendation proof\n',
...
describe('persisted endorsement and immutable adoption', () => {
  it('accepts a current persisted ENDORSE and unchanged historical bytes', () => {
    ...
    expect(currentRecommendationEndorsement(root, SUBJECT, spec()).ok).toBe(true);
    expect(examinedRecommendationEndorsementCount()).toBe(2);
    expect(findRecommendationEndorsementFindings(root)).toEqual([]);
    expect(examinedRecommendationEndorsementCount()).toBe(2);
  });

  it('does not let an unchanged adoption exemption satisfy current GATE-APPROVAL endorsement', () => {
    const { root } = repository();
    const historical = readFileSync(
      path.join(root, '.agents/spec-docs/done/INFRA-001-historical.md'),
      'utf8',
    );
    expect(findRecommendationEndorsementFindings(root)).toEqual([]);
    expect(
      currentRecommendationEndorsement(root, 'INFRA-001-historical.md', historical),
  ...
  it('accepts immutable bootstrap and persisted evidence in a squash-only fresh clone', () => {
    ...
    write(
      ...
      `${JSON.stringify(
        {
          adoptionRevision,
          bootstrap: bootstrap(removedTopicRevision, digest),
        },
        null,
        2,
      )}\n`,
  ...
  it('accepts a reviewed topic commit that exists beside, not beneath, the squash landing', () => {
    ...
    write(
      ...
      `${JSON.stringify(
        {
          adoptionRevision,
          bootstrap: bootstrap(reviewedRevision, digest),
        },
        null,
        2,
      )}\n`,
      ...
      `${JSON.stringify(
        {
          ...
          bootstrap: {
            ...
            reviewedRevision,
            projectionDigest: 'a'.repeat(64),
            endorsementKey: recommendationEndorsementKey(SUBJECT, 'a'.repeat(64)),
          },
        },
        null,
        2,
...
describe('topic ordering', () => {
  ...
  it('keeps the checkpoint valid after a clean rebase rewrites every topic commit SHA', () => {
    const { root } = endorsedTopic();
    const oldReviewedRevision = JSON.parse(readFileSync(path.join(root, LEDGER), 'utf8')).extensions
      .recommendationReview.observations[0].revision;
    ...
    expect(git(root, ['log', '-1', '--format=%H', '--grep', 'reviewed plan'])).not.toBe(
      oldReviewedRevision,
    );
    expect(findRecommendationTopicFindings(root, rebasedBase)).toEqual([]);
  });

  it('classifies exactly the two valid committed/staged checkpoints and kills an always-true mutant', () => {
    const committedValid = endorsedTopic();
    const validCommit = git(committedValid.root, [
      'log',
      '-1',
      '--format=%H',
      '--grep',
    ...
    const validParent = git(committedValid.root, ['rev-parse', `${validCommit}^`]);
    const results = [
      isCommittedRecommendationCheckpoint(
        committedValid.root,
        validParent,
        validCommit,
        changedPaths(committedValid.root, validParent, validCommit),
      ),
    ];

    for (const mixed of [false, true]) {
      const fixture = reviewedTopic();
      write(
        fixture.root,
        LEDGER,
        `${JSON.stringify(attestation({ digest: fixture.digest, revision: fixture.revision }))}\n`,
      );
      if (mixed) {
        write(fixture.root, TASK, `${task()}\nRecommendation review recorded.\n`);
        write(
          fixture.root,
          ACTIVE_SPEC,
          spec({ evidence: 'Recommendation endorsement checkpoint recorded.' }),
        );
        write(fixture.root, 'scripts/harness/mixed.mjs', 'export const mixed = true;\n');
      }
      const commitRevision = commit(fixture.root, mixed ? 'mixed checkpoint' : 'ledger only');
      results.push(
        isCommittedRecommendationCheckpoint(
          fixture.root,
          `${commitRevision}^`,
          commitRevision,
          changedPaths(fixture.root, `${commitRevision}^`, commitRevision),
        ),
      );
    }

    for (const mode of ['valid', 'ledger-only', 'mixed']) {
      const fixture = reviewedTopic();
      write(
        fixture.root,
        LEDGER,
      ...
      const paths = git(fixture.root, ['diff', '--cached', '--name-only', '--no-renames'])
        .split('\n')
        .filter(Boolean);
      results.push(isStagedRecommendationCheckpoint(fixture.root, paths));
    }
  ...
  it('seeds the exact bootstrap digest during replay and invalidates only a later projection', () => {
    ...
    write(
      ...
      `${JSON.stringify(
        {
          adoptionRevision,
          bootstrap: bootstrap(revision, digest),
        },
        null,
        2,
      )}\n`,
