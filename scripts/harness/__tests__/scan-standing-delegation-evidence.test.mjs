...
describe('frozen disposition categories', () => {
  ...
  it('keeps the immutable manifest enforced after the current baseline shrinks to zero', () => {
    ...
    const movedRelative = 'active/FIX-001-legacy.md';
    const movedSpecPath = path.join(root, '.agents/spec-docs', movedRelative);
    mkdirSync(path.dirname(movedSpecPath), { recursive: true });
    renameSync(specPath, movedSpecPath);
    writeFileSync(baselinePath, `${JSON.stringify({ exempt: [movedRelative] }, null, 2)}\n`);
    expect(findEvidenceFindings(root).findings.map(({ problem }) => problem)).toContain(
      'current frozen baseline path differs from its adoption-time path',
    );
    renameSync(movedSpecPath, specPath);
    writeFileSync(baselinePath, `${JSON.stringify({ exempt: [relative] }, null, 2)}\n`);
    writeFileSync(
      specPath,
