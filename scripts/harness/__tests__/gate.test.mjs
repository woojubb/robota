...
describe('judge — GATE-IMPLEMENT reads the worktree', () => {
  ...
  it('does not count the auto-generated churn as a path outside the pair (#2376)', () => {
    // The lessons files are regenerated by any scan run — the judging run included — and the
    // pre-commit hook refuses to stage them, so a tree can never be both clean of them and
    // committable. The single owner of that exemption is `AUTO_GENERATED_CHURN`; a real path beside
    // them is still named.
    const { root, doc } = approvedWorkspace();
    gitInit(root);
    for (const churn of AUTO_GENERATED_CHURN) {
      mkdirSync(path.dirname(path.join(root, churn)), { recursive: true });
      writeFileSync(path.join(root, churn), 'regenerated by a scan\n');
    }
    const clean = judge(root, doc, 'GATE-IMPLEMENT', ['--lane', 'L2']);
    expect(clean.status, clean.stdout + clean.stderr).toBe(0);
    expect(readFileSync(doc, 'utf8')).not.toContain('outside the paired spec/Task');

    writeFileSync(path.join(root, 'src-change.mjs'), 'export const early = true;\n');
    const dirty = judge(root, doc, 'GATE-IMPLEMENT', ['--lane', 'L2']);
