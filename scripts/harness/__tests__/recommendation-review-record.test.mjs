...
describe('recommendation review record command', () => {
  ...
  it('binds the stable key to subject plus projection rather than commit identity', () => {
    const digest = decisionProjectionDigest(reviewedSpec());
    const subject = 'INFRA-999-review-record-fixture.md';
    const key = recommendationEndorsementKey(subject, digest);
    expect(key).toMatch(/^[0-9a-f]{64}$/);
    expect(recommendationEndorsementKey(subject, digest)).toBe(key);
    expect(recommendationEndorsementKey('INFRA-998-review-record-fixture.md', digest)).not.toBe(
      key,
    );
  });
});
