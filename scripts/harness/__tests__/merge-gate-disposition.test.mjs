...
function stubbedPath(prs) {
  ...
  writeFileSync(
    ...
    [
      ...
      '  const first = Object.values(prs)[0] ?? {};',
      '  const target = prs[num] ?? first;',
      '  if (target.threadsUnreadable) process.exit(1);',
      '  console.log(String(target.unresolved ?? 0));',
      '  process.exit(0);',
      '}',
      ...
      '    console.log([...new Set(comments.map((c) => c.author.login))].join(", "));',
      '  } else {',
      '    console.log(JSON.stringify(comments.filter((c) => re.test(c.author.login)).at(-1) ?? {}));',
      '  }',
      '  process.exit(0);',
      '}',
