...
export function parseCatalogue(text) {
  ...
  for (const line of lines) {
    ...
    if (heading) {
      ...
      current = { gate: heading[1], upgrade: upgrade ? { from: upgrade[1], to: upgrade[2] } : null, criteria: [], lines: [] };
...
export function parsePriorGateMap(text) {
  ...
  for (const cells of tableRows(section.body)) {
    ...
    if (/^GATE-[A-Z]+(?: \((?:continuation|correction)\))?$/.test(gate ?? '') && /^GATE-[A-Z]+$/.test(prior ?? '')) {
