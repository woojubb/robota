...
function readBaseline(root, sourceRevision = 'worktree') {
  ...
  const introductions = git(root, ['log', '--diff-filter=A', '--format=%H', '--', BASELINE])
    .split('\n')
    .filter(Boolean);
  const stagedIntroduction =
    introductions.length === 0 &&
    bytesAt(root, 'HEAD', BASELINE) === null &&
    indexText(root, BASELINE) === source;
  if (introductions.length !== 1 && !stagedIntroduction) {
    throw new Error(
      `recommendation-endorsement: ${BASELINE} must have exactly one immutable introduction commit.`,
    );
  }
  const introductionRevision = stagedIntroduction ? ':index' : introductions[0];
  const introducedBytes = stagedIntroduction
    ? indexText(root, BASELINE)
    : bytesAt(root, introductionRevision, BASELINE);
  ...
  const introductionAncestryTarget = stagedIntroduction ? 'HEAD' : introductionRevision;
  if (!isAncestor(root, parsed.adoptionRevision, introductionAncestryTarget)) {
    throw new Error(
  ...
  if (parsed.bootstrap !== null) {
    ...
    if (
      ...
      !/^[0-9a-f]{64}$/.test(parsed.bootstrap.endorsementKey)
    ) {
      throw new Error(
        'recommendation-endorsement: bootstrap tuple contains a malformed or wildcard value.',
      );
    }
    // The reviewed SHA is retained as provenance, but validity is intentionally independent of
    // whether a rebase or squash leaves that object reachable. The immutable introduction tree
    // carries the self-bootstrap projection and stable key that are mechanically checked.
    const reviewed = stagedIntroduction
      ? indexSpec(root, parsed.bootstrap.subject)
      : specAt(root, introductionRevision, parsed.bootstrap.subject);
    ...
    if (
      reviewedDigest !== parsed.bootstrap.projectionDigest ||
      parsed.bootstrap.endorsementKey !==
        recommendationEndorsementKey(parsed.bootstrap.subject, parsed.bootstrap.projectionDigest)
    ) {
      throw new Error(
...
function specsAt(root, revision, name) {
  const found = [];
...
function validReviewRecord(_root, record, subject, _targetRevision) {
...
function validCheckpointRecord(root, record, subject, targetRevision, targetSpec, paths) {
  ...
  return (
    validReviewRecord(root, record, subject, targetRevision) &&
...
export function isCommittedRecommendationCheckpoint(root, parent, commit, paths) {
  ...
  return (
    ...
    added.every((item) => validReviewRecord(root, item.record, subject, commit)) &&
...
export function isStagedRecommendationCheckpoint(root, paths) {
  ...
  return (
    targetSpec !== null &&
    targetSpec.state !== 'rejected' &&
    added.every((item) => validReviewRecord(root, item.record, subject, ':index')) &&
    ...
    if (
      ...
      if (adopted === baseSpec.text) {
        ...
        try {
          ...
          const persisted =
            errors.length === 0 &&
            validLatestAttestation(index.get(subject), subject, baseDigest).ok;
...
export function findRecommendationStagedFindings(root = WORKSPACE_ROOT, requestedBase) {
  ...
  for (const subject of subjects) {
    ...
    const previous = validLatestAttestation(previousRecords, subject, digest);
...
function validLatestAttestation(records, subject, digest) {
...
/** Current-endorsement evaluator for GATE-APPROVAL. Adoption exemptions never satisfy this API. */
export function currentRecommendationEndorsement(root, subject, markdown) {
  ...
  return validLatestAttestation(index.get(subject), subject, digest);
}
...
export function findRecommendationEndorsementFindings(root = WORKSPACE_ROOT) {
  ...
  for (const spec of specs) {
    ...
    const verdict = validLatestAttestation(index.get(spec.name), spec.name, digest);
    if (!verdict.ok) {
